...
void CharacterDatabaseConnection::DoPrepareStatements()
{
    ...
    // Creature
    PrepareStatement(CHAR_SEL_CREATURE_VENDOR, "SELECT spawnId, item, count, lastIncrementTime FROM creature_vendor WHERE instanceId = ?", CONNECTION_SYNCH);
	...
	//
	PrepareStatement(CHAR_SEL_ACCOUNT_CHARACTERS, "SELECT guid FROM characters WHERE account = ? AND level >= 20", CONNECTION_SYNCH);
}
