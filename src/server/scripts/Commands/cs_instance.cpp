...
public:
    ...
    static bool HandleInstanceGetBossStateCommand(ChatHandler* handler, char const* args)
    {
        ...
        //@todo: Lothloryen - add a distance parameter, as well as the limit of creatures to show.
        char* param1 = strtok((char*)args, " ");
        char* param2 = strtok(nullptr, " ");
        uint32 encounterId = 0;
        Player* player = nullptr;
        ...
        if (player /*&& state == IN_PROGRESS*/)
        {
            std::list<Creature*> pCreatureList;
            Trinity::AllCreaturesInRangeInCombat checker(player, 500.0f);
            Trinity::CreatureListSearcher<Trinity::AllCreaturesInRangeInCombat> searcher(player, pCreatureList, checker);
            ...
            if (!pCreatureList.empty())
            {
                for (std::list<Creature*>::iterator i = pCreatureList.begin(); i != pCreatureList.end(); ++i)
                {
                    ...
                    uint32 entry = (*i)->GetEntry();
                    uint32 guid = (*i)->GetGUID().GetCounter();

                    float x = (*i)->GetPositionX();
                    float y = (*i)->GetPositionY();
                    float z = (*i)->GetPositionZ();

                    std::string topThreatName = "";
                    uint32 topThreatEntry = 0;
                    uint32 topThreatGUID = 0;                

                    ThreatContainer::StorageType const &threatList = (*i)->getThreatManager().getThreatList();
                    ThreatContainer::StorageType::const_iterator itr;
                    uint32 count = 0;                    

                    /*
                    We could just use getHostileRefManager().getFirst(), but since we probably need to see more references in the
                    near future, I will leave the loop limited to 1 result here.
                    */
                    for (itr = threatList.begin(); itr != threatList.end(); ++itr)
                    {
                        ...
                        if (!unit)
                            continue;
                        ++count;
                        topThreatName = unit->GetName();
                        topThreatEntry = unit->GetEntry();
                        topThreatGUID = unit->GetGUID().GetCounter();
                    }

                    handler->PSendSysMessage(LANG_DEBUG_CREATURES_IN_COMBAT, name, entry, guid, x, y, z, topThreatName, topThreatEntry, topThreatGUID);
                }
            }
            else
                handler->PSendSysMessage("No creatures in combat within 500 yards.");
        }        
        return true;
    }
};
