            ...
            void HandleDummy(SpellEffIndex /*effIndex*/)
            {
                if (Creature* creatureTarget = GetHitCreature())
                {
					...
					if (uiNewEntry)
					{
						...
						if (Unit* caster = GetCaster())
							creatureTarget->GetMotionMaster()->MovePoint(0, caster->GetPositionX(), caster->GetPositionY(), caster->GetPositionZ());
					}                        
                }
            }
...
enum Kegs
{
    GO_RIPE_MOONSHINE_KEG = 185213,
    GO_GREEN_SPOT_KEG = 185206,
    GO_FERMENTED_SEED_BEER_KEG = 185214,
    RIPE_MOONSHINE_CREDIT_CREATURE = 22367,
    GREEN_SPOT_CREDIT_CREATURE = 22356,
    FERMENTED_SEED_BEER_CREDIT_CREATURE = 22368
};
        ...
        {            
            if (GetCaster() && GetHitUnit()) {                
                if (Player* player = GetCaster()->GetCharmerOrOwnerPlayerOrPlayerItself()) {                    
                    GameObject* go_keg1 = GetCaster()->FindNearestGameObject(GO_RIPE_MOONSHINE_KEG, 5.0f);
                    if (go_keg1)
                        player->KilledMonsterCredit(RIPE_MOONSHINE_CREDIT_CREATURE);
                    GameObject* go_keg2 = GetCaster()->FindNearestGameObject(GO_GREEN_SPOT_KEG, 5.0f);
                    if (go_keg2)
                        player->KilledMonsterCredit(GREEN_SPOT_CREDIT_CREATURE);
                    GameObject* go_keg3 = GetCaster()->FindNearestGameObject(GO_FERMENTED_SEED_BEER_KEG, 5.0f);
                    if (go_keg3)
                        player->KilledMonsterCredit(FERMENTED_SEED_BEER_CREDIT_CREATURE);                 
                }
            }
        }
...
{
    NPC_PUMPKIN = 23356,
    SPELL_DISMOUNT = 41064,
    SPELL_FIRE_VISUAL = 41284
};
...
class spell_quest_dragonmaw_race_generic : public SpellScriptLoader
{
    public:
        ...
        class spell_quest_dragonmaw_race_generic_SpellScript : public SpellScript
        {
            ...
            {                
                Unit* caster = GetCaster();
                float o = Position::NormalizeOrientation(caster->GetOrientation() + frand(0.0f, 6.28f));
            ...
            void HandleDummy(SpellEffIndex effIndex)
            {
                PreventHitDefaultEffect(effIndex);
                Unit* caster = GetCaster();            
                if (Creature* trigger = caster->SummonCreature(NPC_PUMPKIN, _x, _y, _z, 0.0f, TEMPSUMMON_TIMED_DESPAWN, 1500))
                {
                    trigger->CastSpell(trigger, SPELL_DISMOUNT, true);
                    trigger->CastSpell(trigger, SPELL_FIRE_VISUAL, true);
                }
            ...
            void Register() override
            {
                ...
                OnEffectHit += SpellEffectFn(spell_quest_dragonmaw_race_generic_SpellScript::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);                
            }
