...
enum DruidSpells
{
    ...
    SPELL_DRUID_PET_SCALING_01              = 35669,
    SPELL_DRUID_PET_SCALING_02              = 35670,
    SPELL_DRUID_PET_SCALING_03              = 35671,
    SPELL_DRUID_PET_SCALING_04              = 35672
};
        ...
        class spell_dru_lifebloom_AuraScript : public AuraScript
        {
            ...
            void AfterRemove(AuraEffect const* aurEff, AuraEffectHandleModes /*mode*/)
            {
                ...
                if (Unit* caster = GetCaster())
                {
                    ...
                    // restore mana only if aura was not stolen
                    if (!IsStolen())
                    {
            ...
            void HandleSteal(StealInfo* stealInfo)
            {
                if (Unit* target = GetUnitOwner())
                {
                    if (AuraEffect const* aurEff = GetEffect(EFFECT_1))
                    {
                        ...
                        if (Unit* caster = GetCaster())
                        {
                            healAmount = caster->SpellHealingBonusDone(target, GetSpellInfo(), healAmount, HEAL, 1);
                            target->CastCustomSpell(target, SPELL_DRUID_LIFEBLOOM_FINAL_HEAL, &healAmount, NULL, NULL, true, NULL, NULL, GetCasterGUID());

                            // restore mana
                            int32 returnMana = CalculatePct(caster->GetCreateMana(), GetSpellInfo()->ManaCostPercentage) * 1 / 2;
                            caster->CastCustomSpell(caster, SPELL_DRUID_LIFEBLOOM_ENERGIZE, &returnMana, NULL, NULL, true, NULL, NULL, GetCasterGUID());
                            return;
                        }
            ...
            void Register() override
            {
                ...
                AfterSteal += AuraStealFn(spell_dru_lifebloom_AuraScript::HandleSteal);
            }
        };
...
// 35669 - Druid Pet Scaling 01 (SERVERSIDE)
// 35670 - Druid Pet Scaling 02 (SERVERSIDE)
// 35671 - Druid Pet Scaling 03 (SERVERSIDE)
// 35672 - Druid Pet Scaling 04 (SERVERSIDE)
class spell_dru_pet_scaling : public SpellScriptLoader
{
public:
    spell_dru_pet_scaling() : SpellScriptLoader("spell_dru_pet_scaling") { }

    class spell_dru_pet_scaling_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_dru_pet_scaling_AuraScript);

        bool Validate(SpellInfo const* /*spellInfo*/) override
        {
            if (!sSpellMgr->GetSpellInfo(SPELL_DRUID_PET_SCALING_01) || !sSpellMgr->GetSpellInfo(SPELL_DRUID_PET_SCALING_02) ||
                !sSpellMgr->GetSpellInfo(SPELL_DRUID_PET_SCALING_03) || !sSpellMgr->GetSpellInfo(SPELL_DRUID_PET_SCALING_04))
                return false;
            return true;
        }
        ...
        void CalculateAPAmount(AuraEffect const* aurEff, int32& amount, bool& /*canBeRecalculated*/)
        {
            ...
            {
                int32 mod = 105;
                int32 natureMod = owner->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_NATURE);

                amount = CalculatePct(std::max<int32>(0, natureMod), mod);
            }
        }
        ...
        void Register() override
        {
            if (m_scriptSpellId != SPELL_DRUID_PET_SCALING_01)
                DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_dru_pet_scaling_AuraScript::CalculateResistanceAmount, EFFECT_ALL, SPELL_AURA_MOD_RESISTANCE);

            if (m_scriptSpellId == SPELL_DRUID_PET_SCALING_01 || m_scriptSpellId == SPELL_DRUID_PET_SCALING_02)
                DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_dru_pet_scaling_AuraScript::CalculateStatAmount, EFFECT_ALL, SPELL_AURA_MOD_STAT);

            if (m_scriptSpellId == SPELL_DRUID_PET_SCALING_01)
            {
                DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_dru_pet_scaling_AuraScript::CalculateAPAmount, EFFECT_ALL, SPELL_AURA_MOD_ATTACK_POWER);
                DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_dru_pet_scaling_AuraScript::CalculateSPAmount, EFFECT_ALL, SPELL_AURA_MOD_DAMAGE_DONE);
            }
        }
    };
    ...
    AuraScript* GetAuraScript() const override
    {
        return new spell_dru_pet_scaling_AuraScript();
    }
};
