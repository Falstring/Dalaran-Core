...
enum DeathKnightSpells
{
    ...
    SPELL_DEATH_KNIGHT_BONE_SHIELD              = 49222,
	SPELL_DEATH_KNIGHT_KILLING_MACHINE_R1       = 51123
};
...
{
    NPC_DK_GHOUL                                = 26125,
    NPC_DK_ARMY_OF_THE_DEAD                     = 24207,
    NPC_DK_EBON_GARGOYLE                        = 27829
};
        ...
        class spell_dk_death_and_decay_AuraScript : public AuraScript
        {
            ...
            void HandleDummyTick(AuraEffect const* aurEff)
            {
                ...
                if (Unit* caster = GetCaster())
                    caster->CastCustomSpell(SPELL_DK_DEATH_AND_DECAY_DAMAGE, SPELLVALUE_BASE_POINT0, aurEff->GetAmount(), GetTarget(), triggerFlags, nullptr, aurEff);
            }
...
public:
    ...
    class spell_dk_death_grip_initial_SpellScript : public SpellScript
    {
        ...
        void HandleDummy(SpellEffIndex /*effIndex*/)
        {
            ...
            if (caster != target)
                caster->CastSpell(target, SPELL_DK_DEATH_GRIP, true);
            else
                baseTarget->CastSpell(caster, SPELL_DK_DEATH_GRIP, true);
        }
...
public:
    ...
    class spell_dk_pet_scaling_AuraScript : public AuraScript
    {
        ...
        bool Validate(SpellInfo const* /*spellInfo*/) override
        {
            if (!sSpellMgr->GetSpellInfo(SPELL_DEATH_KNIGHT_PET_SCALING_01) || !sSpellMgr->GetSpellInfo(SPELL_DEATH_KNIGHT_PET_SCALING_02) ||
                !sSpellMgr->GetSpellInfo(SPELL_DEATH_KNIGHT_PET_SCALING_03))
                return false;
            return true;
        }
        ...
        void CalculateStatAmount(AuraEffect const* aurEff, int32& amount, bool& /*canBeRecalculated*/)
        {
            ...
            {
                Stats stat = Stats(aurEff->GetMiscValue());
                int32 mod = stat == STAT_STRENGTH ? 70 : 30;

                // Ebon Garogyle - Only inherit 30 % Stamina
                if (GetUnitOwner()->GetEntry() == NPC_DK_EBON_GARGOYLE)
                {
                    amount = CalculatePct(std::max<int32>(0, owner->GetStat(stat)), mod);
                    return;
                }
                ...
                if (AuraEffect const* aurEff = owner->GetAuraEffect(SPELL_AURA_MOD_TOTAL_STAT_PERCENTAGE, SPELLFAMILY_DEATHKNIGHT, 3010, EFFECT_0))
                    AddPct(mod, aurEff->GetSpellInfo()->Effects[EFFECT_1].CalcValue());
        ...
        void CalculateSPAmount(AuraEffect const* aurEff, int32& amount, bool& /*canBeRecalculated*/)
        {
            ...
            if (Unit* owner = GetUnitOwner()->GetOwner())
            {
                ...
                // Impurity
                if (AuraEffect const* aurEff = owner->GetDummyAuraEffect(SPELLFAMILY_DEATHKNIGHT, 1986, EFFECT_0))
					mod += std::ceil(CalculatePct(mod, aurEff->GetAmount()));
        ...
        void CalculatePctDamageAmount(AuraEffect const* aurEff, int32& amount, bool& /*canBeRecalculated*/)
        {
            if (Unit* owner = GetUnitOwner()->GetOwner())
            {
                int32 bonus = 0;
                ...
                // Bone Shield
                if (AuraEffect const* aurEff = owner->GetAuraEffect(SPELL_DEATH_KNIGHT_BONE_SHIELD, EFFECT_1))
        ...
        {
            if (Unit* owner = GetUnitOwner()->GetOwner())
                if (owner->m_modAttackSpeedPct[BASE_ATTACK] < 1.0f)
        ...
        void Register() override
        {
            if (m_scriptSpellId == SPELL_DEATH_KNIGHT_PET_SCALING_01)
            {
            ...
            else if (m_scriptSpellId == SPELL_DEATH_KNIGHT_PET_SCALING_02)
            {
                DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_dk_pet_scaling_AuraScript::CalculatePctDamageAmount, EFFECT_0, SPELL_AURA_MOD_DAMAGE_PERCENT_DONE);
                DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_dk_pet_scaling_AuraScript::CalculateHasteAmount, EFFECT_1, SPELL_AURA_MELEE_SLOW);
            }
            else if (m_scriptSpellId == SPELL_DEATH_KNIGHT_PET_SCALING_03)
            {
                DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_dk_pet_scaling_AuraScript::CalculateHitAmount, EFFECT_0, SPELL_AURA_MOD_HIT_CHANCE);
                DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_dk_pet_scaling_AuraScript::CalculateSpellHitAmount, EFFECT_1, SPELL_AURA_MOD_SPELL_HIT_CHANCE);
            }
...
public:
	...
	class spell_dk_killing_machine_AuraScript : public AuraScript
	{
		...
		void Register()
		{
			DoCheckProc += AuraCheckProcFn(spell_dk_killing_machine_AuraScript::CheckProc);
		}
	};
...
{
    public:
        RuneStates(Player* player)
        {
            for (uint32 i = 0; i<BLOOD_RUNE_COUNT; i++)
            {
                RuneState state;
                state.id = i;
                state.type = player->GetCurrentRune(i);
                state.active = !player->GetRuneCooldown(i);
                states[i] = state;
            }
        }

        uint32 GetInactiveCount(int32 type = -1) { return GetCount(type, false); }
        uint32 GetActiveCount(int32 type = -1) { return GetCount(type, true); }

        int32 GetFirstInactive(int32 type = -1) { return GetFirst(type, false); }
        int32 GetFirstActive(int32 type = -1) { return GetFirst(type, true); }

        int32 GetFirst(int32 type = -1, int32 active = -1)
        {
            for (uint32 i = 0; i < BLOOD_RUNE_COUNT; i++)
                if ((type < 0 || states[i].type == type) && (active < 0 || states[i].active == (active != 0)))
                    return states[i].id;
            return -1;
        }

        uint32 GetCount(int32 type = -1, int32 active = -1)
        {
            uint32 count = 0;
            for (uint32 i = 0; i < BLOOD_RUNE_COUNT; i++)
                if ((type < 0 || states[i].type == type) && (active < 0 || states[i].active == (active != 0)))
    ...
    private:
        ...
        static const uint32 BLOOD_RUNE_COUNT = 2;
        RuneState states[BLOOD_RUNE_COUNT];
};

// 45529 - Blood Tap
class spell_dk_blood_tap_activate : public SpellScriptLoader
{
    public:
        spell_dk_blood_tap_activate() : SpellScriptLoader("spell_dk_blood_tap_activate") { }

        class spell_dk_blood_tap_activate_SpellScript : public SpellScript
        {
            PrepareSpellScript(spell_dk_blood_tap_activate_SpellScript);
            ...
            void PickRune()
            {
                ...
                Player* player = GetCaster()->ToPlayer();
                RuneStates runes(player);
                int32 id = -1;

                if (runes.GetInactiveCount())
                    id = runes.GetFirstInactive();
                else if (runes.GetActiveCount(RUNE_BLOOD))
                    id = runes.GetFirstActive(RUNE_BLOOD);
                else
                    id = 0;

                player->SetBloodTapRuneIndex(id);
            }

            void ActivateRune(SpellEffIndex effIndex)
            {
                PreventHitDefaultEffect(effIndex);
                ...
                int32 runeId = player->GetBloodTapRuneIndex();
                if (runeId != -1)
                    player->SetRuneCooldown(runeId, 0);
            }

            void Register() override
            {
                OnCast += SpellCastFn(spell_dk_blood_tap_activate_SpellScript::PickRune);
                OnEffectLaunch += SpellEffectFn(spell_dk_blood_tap_activate_SpellScript::ActivateRune, EFFECT_0, SPELL_EFFECT_ACTIVATE_RUNE);
            }
        };

        SpellScript* GetSpellScript() const override
        {
            return new spell_dk_blood_tap_activate_SpellScript();
        }
};

// 45529 - Blood Tap
class spell_dk_blood_tap_convert : public SpellScriptLoader
{
    public:
        spell_dk_blood_tap_convert() : SpellScriptLoader("spell_dk_blood_tap_convert") { }

        class spell_dk_blood_tap_convert_AuraScript : public AuraScript
        {
            PrepareAuraScript(spell_dk_blood_tap_convert_AuraScript);

            void ConvertRune(AuraEffect const* aurEff, AuraEffectHandleModes /*mode*/)
            {
                PreventDefaultAction();
                ...
                int32 runeId = player->GetBloodTapRuneIndex();
                if (runeId != -1)
                    player->AddRuneByAuraEffect(runeId, RUNE_DEATH, aurEff);
            }
            ...
            void OnRemove(AuraEffect const* aurEff, AuraEffectHandleModes /*mode*/)
            {
                ...
                player->SetBloodTapRuneIndex(-1);
            }

            void Register() override
            {
                OnEffectApply += AuraEffectApplyFn(spell_dk_blood_tap_convert_AuraScript::ConvertRune, EFFECT_1, SPELL_AURA_CONVERT_RUNE, AURA_EFFECT_HANDLE_REAL);
                OnEffectRemove += AuraEffectRemoveFn(spell_dk_blood_tap_convert_AuraScript::OnRemove, EFFECT_1, SPELL_AURA_CONVERT_RUNE, AURA_EFFECT_HANDLE_REAL);
            }
        };

        AuraScript* GetAuraScript() const override
        {
            return new spell_dk_blood_tap_convert_AuraScript();
        }
};

class BossCheckPredicate
{
public:
    bool operator()(WorldObject* object) const
    {
        if (!object->ToCreature())
            return false;
        return object->ToCreature()->isWorldBoss();
    }
};
...
public:
    ...
    class spell_dk_ghul_taunt_SpellScript : public SpellScript
    {
        ...
        void FilterTargets(std::list<WorldObject*>& targetList)
        {
            targetList.remove_if(BossCheckPredicate());
        }
...
void AddSC_deathknight_spell_scripts()
{
    ...
    new spell_dk_blood_tap_convert();
    new spell_dk_blood_tap_activate();
