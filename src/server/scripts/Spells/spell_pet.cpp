...
class spell_gen_pet_calculate : public SpellScriptLoader
{
    public:
        ...
        class spell_gen_pet_calculate_AuraScript : public AuraScript
        {
            ...
            {
				//std::cout << " \n AMOUNT PREVIO A TODO: " << uint32(amount) << "\n";
                if (Player* owner = GetCaster()->GetOwner()->ToPlayer())
                {
                    // For others recalculate it from:
                   ...
                   CritMelee += owner->GetMeleeCritFromAgility();
					...
					//std::cout << " \n TOTAL: "<< uint32(amount)<< "CRIT FROM AGILITY: " << uint32(owner->GetMeleeCritFromAgility()) << " - GetTotalAuraModifier(SPELL_AURA_MOD_WEAPON_CRIT_PERCENT):" << uint32(owner->GetTotalAuraModifier(SPELL_AURA_MOD_WEAPON_CRIT_PERCENT)) << " - GetTotalAuraModifier(SPELL_AURA_MOD_CRIT_PCT): " << uint32(owner->GetTotalAuraModifier(SPELL_AURA_MOD_CRIT_PCT)) << " - GetRatingBonusValue(CR_CRIT_MELEE): " << uint32(owner->GetRatingBonusValue(CR_CRIT_MELEE)) << "\n";
                }
            }
