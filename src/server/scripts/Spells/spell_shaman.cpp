...
enum ShamanSpells
{
    ...
    SPELL_SHAMAN_LAVA_LASH_GLYPH                = 55444,
    ...
    SPELL_SHAMAN_FERAL_SPIRIT_PET_SCALING_01    = 35674,
    SPELL_SHAMAN_FERAL_SPIRIT_PET_SCALING_02    = 35675,
    SPELL_SHAMAN_FERAL_SPIRIT_PET_SCALING_03    = 35676,
    SPELL_SHAMAN_FERAL_SPIRIT_PET_SCALING_04    = 61783,
    SPELL_SHAMAN_GLYPH_OF_FERAL_SPIRIT          = 63271,
    SPELL_SHAMAN_FIRE_ELEMENTAL_PET_SCALING_01  = 35665,
    SPELL_SHAMAN_FIRE_ELEMENTAL_PET_SCALING_02  = 35666,
    SPELL_SHAMAN_FIRE_ELEMENTAL_PET_SCALING_03  = 35667,
    SPELL_SHAMAN_FIRE_ELEMENTAL_PET_SCALING_04  = 35668,
    SPELL_SHAMAN_EARTH_ELEMENTAL_PET_SCALING_01 = 65225,
    SPELL_SHAMAN_EARTH_ELEMENTAL_PET_SCALING_02 = 65226,
    SPELL_SHAMAN_EARTH_ELEMENTAL_PET_SCALING_03 = 65227,
    SPELL_SHAMAN_EARTH_ELEMENTAL_PET_SCALING_04 = 65228
};
        ...
        class spell_sha_earth_shield_AuraScript : public AuraScript
        {
            ...
            bool CheckProc(ProcEventInfo& /*eventInfo*/)
            {
				...
				if (Creature* creature = GetTarget()->ToCreature())
					if (creature->GetSpellHistory()->HasCooldown(SPELL_SHAMAN_EARTH_SHIELD_HEAL))
						return false;
                return true;
            }
            ...
            void HandleProc(AuraEffect const* aurEff, ProcEventInfo& /*eventInfo*/)
            {
				...
				if (Creature* creature = GetTarget()->ToCreature())
				{
					creature->GetSpellHistory()->AddCooldown(SPELL_SHAMAN_EARTH_SHIELD_HEAL, 0, std::chrono::seconds(3));
					return;
				}
            }
    ...
    public:
        ...
        class spell_sha_fire_nova_SpellScript : public SpellScript
        {
            ...
            SpellCastResult CheckCast()
            {
                ...
                {
                    if (caster->GetDistance(totem) > 30.0f)
            ...
            void HandleDummy(SpellEffIndex /*effIndex*/)
            {
                if (Player* caster = GetCaster()->ToPlayer())
                {
                    if (caster->GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_OFFHAND))
                    {
                        ...
                        if (caster->HasAura(SPELL_SHAMAN_LAVA_LASH_GLYPH))
                            damage += 10;
...
// 35674 - Shaman Feral Spirit Pet Scaling 01 (SERVERSIDE)
// 35675 - Shaman Feral Spirit Pet Scaling 02 (SERVERSIDE)
// 35676 - Shaman Feral Spirit Pet Scaling 03 (SERVERSIDE)
// 61783 - Shaman Feral Spirit Pet Scaling 04
...
public:
    ...
    class spell_sha_feral_spirit_scaling_AuraScript : public AuraScript
    {
        ...
        bool Validate(SpellInfo const* /*spellInfo*/) override
        {
            if (!sSpellMgr->GetSpellInfo(SPELL_SHAMAN_FERAL_SPIRIT_PET_SCALING_01) || !sSpellMgr->GetSpellInfo(SPELL_SHAMAN_FERAL_SPIRIT_PET_SCALING_02) ||
                !sSpellMgr->GetSpellInfo(SPELL_SHAMAN_FERAL_SPIRIT_PET_SCALING_03) || !sSpellMgr->GetSpellInfo(SPELL_SHAMAN_FERAL_SPIRIT_PET_SCALING_04))
                return false;
            return true;
        }
        ...
        void CalculateAPAmount(AuraEffect const* aurEff, int32& amount, bool& /*canBeRecalculated*/)
        {
            ...
            {
                int32 mod = 30;
                float ownerAP = owner->GetTotalAttackPowerValue(BASE_ATTACK);

                // Glyph of Feral Spirit
                if (AuraEffect const* aurEff = owner->GetAuraEffect(SPELL_SHAMAN_GLYPH_OF_FERAL_SPIRIT, EFFECT_0))
        ...
        void CalculateModDamagePercentDoneAmount(AuraEffect const* aurEff, int32& amount, bool& /*canBeRecalculated*/)
        {
            if (Unit* owner = GetUnitOwner()->GetOwner())
            {
                float DoneTotalMod = 1.0f;

                Unit::AuraEffectList const& mModDamagePercentDone = owner->GetAuraEffectsByType(SPELL_AURA_MOD_DAMAGE_PERCENT_DONE);
                for (Unit::AuraEffectList::const_iterator i = mModDamagePercentDone.begin(); i != mModDamagePercentDone.end(); ++i)
                    DoneTotalMod += AddPct(DoneTotalMod, (*i)->GetAmount());
        ...
        void Register() override
        {
            if (m_scriptSpellId == SPELL_SHAMAN_FERAL_SPIRIT_PET_SCALING_01)
            {
            ...
            else if (m_scriptSpellId == SPELL_SHAMAN_FERAL_SPIRIT_PET_SCALING_02 || m_scriptSpellId == SPELL_SHAMAN_FERAL_SPIRIT_PET_SCALING_03)
            ...
            else if (m_scriptSpellId == SPELL_SHAMAN_FERAL_SPIRIT_PET_SCALING_04)
            {
                DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_sha_feral_spirit_scaling_AuraScript::CalculateHitAmount, EFFECT_0, SPELL_AURA_MOD_HIT_CHANCE);
                DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_sha_feral_spirit_scaling_AuraScript::CalculateSpellHitAmount, EFFECT_1, SPELL_AURA_MOD_SPELL_HIT_CHANCE);
                DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_sha_feral_spirit_scaling_AuraScript::CalculateModDamagePercentDoneAmount, EFFECT_2, SPELL_AURA_MOD_DAMAGE_PERCENT_DONE);
            }
...
// 35665 - Fire Elemental Pet Scaling 01 (SERVERSIDE)
// 35666 - Fire Elemental Pet Scaling 02 (SERVERSIDE)
// 35667 - Fire Elemental Pet Scaling 03 (SERVERSIDE)
// 35668 - Fire Elemental Pet Scaling 04 (SERVERSIDE)
// 65225 - Earth Elemental Pet Scaling 01 (SERVERSIDE)
// 65226 - Earth Elemental Pet Scaling 02 (SERVERSIDE)
// 65227 - Earth Elemental Pet Scaling 03 (SERVERSIDE)
// 65228 - Earth Elemental Pet Scaling 04 (SERVERSIDE)
class spell_sha_elemental_scaling : public SpellScriptLoader
{
public:
    spell_sha_elemental_scaling() : SpellScriptLoader("spell_sha_elemental_scaling") { }

    class spell_sha_elemental_scaling_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_sha_elemental_scaling_AuraScript);

        bool Validate(SpellInfo const* /*spellInfo*/) override
        {
            if (!sSpellMgr->GetSpellInfo(SPELL_SHAMAN_FIRE_ELEMENTAL_PET_SCALING_01) || !sSpellMgr->GetSpellInfo(SPELL_SHAMAN_FIRE_ELEMENTAL_PET_SCALING_02) ||
                !sSpellMgr->GetSpellInfo(SPELL_SHAMAN_FIRE_ELEMENTAL_PET_SCALING_03) || !sSpellMgr->GetSpellInfo(SPELL_SHAMAN_FIRE_ELEMENTAL_PET_SCALING_04) ||
                !sSpellMgr->GetSpellInfo(SPELL_SHAMAN_EARTH_ELEMENTAL_PET_SCALING_01) || !sSpellMgr->GetSpellInfo(SPELL_SHAMAN_EARTH_ELEMENTAL_PET_SCALING_02) ||
                !sSpellMgr->GetSpellInfo(SPELL_SHAMAN_EARTH_ELEMENTAL_PET_SCALING_03) || !sSpellMgr->GetSpellInfo(SPELL_SHAMAN_EARTH_ELEMENTAL_PET_SCALING_04))
                return false;
            return true;
        }
        ...
        void CalculateAPAmount(AuraEffect const* aurEff, int32& amount, bool& /*canBeRecalculated*/)
        {
            if (Unit* owner = GetUnitOwner()->GetOwner())
            {
                bool fireElemental = GetUnitOwner()->GetEntry() == NPC_FIRE_ELEMENTAL;
                int32 mod = fireElemental ? 300 : 150;
                int32 fireMod = owner->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_FIRE);

                amount = CalculatePct(std::max<int32>(0, fireMod), mod);
            }
        }
        ...
        void Register() override
        {
            if (m_scriptSpellId != SPELL_SHAMAN_FIRE_ELEMENTAL_PET_SCALING_01 && m_scriptSpellId != SPELL_SHAMAN_EARTH_ELEMENTAL_PET_SCALING_01)
                DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_sha_elemental_scaling_AuraScript::CalculateResistanceAmount, EFFECT_ALL, SPELL_AURA_MOD_RESISTANCE);

            if (m_scriptSpellId == SPELL_SHAMAN_FIRE_ELEMENTAL_PET_SCALING_02 || m_scriptSpellId == SPELL_SHAMAN_EARTH_ELEMENTAL_PET_SCALING_02)
                DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_sha_elemental_scaling_AuraScript::CalculateStatAmount, EFFECT_ALL, SPELL_AURA_MOD_STAT);

            if (m_scriptSpellId == SPELL_SHAMAN_FIRE_ELEMENTAL_PET_SCALING_01 || m_scriptSpellId == SPELL_SHAMAN_EARTH_ELEMENTAL_PET_SCALING_01)
            {
                DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_sha_elemental_scaling_AuraScript::CalculateAPAmount, EFFECT_ALL, SPELL_AURA_MOD_ATTACK_POWER);
                DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_sha_elemental_scaling_AuraScript::CalculateSPAmount, EFFECT_ALL, SPELL_AURA_MOD_DAMAGE_DONE);
            }
        }
    };
    ...
    AuraScript* GetAuraScript() const override
    {
        return new spell_sha_elemental_scaling_AuraScript();
    }
};
...
void AddSC_shaman_spell_scripts()
{
    ...
    new spell_sha_elemental_scaling();
}
