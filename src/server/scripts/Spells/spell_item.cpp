            ...
            void HandleDummy(SpellEffIndex /*effIndex*/)
            {
                if (Creature* target = GetHitCreature())
                    ...
                    {                        
                        ...
                        target->SetFloatValue(UNIT_FIELD_MINDAMAGE, 400.0f);
                        target->SetFloatValue(UNIT_FIELD_MAXDAMAGE, 700.0f);
    ...
    public:
        ...
        class spell_item_rocket_boots_SpellScript : public SpellScript
        {
            ...
            void HandleDummy(SpellEffIndex /* effIndex */)
            {
                ...
                caster->GetSpellHistory()->ResetCooldown(SPELL_ROCKET_BOOTS);
                caster->CastSpell(caster, SPELL_ROCKET_BOOTS_PROC, true, NULL);
                caster->GetSpellHistory()->AddCooldown(SPELL_ROCKET_BOOTS, 0, std::chrono::minutes(5));
            }
...
enum ShiftingNaaruSilver
{
    SPELL_LIMITLESS_POWER = 45044,
    SPELL_POWER_CIRCLE = 45043
};
    ...
    class spell_item_shifting_naaru_silver_AuraScript : public AuraScript
    {
        ...
        void OnApply(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
        {
            if (GetTarget() == GetCaster())
                if (Aura* aur = GetTarget()->AddAura(SPELL_LIMITLESS_POWER, GetTarget()))
                    aur->SetDuration(GetDuration());
        }

        void OnRemove(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
        {
            if (Aura* aur = GetTarget()->GetAura(SPELL_LIMITLESS_POWER, GetTarget()->GetGUID()))
                aur->SetDuration(0);
        }

        void OnBaseRemove(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
        {
            GetUnitOwner()->RemoveDynObject(SPELL_POWER_CIRCLE);
        }

        void Register()
        {
            if (m_scriptSpellId == SPELL_POWER_CIRCLE)
            {
...
class spell_item_crazy_alchemists_potion : public SpellScriptLoader
{
    enum POTION_EFFECTS
    {
        SPELL_WILD_MAGIC            = 53909,
        SPELL_SPEED                 = 53908,
        SPELL_REJUVENATION_POTION   = 53761,
        SPELL_INDESTRUCTIBLE        = 53762,
        SPELL_ARCANE_PROTECTION     = 53910,
        SPELL_FIRE_PROTECTION       = 53911,
        SPELL_FROST_PROTECTION      = 53913,
        SPELL_NATURE_PROTECTION     = 53914,
        SPELL_SHADOW_PROTECTION     = 53915,
    ...
    public:
        ...
        class spell_item_crazy_alchemists_potion_SpellScript : public SpellScript
        {
            ...
            void HandleHeal(SpellEffIndex /*effIndex*/)
            {
                if (Player* target = GetHitPlayer())
                {
                    int choice = urand(0, 21);
                    if (choice > 0 && choice < 12)
                        target->UpdatePotionCooldown();
                    
                    switch(choice)
                    {
                        case 1:                            
                            if ((target->getPowerType() ==  POWER_MANA) && (target->GetPower(POWER_MANA) < target->GetMaxPower(POWER_MANA)))
                            {
                                target->CastSpell(target, SPELL_RESTORE_MANA, true);
                                break;
                            }                                
                        case 2: 
                            target->CastSpell(target, SPELL_WILD_MAGIC, true);
                        ...
                        case 3:
                            ...
                            {
                                target->CastSpell(target, SPELL_HEALING_POTION, true);
                                break;
                            }                                
                        case 4:
                            ...
                            break;
                        case 5:
                            target->CastSpell(target, SPELL_INDESTRUCTIBLE, true);
                            break;
                        case 6:
                            target->CastSpell(target, SPELL_ARCANE_PROTECTION, true);
                            break;
                        case 7:
                            target->CastSpell(target, SPELL_FIRE_PROTECTION, true);
                            break;
                        case 8:
                            target->CastSpell(target, SPELL_FROST_PROTECTION, true);
                            break;
                        case 9:
                            target->CastSpell(target, SPELL_NATURE_PROTECTION, true);
                            break;
                        case 10:
                            target->CastSpell(target, SPELL_SHADOW_PROTECTION, true);
                            break;                            
                        case 11:
                            target->CastSpell(target, SPELL_REJUVENATION_POTION, true);
                            break;
                    }                    
                }
            }
