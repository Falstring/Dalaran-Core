...
enum PriestSpells
{
    ...
    SPELL_PRIEST_T9_HEALING_4P                      = 67202,
    SPELL_PRIEST_SERENDIPITY                        = 63730,
    ...
    public:
        ...
        class spell_pri_divine_aegis_AuraScript : public AuraScript
        {
            ...
            void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
            {
                ...
                if (AuraEffect* aurEff = GetCaster()->GetAuraEffect(SPELL_PRIEST_T9_HEALING_4P, EFFECT_0))
                    AddPct(absorb, aurEff->GetAmount());
            ...
            void HandleApplyEffect(AuraEffect const* aurEff, AuraEffectHandleModes /*mode*/)
            {
                if (Unit* caster = GetCaster())
                {
                    ...
                    if (AuraEffect const* empoweredRenewAurEff = caster->GetDummyAuraEffect(SPELLFAMILY_PRIEST, PRIEST_ICON_ID_EMPOWERED_RENEW_TALENT, EFFECT_1))
                    {
                        ...
                        // Item - Priest T9 Healing 4P Bonus
                        if (AuraEffect* aurEff = caster->GetAuraEffect(SPELL_PRIEST_T9_HEALING_4P, EFFECT_0))
...
class spell_pri_serendipity : public SpellScriptLoader
{
    public:
        ...
        class spell_pri_serendipity_AuraScript : public AuraScript
        {
            ...
            bool Validate(SpellInfo const* /*spellInfo*/) override
            {
                if (!sSpellMgr->GetSpellInfo(SPELL_PRIEST_SERENDIPITY))
                    return false;
                return true;
            }
            ...
            bool CheckProc(ProcEventInfo& eventInfo)
            {
                ...
                // Binding Heal self heal
                if (spellInfo && (spellInfo->SpellFamilyFlags[1] & 0x4) && eventInfo.GetActionTarget() == eventInfo.GetActor())
                    return false;
                return true;
            }
            ...
            void Register() override
            {
                DoCheckProc += AuraCheckProcFn(spell_pri_serendipity_AuraScript::CheckProc);
            }
        };
...
// 35661 - Priest Pet Scaling 01 (SERVERSIDE)
// 35662 - Priest Pet Scaling 02 (SERVERSIDE)
// 35663 - Priest Pet Scaling 03 (SERVERSIDE)
// 35664 - Priest Pet Scaling 04 (SERVERSIDE)
class spell_pri_pet_scaling : public SpellScriptLoader
{
public:
    spell_pri_pet_scaling() : SpellScriptLoader("spell_pri_pet_scaling") { }

    class spell_pri_pet_scaling_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_pri_pet_scaling_AuraScript);
        ...
        void Register() override
        {
            if (m_scriptSpellId != SPELL_PRIEST_PET_SCALING_01)
                DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_pri_pet_scaling_AuraScript::CalculateResistanceAmount, EFFECT_ALL, SPELL_AURA_MOD_RESISTANCE);

            if (m_scriptSpellId == SPELL_PRIEST_PET_SCALING_01 || m_scriptSpellId == SPELL_PRIEST_PET_SCALING_02)
                DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_pri_pet_scaling_AuraScript::CalculateStatAmount, EFFECT_ALL, SPELL_AURA_MOD_STAT);

            if (m_scriptSpellId == SPELL_PRIEST_PET_SCALING_01)
            {
                DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_pri_pet_scaling_AuraScript::CalculateAPAmount, EFFECT_ALL, SPELL_AURA_MOD_ATTACK_POWER);
                DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_pri_pet_scaling_AuraScript::CalculateSPAmount, EFFECT_ALL, SPELL_AURA_MOD_DAMAGE_DONE);
            }
        }
    };
    ...
    AuraScript* GetAuraScript() const override
    {
        return new spell_pri_pet_scaling_AuraScript();
    }
};
