...
enum WarriorSpells
{
    ...
    SPELL_WARRIOR_DEATH_WISH                        = 12292,
    SPELL_WARRIOR_TASTE_FOR_BLOOD                   = 56636,
    SPELL_WARRIOR_TASTE_FOR_BLOOD_PROC              = 60503
};
        ...
        private:
            ...
            bool CheckProc(ProcEventInfo& eventInfo)
            {
                ...
                if (const SpellInfo* procSpell = eventInfo.GetDamageInfo()->GetSpellInfo())
                    if (procSpell->Id == SPELL_WARRIOR_SWEEPING_STRIKES_EXTRA_ATTACK_1 ||
                        procSpell->Id == SPELL_WARRIOR_SWEEPING_STRIKES_EXTRA_ATTACK_2)
                        return false;

                //! HACK due to currenct proc system implementation
                if (Player* player = GetTarget()->ToPlayer())
                    if (player->GetSpellHistory()->HasCooldown(SPELL_WARRIOR_SWEEPING_STRIKES_EXTRA_ATTACK_2))
                        return false;
            ...
            void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
            {
                ...
                if (eventInfo.GetDamageInfo())
                {
                    ...
                    if (spellInfo && (spellInfo->Id == SPELL_WARRIOR_BLADESTORM_PERIODIC_WHIRLWIND || (spellInfo->Id == SPELL_WARRIOR_EXECUTE && !_procTarget->HasAuraState(AURA_STATE_HEALTHLESS_20_PERCENT))))
                    {
                        ...
                        /// @hack: due to currenct proc system implementation
                        // We need to put ICD to 500 milliseconds in order to drop only one sweeping strike charge
                        if (Player* player = GetTarget()->ToPlayer())
                            player->GetSpellHistory()->AddCooldown(SPELL_WARRIOR_SWEEPING_STRIKES_EXTRA_ATTACK_2, 0, std::chrono::milliseconds(500));
                    }
                    else
                    {
...
// -12317, -46867, -29593 - Enrage(Enrage), Enrage (Wrecking Crew), Enrage (Improved Defensive Stance)
class spell_warr_enrage : public SpellScriptLoader
{
    public:
        ...
        class spell_warr_enrage_AuraScript : public AuraScript
        {
            ...
            bool CheckProc(ProcEventInfo& /*eventInfo*/)
            {
                // Don't proc if Warrior has Death Wish
                return !GetCaster()->HasAura(SPELL_WARRIOR_DEATH_WISH);
            }
...
class spell_warr_taste_for_blood : public SpellScriptLoader
{
    public:
        ...
        class spell_warr_taste_for_blood_AuraScript : public AuraScript
        {
            ...
            bool Validate(SpellInfo const* /*spellInfo*/) override
            {
                if (!sSpellMgr->GetSpellInfo(SPELL_WARRIOR_TASTE_FOR_BLOOD) || !sSpellMgr->GetSpellInfo(SPELL_WARRIOR_TASTE_FOR_BLOOD_PROC))
                    return false;
                return true;
            }
            ...
            bool CheckProc(ProcEventInfo& /*eventInfo*/)
            {
                //! HACK due to currenct proc system implementation
                if (Player* player = GetTarget()->ToPlayer())
                    if (player->GetSpellHistory()->HasCooldown(SPELL_WARRIOR_TASTE_FOR_BLOOD))
                        return false;
                return true;
            }
            ...
            void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
            {
                ...
                if (AuraEffect* protEff = eventInfo.GetProcTarget()->GetAuraEffect(SPELL_WARRIOR_TASTE_FOR_BLOOD_PROC, 0, eventInfo.GetActor()->GetGUID()))
                {
                    // Refresh and return to prevent replacing the aura
                    protEff->GetBase()->RefreshDuration();
                }
                else
                    GetTarget()->CastSpell(eventInfo.GetProcTarget(), SPELL_WARRIOR_TASTE_FOR_BLOOD_PROC, true);

                /// @hack: due to currenct proc system implementation
                // We need to put ICD to 5 seconds in order to make it proc at 6 seconds again
                if (Player* player = GetTarget()->ToPlayer())
                    player->GetSpellHistory()->AddCooldown(SPELL_WARRIOR_TASTE_FOR_BLOOD, 0, std::chrono::seconds(5));
            }

            void Register() override
            {
                DoCheckProc += AuraCheckProcFn(spell_warr_taste_for_blood_AuraScript::CheckProc);
                OnEffectProc += AuraEffectProcFn(spell_warr_taste_for_blood_AuraScript::HandleProc, EFFECT_0, SPELL_AURA_PROC_TRIGGER_SPELL);
            }
        };
