...
enum HunterSpells
{
    ...
    SPELL_HUNTER_ROAR_OF_SACRIFICE_TRIGGERED        = 67481,
...
class spell_hun_pet_scaling : public SpellScriptLoader
{
public:
    spell_hun_pet_scaling() : SpellScriptLoader("spell_hun_pet_scaling") { }

    class spell_hun_pet_scaling_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_hun_pet_scaling_AuraScript);
        ...
        void Register() override
        {
            ...
            {
                DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_hun_pet_scaling_AuraScript::CalculateStatAmount, EFFECT_ALL, SPELL_AURA_MOD_STAT);
                DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_hun_pet_scaling_AuraScript::CalculateAPAmount, EFFECT_ALL, SPELL_AURA_MOD_ATTACK_POWER);
                DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_hun_pet_scaling_AuraScript::CalculateSPAmount, EFFECT_ALL, SPELL_AURA_MOD_DAMAGE_DONE);
            }
            else if (m_scriptSpellId == SPELL_HUNTER_PET_SCALING_04)
            {
                DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_hun_pet_scaling_AuraScript::CalculateHitAmount, EFFECT_0, SPELL_AURA_MOD_HIT_CHANCE);
                DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_hun_pet_scaling_AuraScript::CalculateSpellHitAmount, EFFECT_1, SPELL_AURA_MOD_SPELL_HIT_CHANCE);
                DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_hun_pet_scaling_AuraScript::CalculateExpertiseAmount, EFFECT_2, SPELL_AURA_MOD_EXPERTISE);
            }
            else
            {
                DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_hun_pet_scaling_AuraScript::CalculateResistanceAmount, EFFECT_ALL, SPELL_AURA_MOD_RESISTANCE);
            }

            DoEffectCalcPeriodic += AuraEffectCalcPeriodicFn(spell_hun_pet_scaling_AuraScript::CalcPeriodic, EFFECT_ALL, SPELL_AURA_ANY);
            OnEffectPeriodic += AuraEffectPeriodicFn(spell_hun_pet_scaling_AuraScript::HandlePeriodic, EFFECT_ALL, SPELL_AURA_ANY);
        }
    };

    AuraScript* GetAuraScript() const override
    {
        return new spell_hun_pet_scaling_AuraScript();
    }
};
...
public:
    ...
    class spell_hun_roar_of_sacrifice_AuraScript : public AuraScript
    {
        ...
        bool Validate(SpellInfo const* /*spellInfo*/) override
        {
            if (!sSpellMgr->GetSpellInfo(SPELL_HUNTER_ROAR_OF_SACRIFICE_TRIGGERED))
                return false;
            return true;
        }
        ...
        void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
        {
            ...
            eventInfo.GetActor()->CastCustomSpell(SPELL_HUNTER_ROAR_OF_SACRIFICE_TRIGGERED, SPELLVALUE_BASE_POINT0, damage, GetCaster(), TRIGGERED_FULL_MASK, nullptr, aurEff);
        }
