...
class spell_warl_pet_scaling : public SpellScriptLoader
{
public:
	spell_warl_pet_scaling() : SpellScriptLoader("spell_warl_pet_scaling") { }

	class spell_warl_pet_scaling_AuraScript : public AuraScript
	{
		PrepareAuraScript(spell_warl_pet_scaling_AuraScript);

		bool Validate(SpellInfo const* /*spellInfo*/) override
		{
			if (!sSpellMgr->GetSpellInfo(SPELL_WARLOCK_PET_SCALING_01) || !sSpellMgr->GetSpellInfo(SPELL_WARLOCK_PET_SCALING_02) ||
				!sSpellMgr->GetSpellInfo(SPELL_WARLOCK_PET_SCALING_03) || !sSpellMgr->GetSpellInfo(SPELL_WARLOCK_PET_SCALING_04) ||
				!sSpellMgr->GetSpellInfo(SPELL_WARLOCK_PET_SCALING_05))
				return false;
			return true;
		}
		...
		void CalculateStatAmount(AuraEffect const* aurEff, int32& amount, bool& /*canBeRecalculated*/)
		{
			if (Unit* owner = GetUnitOwner()->GetOwner())
			{
				...
				int32 mod = stat == STAT_STAMINA ? 75 : 30;
		...
		void CalculateAPAmount(AuraEffect const* aurEff, int32& amount, bool& /*canBeRecalculated*/)
		{
			if (Unit* owner = GetUnitOwner()->GetOwner())
			{
				...
				int32 max = std::max(fireMod, shadowMod);
				amount = CalculatePct(std::max<int32>(0, max), 57);
			}
		}
		...
		void CalculateSPAmount(AuraEffect const* aurEff, int32& amount, bool& /*canBeRecalculated*/)
		{
			if (Unit* owner = GetUnitOwner()->GetOwner())
			{
				...
				int32 shadowMod = owner->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_SHADOW);
				int32 max = std::max(fireMod, shadowMod);
				amount = CalculatePct(std::max<int32>(0, max), 15);
		...
		void Register() override
		{
			if (m_scriptSpellId != SPELL_WARLOCK_PET_SCALING_01)
				DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_warl_pet_scaling_AuraScript::CalculateResistanceAmount, EFFECT_ALL, SPELL_AURA_MOD_RESISTANCE);

			if (m_scriptSpellId == SPELL_WARLOCK_PET_SCALING_01 || m_scriptSpellId == SPELL_WARLOCK_PET_SCALING_02)
			{
				DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_warl_pet_scaling_AuraScript::CalculateStatAmount, EFFECT_ALL, SPELL_AURA_MOD_STAT);
			}

			if (m_scriptSpellId == SPELL_WARLOCK_PET_SCALING_01)
			{
				DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_warl_pet_scaling_AuraScript::CalculateAPAmount, EFFECT_ALL, SPELL_AURA_MOD_ATTACK_POWER);
				DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_warl_pet_scaling_AuraScript::CalculateSPAmount, EFFECT_ALL, SPELL_AURA_MOD_DAMAGE_DONE);
			}

			if (m_scriptSpellId == SPELL_WARLOCK_PET_SCALING_05)
			{
				DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_warl_pet_scaling_AuraScript::CalculateHitAmount, EFFECT_0, SPELL_AURA_MOD_HIT_CHANCE);
				DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_warl_pet_scaling_AuraScript::CalculateSpellHitAmount, EFFECT_1, SPELL_AURA_MOD_SPELL_HIT_CHANCE);
				DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_warl_pet_scaling_AuraScript::CalculateExpertiseAmount, EFFECT_2, SPELL_AURA_MOD_EXPERTISE);
			}

			DoEffectCalcPeriodic += AuraEffectCalcPeriodicFn(spell_warl_pet_scaling_AuraScript::CalcPeriodic, EFFECT_ALL, SPELL_AURA_ANY);
			OnEffectPeriodic += AuraEffectPeriodicFn(spell_warl_pet_scaling_AuraScript::HandlePeriodic, EFFECT_ALL, SPELL_AURA_ANY);
		}
	};

	AuraScript* GetAuraScript() const override
	{
		return new spell_warl_pet_scaling_AuraScript();
	}
};
