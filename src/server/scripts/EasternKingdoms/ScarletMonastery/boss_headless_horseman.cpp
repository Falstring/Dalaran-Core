...
public:
    ...
    struct boss_headless_horsemanAI : public ScriptedAI
    {
        ...
        void UpdateAI(uint32 diff) override
        {
			if (me->HasAura(SPELL_CONFUSE))
			{
				if (me->GetPositionZ() > 19.0f)
				{	
					me->GetMotionMaster()->Clear();
					me->GetMotionMaster()->MovePoint(0, me->GetPositionX(), me->GetPositionY(), 18.887659f, true);
				}
			}
            if (withhead)
            {
...
class npc_hh_pumpkin_fiend : public CreatureScript
{

public:
	npc_hh_pumpkin_fiend() : CreatureScript("npc_hh_pumpkin_fiend") {}

	struct npc_hh_pumpkin_fiendAI : public ScriptedAI
	{
		npc_hh_pumpkin_fiendAI(Creature* creature) : ScriptedAI(creature) { }


		void Reset()
		{
			events.ScheduleEvent(EVENT_TRASH, 1000);
			events.ScheduleEvent(EVENT_SQUASH_SOUL, 3000);
		}
		...
		void UpdateAI(uint32 diff) override
		{
			...
			while (uint32 eventId = events.ExecuteEvent())
			{
				...
				case EVENT_TRASH:
					DoCast(me, 3391);
					events.ScheduleEvent(EVENT_TRASH, 5000);
					break;
				case EVENT_SQUASH_SOUL:
					if (me->GetVictim())
						DoCast(me->GetVictim(), 42514);
					events.ScheduleEvent(EVENT_SQUASH_SOUL, 8000);
					break;
				default:
					break;
				}
	...
	{
		return new npc_hh_pumpkin_fiendAI(creature);
	}
};
