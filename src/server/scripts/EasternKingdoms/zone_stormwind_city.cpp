...
class npc_varian_wrynn_sw : public CreatureScript
{
    public:
        ...
        {
            if (quest->GetQuestId() == QUEST_THE_KILLING_TIME)
        ...
        {
            npc_varian_wrynn_swAI(Creature* c) : ScriptedAI(c) {}
                    
            void Reset() override
            {
                heroicLeapTimer = 10 * IN_MILLISECONDS;
                whirlwindTimer = 12 * IN_MILLISECONDS;
            }
            ...
            void UpdateAI(uint32 diff) override
            {
                ...
                {
                    me->CastSpell(SelectTarget(SELECT_TARGET_RANDOM, 0, 25.0f, true), SPELL_HEROIC_LEAP);
    ...
    public:
        ...
        bool OnGossipSelect(Player* player, Creature* creature, uint32 /*uiSender*/, uint32 uiAction)
        {
            ...
            if (uiAction == GOSSIP_ACTION_INFO_DEF + 1)
            {
                ...
                creature->AI()->SetData(ACTION_ACTIVE, 0);
            }
                        ...
                        switch (step)
                        {
                            ...
                            case 1:
                                if (!me->FindNearestGameObject(GO_PORTAL_SW_TO_OG, 15.0f))
                                    me->CastSpell(me, SPELL_SW_PORTAL, false);
