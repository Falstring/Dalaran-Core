...
public:
    ...
    struct boss_sartharionAI : public ScriptedAI
    {
        ...
        uint8 drakeCount;
        std::list<ObjectGuid> _volcanoBlows;

        void DespawnCreatures(int creatureId)
        {
            std::list<Creature*> pCreatureList;

            Trinity::AllCreaturesOfEntryInRange checker(me, creatureId, 200.0f);
        ...
        void Reset() override
        {
            ...
            DespawnCreatures(30643); /* Lava Blazer */
            /*DespawnCreatures(30882); /* Twilight Whelp */
            //DespawnCreatures(31214); /* Sartharion Twilight Whelp */

            Creature* acolyteVesperon = ObjectAccessor::GetCreature((*me), instance->GetGuidData(DATA_ACOLYTE_VESPERON));

            if(acolyteVesperon)
                acolyteVesperon->DespawnOrUnsummon();

            Creature* acolyteShadron = ObjectAccessor::GetCreature((*me), instance->GetGuidData(DATA_ACOLYTE_SHADRON));

            if(acolyteShadron)
                acolyteShadron->DespawnOrUnsummon();
            ...
            if (instance)
            {
                Creature* pTenebron = ObjectAccessor::GetCreature(*me, instance->GetGuidData(DATA_TENEBRON));
                Creature* pShadron = ObjectAccessor::GetCreature(*me, instance->GetGuidData(DATA_SHADRON));
                Creature* pVesperon = ObjectAccessor::GetCreature(*me, instance->GetGuidData(DATA_VESPERON));
                if (pTenebron)
                {
        ...
        void SetData(uint32 type, ObjectGuid guid)
        {
            if (type != 0)
                return;

            if (!_volcanoBlows.empty())
                for (std::list<ObjectGuid>::const_iterator itr = _volcanoBlows.begin(); itr != _volcanoBlows.end(); ++itr)
                    if (guid == (*itr))
                        return;

            _volcanoBlows.push_back(guid);
        }

        bool GetData(ObjectGuid guid) const override
        {
        ...
        void FetchDragons()
        {
            ...
            Creature* pFetchTene = ObjectAccessor::GetCreature(*me, instance->GetGuidData(DATA_TENEBRON));
            Creature* pFetchShad = ObjectAccessor::GetCreature(*me, instance->GetGuidData(DATA_SHADRON));
            Creature* pFetchVesp = ObjectAccessor::GetCreature(*me, instance->GetGuidData(DATA_VESPERON));
        ...
        void CallDragon(uint32 uiDataId)
        {
            if (instance)
            {
                if (Creature* temp = ObjectAccessor::GetCreature(*me, instance->GetGuidData(uiDataId)))
                {
        ...
        void UpdateAI(uint32 uiDiff) override
        {
            ...
            if(m_pyroRange<=uiDiff)
            {
            if (Creature* trigger = me->FindNearestCreature(NPC_WORLD_TRIGGER, 100.0f, true))// 257661 256901
            {
                ...
                if (map->IsDungeon())
                {
                    ...
                    for (Map::PlayerList::const_iterator i = PlayerList.begin(); i != PlayerList.end(); ++i)
                    {
                        if (i->GetSource()->IsAlive() && i->GetSource()->GetTypeId()==TYPEID_PLAYER && !i->GetSource()->HasAura(SPELL_TWILIGHT_SHIFT) && !i->GetSource()->IsGameMaster())
                        {
                            if(!trigger->IsWithinDistInMap(i->GetSource(), 47.0f,true))
                            {

                                /* Check now secondary trigger, without GUID, just find nearest one.  */

                                if(Creature* secondaryTrigger = i->GetSource()->FindNearestCreature(21987, 100.0f, true))
                                {
                                    if(!i->GetSource()->IsWithinDistInMap(secondaryTrigger, 10.0f, true))
                                    {
                                        DoCast(i->GetSource(), 57557);
                                        m_pyroRange = 2000;
                                    }
                                }
                                else
            ...
            Unit* pTene = ObjectAccessor::GetCreature(*me, instance ? instance->GetGuidData(DATA_TENEBRON) : ObjectGuid::Empty);
            Unit* pShad = ObjectAccessor::GetCreature(*me, instance ? instance->GetGuidData(DATA_SHADRON) : ObjectGuid::Empty);
            Unit* pVesp = ObjectAccessor::GetCreature(*me, instance ? instance->GetGuidData(DATA_VESPERON) : ObjectGuid::Empty);
            ...
            EnterEvadeIfOutOfCombatArea(uiDiff); //Quizá hacer un dragon count y si es más de uno hacerles un evade
        }
    };
...
//        debug_log("dummy_dragonAI: %s reached point %u", me->GetName(), uiPointId);
    ...
    void OpenPortal()
    {
        ...
        //GameObject* pPortal = me->SummonGameObject(GO_TWILIGHT_PORTAL, me->GetPositionX()+12.0f, me->GetPositionY(), me->GetPositionZ(), 0.f, 0.f, 0.f, 0.f, 0.f,m_iPortalRespawnTime);
        ...
        switch (me->GetEntry())
        {
            case NPC_TENEBRON:
            {
                ...
                {
                    for (uint32 i = 0; i < 6; ++i) // NPC_SARTHARION_TWILIGHT_EGG
                        me->SummonCreature(NPC_TWILIGHT_EGG, TwilightEggsSarth[i].x, TwilightEggsSarth[i].y, TwilightEggsSarth[i].z, 0); //, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 20000
                }
                break;
            }
            case NPC_SHADRON:
            {
                iTextId = WHISPER_SHADRON_DICIPLE;
                if (instance && instance->GetData(TYPE_SARTHARION_EVENT) != IN_PROGRESS)
                    me->SummonCreature(NPC_ACOLYTE_OF_SHADRON, AcolyteofShadron.x, AcolyteofShadron.y, AcolyteofShadron.z, 0);
                else
                    me->SummonCreature(NPC_ACOLYTE_OF_SHADRON, AcolyteofShadron2.x, AcolyteofShadron2.y, AcolyteofShadron2.z, 0);

                break;
            }
            case NPC_VESPERON:
            {
                iTextId = WHISPER_VESPERON_DICIPLE;
                if (instance && instance->GetData(TYPE_SARTHARION_EVENT) != IN_PROGRESS)
                {
                    if (Creature* Acolyte = me->SummonCreature(NPC_ACOLYTE_OF_VESPERON, AcolyteofVesperon.x, AcolyteofVesperon.y, AcolyteofVesperon.z, 0))
                    {
                ...
                {
                    if (Creature* Acolyte = me->SummonCreature(NPC_ACOLYTE_OF_VESPERON, AcolyteofVesperon2.x, AcolyteofVesperon2.y, AcolyteofVesperon2.z, 0))
                    {
                        me->InterruptNonMeleeSpells(true);
                        Acolyte->InterruptNonMeleeSpells(true);
                        me->CastSpell(me, 32747, false);
        ...
        /*if(pPortal)
        {
            ...
            pPortal->SetPhaseMask(0x1, true); //TEST
        }	*/

        if (pPortal && !pPortal->isSpawned())
            pPortal->SetRespawnTime(30000);
    ...
    void JustDied(Unit* /*killer*/) override
    {
        ...
        int32 iTextId = 0;
        uint32 uiSpellId = 0;
        Creature* acolyteShadron = NULL;
        Creature* acolyteVesperon = NULL;
        ...
        switch (me->GetEntry())
        {
            ...
            case NPC_SHADRON:
                ...
                acolyteShadron = ObjectAccessor::GetCreature((*me), instance->GetGuidData(DATA_ACOLYTE_SHADRON));

                if(acolyteShadron)
                    acolyteShadron->Kill(acolyteShadron);
            ...
            case NPC_VESPERON:
                ...
                if (instance && instance->GetData(TYPE_SARTHARION_EVENT) != IN_PROGRESS)
                 ...
                 acolyteVesperon = ObjectAccessor::GetCreature((*me), instance->GetGuidData(DATA_ACOLYTE_VESPERON));

                if(acolyteVesperon)
                    acolyteVesperon->Kill(acolyteVesperon);
        ...
        if (instance)
        {
            ...
            /* Quitar debuffo en raid, si no tira hacer uno por uno a los jugadores */
            ...
            /*
                ...
                if (map->IsDungeon())
                {
                    ...
                    for (Map::PlayerList::const_iterator i = PlayerList.begin(); i != PlayerList.end(); ++i)
                    {
                        ...
                        {
                            i->GetSource()->RemoveAura(uiSpellId);
            ...
            if (Unit* pSartharion = ObjectAccessor::GetCreature(*me, instance->GetGuidData(DATA_SARTHARION)))
...
public:
    ...
    struct npc_tenebronAI : public dummy_dragonAI
    {
        ...
        uint32 m_uiSpellHatchEggTimer;
        uint32 m_uiHatchEggTimer;
        uint32 m_uiRemoveAurasTimer;
        ...
        void Reset() override
        {
            ...
            m_uiShadowBreathTimer = 18000;
            m_uiShadowFissureTimer = 5000;
            m_uiSpellHatchEggTimer = 20000;
            m_uiHatchEggTimer = 25000;
            //m_uiRemoveAurasTimer=0;

            m_bHasPortalOpen = false;

            if(me->HasAura(SPELL_POWER_OF_TENEBRON))
                me->RemoveAura(SPELL_POWER_OF_TENEBRON);
        }
        ...
        void UpdateAI(uint32 uiDiff) override
        {
            ...
            if (m_uiRemoveAurasTimer <= uiDiff)
            {
                if(m_bHasPortalOpen)
                {
                Map* map = me->GetMap();
                ...
                m_bHasPortalOpen = false;
                //m_uiRemoveAurasTimer = 20000;
            }
            }
            else
                m_uiRemoveAurasTimer -= uiDiff;
            ...
            if (m_uiHatchEggTimer <= uiDiff)
            {
                ...
                m_uiRemoveAurasTimer = 15000;
                m_uiHatchEggTimer = 60000;
            }
            else
                 m_uiHatchEggTimer -= uiDiff;


            if (m_uiSpellHatchEggTimer <= uiDiff)
            {
                Talk(WHISPER_HATCH_EGGS);
                DoCast(SPELL_HATCH_EGGS);
                m_uiSpellHatchEggTimer = 60000; /* IN_MILISECONDS */
            }
                else
                    m_uiSpellHatchEggTimer -= uiDiff;
...
public:
    ...
    struct npc_shadronAI : public dummy_dragonAI
    {
        ...
        void Reset() override
        {
            ...
            if(me->HasAura(SPELL_POWER_OF_SHADRON))
                me->RemoveAura(SPELL_POWER_OF_SHADRON);
        ...
        void UpdateAI(uint32 uiDiff) override
        {
            ...
            if (m_uiShadowBreathTimer <= uiDiff)
            {
                ...
                DoCastVictim(RAID_MODE(SPELL_SHADOW_BREATH, SPELL_SHADOW_BREATH_H));
                m_uiShadowBreathTimer = urand(17000, 25000);
            }
            else
                m_uiShadowBreathTimer -= uiDiff;
...
public:
    ...
    struct npc_vesperonAI : public dummy_dragonAI
    {
        ...
        void Reset() override
        {
            ...
            m_uiShadowFissureTimer = 5000;
            m_uiAcolyteVesperonTimer = 20000;

            /*GameObject* pPortal = me->FindNearestGameObject(GO_TWILIGHT_PORTAL, 50.0f);
            if(pPortal)
            {
                pPortal->SetPhaseMask(2, true); //Despawn portal
            }*/

            m_bHasPortalOpen = false;

            if(me->HasAura(SPELL_POWER_OF_VESPERON))
                me->RemoveAura(SPELL_POWER_OF_VESPERON);
    ...
    struct npc_acolyte_of_shadronAI : public ScriptedAI
    {
        ...
        void Reset() override
        {
            ...
            if (instance)
            {
                ...
                if (instance->GetData(TYPE_SARTHARION_EVENT) == IN_PROGRESS)
                {
                    target = ObjectAccessor::GetCreature((*me), instance->GetGuidData(DATA_SARTHARION));
                    if (target)
                        target->AddAura(SPELL_GIFT_OF_TWILIGTH_SAR, target);

                    Creature* pTenebron  = ObjectAccessor::GetCreature((*me), instance->GetGuidData(DATA_TENEBRON));
                    if(pTenebron && pTenebron->IsAlive())
                        me->AddAura(SPELL_POWER_OF_TENEBRON, me);

                    Creature* pShadron  = ObjectAccessor::GetCreature((*me), instance->GetGuidData(DATA_SHADRON));
                    if(pShadron && pShadron->IsAlive())
                        me->AddAura(SPELL_POWER_OF_SHADRON, me);

                    Creature* pVesperon  = ObjectAccessor::GetCreature((*me), instance->GetGuidData(DATA_VESPERON));
                    if(pVesperon && pVesperon->IsAlive())
                        me->AddAura(SPELL_POWER_OF_VESPERON, me);

                }
                else
                {
                    target = ObjectAccessor::GetCreature((*me), instance->GetGuidData(DATA_SHADRON));
                    if (target)
        ...
        /*bool IsVesperonAcolyteAlive()
        {
            Creature* vesperonAcolyte = me->FindNearestCreature(31219, 80.0f, true);

            if(vesperonAcolyte && vesperonAcolyte->IsAlive())
                return true;
            else
                return false;
        }*/
        ...
        void JustDied(Unit* /*killer*/) override
        {
            if (instance)
            {
                ...
                if(acolyteVesperon)
                {
                    if(!acolyteVesperon->IsAlive()) /* If Acolyte is not alive */
                    {
                    Map* map = me->GetMap();
                    if (map->IsDungeon())
                    {
...
public:
    ...
    struct npc_acolyte_of_vesperonAI : public ScriptedAI
    {
        ...
        /*bool IsShadronAcolyteAlive()
        {
            Creature* shadronAcolyte = me->FindNearestCreature(31218, 80.0f, true);


            if(shadronAcolyte && shadronAcolyte->IsAlive())
                return true;
            else
                return false;
        ...
        void JustDied(Unit* /*killer*/) override
        {
            ...
            /*
            if(pPortal){
                pPortal->SetRespawnTime(-1000);
                pPortal->SetPhaseMask(0x20, true);
            }*/
            ...
            if (instance)
            {
                ...
                if(acolyteShadron)
                {
                    if(!acolyteShadron->IsAlive()) /* If Shadron's acolyte is not alive */
                    {
                    Map* map = me->GetMap();
                    if (map->IsDungeon())
                    {
                        ...
                        for (Map::PlayerList::const_iterator i = PlayerList.begin(); i != PlayerList.end(); ++i)
                        {
                            if (i->GetSource()->IsAlive() && i->GetSource()->HasAura(SPELL_TWILIGHT_SHIFT) && !i->GetSource()->GetVictim())
                            {
                                i->GetSource()->CastSpell(i->GetSource(), SPELL_TWILIGHT_SHIFT_REMOVAL_ALL, true); // 61190, default 13 y lo tienes a 6, 28 default, lo tienes a 57
                                i->GetSource()->CastSpell(i->GetSource(), SPELL_TWILIGHT_RESIDUE, true);
                                i->GetSource()->RemoveAurasDueToSpell(SPELL_TWILIGHT_SHIFT);
                                i->GetSource()->RemoveAurasDueToSpell(SPELL_TWILIGHT_SHIFT_ENTER);
                            }
...
public:
    ...
    struct npc_twilight_eggsAI : public ScriptedAI
    {
        ...
        uint32 m_uiHatchEggTimerEggScript;
        ...
        void Reset() override
        {
            ...
            m_uiHatchEggTimerEggScript = 15000;
        }
        ...
        void UpdateAI(uint32 uiDiff) override
        {
            if (m_uiHatchEggTimerEggScript <= uiDiff)
            {
                SpawnWhelps();
                ...
                m_uiHatchEggTimerEggScript -= uiDiff;
        }
    ...
    class spell_sartharion_lava_strike_SpellScript : public SpellScript
    {
        ...
        void HandleSchoolDamage(SpellEffIndex /*effIndex*/)
        {
            ...
            if (InstanceScript* pInstance = GetCaster()->GetInstanceScript())
                ...
                {
                    sarth->AI()->SetData(0, GetHitUnit()->GetGUID());
