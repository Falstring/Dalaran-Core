    ...
    struct instance_obsidian_sanctum_InstanceMapScript : public InstanceScript
    {
        ...
        void OnCreatureCreate(Creature* creature) override
        {
            switch (creature->GetEntry())
            {
                ...
                case NPC_VESPERON:
                    ...
                    creature->setActive(true);
                    break;
                case NPC_ACOLYTE_VESPERON:
                    m_uiAcolyteVesperonGUID = creature->GetGUID();
                    break;
                case NPC_ACOLYTE_SHADRON:
                    m_uiAcolyteShadronGUID = creature->GetGUID();
                    break;
            }
        }
        ...
        ObjectGuid GetGuidData(uint32 uiData) const override
        {
            switch (uiData)
            {
                ...
                case DATA_ACOLYTE_VESPERON:
                    return m_uiAcolyteVesperonGUID;
                case DATA_ACOLYTE_SHADRON:
        ...
        bool CheckAchievementCriteriaMeet(uint32 criteria_id, Player const* source, Unit const* target = NULL, uint32 miscvalue1 = 0)
        {
            ...
            case 7327:
                if (Creature* cr = instance->GetCreature(m_uiSartharionGUID))
                    if (!cr->AI()->GetData(source->GetGUID()))
                        return true;
                break;
            }
            return false;
