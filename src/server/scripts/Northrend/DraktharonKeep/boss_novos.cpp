...
public:
    ...
    struct boss_novosAI : public BossAI
    {
        ...
        {
            if (_combatStarted && _despawnTimer <= diff)
            {
...
class npc_novos_add : public CreatureScript
{
public:
    npc_novos_add() : CreatureScript("npc_novos_add") { }

    struct npc_novos_addAI : public ScriptedAI
    {
        npc_novos_addAI(Creature* creature) : ScriptedAI(creature)
        {
    ...
    CreatureAI* GetAI(Creature* creature) const override
    {
        return new npc_novos_addAI(creature);
    }
};

/*  UPDATE creature_template SET ScriptName="npc_novos_add" WHERE entry IN(27598, 27597, 27600) */
