	...
	struct boss_ickAI : public ScriptedAI
	{
		...
		void UpdateAI(uint32 diff)
		{
				...
				if (Creature* k = GetKrick())
					if (!k->HasUnitState(UNIT_STATE_CASTING))
						if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0, 40.0f, true))
						{
							...
							events.RescheduleEvent(EVENT_SPELL_TOXIC_WASTE, urand(7000, 10000));
							break;
						}
				events.RescheduleEvent(EVENT_SPELL_TOXIC_WASTE, 2500);
				break;
			case EVENT_SPELL_MIGHTY_KICK:
				me->CastSpell(me->GetVictim(), SPELL_MIGHTY_KICK, false);
				events.RescheduleEvent(EVENT_SPELL_MIGHTY_KICK, urand(20000, 25000));
				break;
				...
				if (Creature* k = GetKrick())
					if (!k->HasUnitState(UNIT_STATE_CASTING))
						if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0, 35.0f, true))
						{
							...
							events.RescheduleEvent(EVENT_SPELL_SHADOW_BOLT, 14000);
							break;
						}
				events.RescheduleEvent(EVENT_SPELL_SHADOW_BOLT, 2500);
				break;
			...
			case EVENT_SPECIAL:
				...
				events.RescheduleEvent(4, urand(25000, 30000));
				break;
			}
...
class npc_pos_exploding_orb : public CreatureScript
{
public:
	npc_pos_exploding_orb() : CreatureScript("npc_pos_exploding_orb") { }

	struct npc_pos_exploding_orbAI : public ScriptedAI
	{
		npc_pos_exploding_orbAI(Creature* creature) : ScriptedAI(creature)
		{
		}
		void Reset() override
		{
	...
	{
		return new npc_pos_exploding_orbAI(creature);
	}
};
