	...
	struct npc_hor_lich_kingAI : public NullCreatureAI
	{
		...
		void SpellHitTarget(Unit* target, const SpellInfo* spell)
		{
			if (target && target->IsAlive() && spell->Id == SPELL_LICH_KING_ZAP_PLAYER)
				me->DealDamage(target, 10000,NULL, DIRECT_DAMAGE, SPELL_SCHOOL_MASK_NORMAL, NULL, false);
		}
		...
		void UpdateAI(uint32 diff)
		{
			...
			case EVENT_LK_START_FOLLOWING:
			{
				...
				//me->GetMotionMaster()->MovePoint(0, PathWaypoints[2]);
				events.ScheduleEvent(EVENT_ACTIVATE_LK, 12000);
			}
			break;
			case EVENT_ACTIVATE_LK:
				Talk(SAY_LK_IW_1);
			...
			case EVENT_LK_REMORSELESS_WINTER:
			{
				...
				me->GetMotionMaster()->MoveSplinePath(&path);
				//me->GetMotionMaster()->MovePoint(1, PathWaypoints[18], true);
			...
			case EVENT_LK_SUMMON_GHOULS:
				...
				for (uint8 i= 0; i <= 6; i++)
					me->CastSpell((Unit*)NULL, 69817, false);
				break;
	...
	struct npc_hor_leader_secondAI : public NullCreatureAI
	{
		...
		void DoAction(int32 actionId)
		{
			...
			case ACTION_INFORM_WALL_DESTROYED:
				...
				case 2:
				case 3:
					events.ScheduleEvent(EVENT_SAY_LEADER_STOP_TEXT, 18000);
					break;
				case 4:
					events.ScheduleEvent(EVENT_SAY_LEADER_STOP_TEXT, 27000);
					break;
				case 5:
					events.ScheduleEvent(EVENT_SAY_OPENING, 3000);
					events.ScheduleEvent(EVENT_FINISH, 19000);
					break;
				default:
					break;
				}
		...
		void MovementInform(uint32 type, uint32 id)
		{
			/*if (me->movespline->Finalized())
				events.ScheduleEvent(EVENT_SAY_LEADER_STOP_TEXT, 1000); // Hacer esto con evento y schedulearlo a gusto, desde start run, o desde ACTION_INFORM_WALL_DESTROYED*/
		}
		...
		void UpdateAI(uint32 diff)
		{
			...
			case EVENT_SAY_LEAVE:
			{
				...
				for (Map::PlayerList::const_iterator itr = pl.begin(); itr != pl.end(); ++itr)
					if (Player* p = itr->GetSource())
						p->KilledMonsterCredit(me->GetEntry(), ObjectGuid::Empty); // for quest
			...
			case EVENT_START_RUN:
			{
				...
				events.ScheduleEvent(EVENT_SAY_LEADER_STOP_TEXT, 23000); //  First wall
			}
			break;
			...
			case EVENT_FINISH:
				if (Creature* c = pInstance->instance->GetCreature(pInstance->GetGuidData(NPC_LICH_KING_BOSS)))
				{
					c->AI()->Talk(SAY_LK_NOWHERE_TO_RUN);
				}
