    ...
    public:
        ...
        bool OnGossipSelect(Player* player, GameObject* /*gameObject*/, uint32 sender, uint32 action) override
        {
            ...
            switch (action)
            {
                ...
                case CONSERVATORY:
                    ...
                    break;
				case SPARK:
					player->TeleportTo(603, 2518.12f, 2569.19f, 412.69f, 6.26f);
					player->CLOSE_GOSSIP_MENU();
					break;
				case MADNESS:
					player->TeleportTo(603, 1854.90f, -11.3163f, 334.57f, 4.74f);
					player->CLOSE_GOSSIP_MENU();
					break;
            }
        ...
        bool OnGossipHello(Player* player, GameObject* gameObject) override
        {
            ...
            if (InstanceScript* instance = gameObject->GetInstanceScript())
            {
				...
				if (instance->GetBossState(BOSS_XT002) == DONE) // Revisar ésto.
                {
				...
				if (instance->GetData(DATA_MIMIRON_ENGAGED) || instance->GetBossState(BOSS_MIMIRON) == DONE)
				...
				if (instance->GetBossState(BOSS_VEZAX) == DONE)
					player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Teleport to The Descend into Madness", GOSSIP_SENDER_MAIN, MADNESS);
            }
