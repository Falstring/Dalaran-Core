...
public:
	...
	struct boss_auriayaAI : public BossAI
	{
		...
		void Reset()
		{
			...
			//me->ApplySpellImmune(0, IMMUNITY_MECHANIC, MECHANIC_SNARE, true); // Infected wounds.
			me->SetSpeed(MOVE_WALK, 1.4f); // Take a loof if this is necessary or not.
			defenderLives = 9;
			crazyCatLady = true;
			nineLives = false;
			...
			for (uint8 i = 0; i < SENTRY_NUMBER; i++)
				if (Creature* sentry = me->SummonCreature(NPC_SANCTUM_SENTRY, *me, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 30 * IN_MILLISECONDS))
				{
				sentry->GetMotionMaster()->MoveFollow(me, (i < 2) ? 0.5f : 4.0f, M_PI - i - 1.5f); //(i < 2) ? 0.5f : 4.0f, M_PI - i - 1.5f
		...
		void EnterCombat(Unit* /*who*/)
		{
			_EnterCombat();
			Talk(SAY_AGGRO);
			me->SetSpeed(MOVE_WALK, 1.69f);
			summons.DoZoneInCombat();
			events.ScheduleEvent(EVENT_SONIC_SCREECH, 45000);
			events.ScheduleEvent(EVENT_TERRIFYING_SCREECH, 28000);
			events.ScheduleEvent(EVENT_ACTIVATE_DEFENDER, 55000); // Poner en rangos que no haga conflicto, y testear que no sea interruptable.
			events.ScheduleEvent(EVENT_SUMMON_SWARMING_GUARDIAN, 49000);
		...
		void UpdateAI(uint32 diff)
		{
			...
			while (uint32 eventId = events.ExecuteEvent())
			{
				...
				case EVENT_SONIC_SCREECH:
					...
					events.ScheduleEvent(EVENT_SONIC_SCREECH, urand(40 * IN_MILLISECONDS, 50 * IN_MILLISECONDS));
					return;
				case EVENT_TERRIFYING_SCREECH:
					Talk(EMOTE_FEAR);
					DoCast(SPELL_TERRIFYING_SCREECH);
					events.ScheduleEvent(EVENT_TERRIFYING_SCREECH, 36000);
				...
				case EVENT_ACTIVATE_DEFENDER:
					...
					if (Creature* trigger = me->FindNearestCreature(NPC_FERAL_DEFENDER_TRIGGER, 150.0f))
					{
				...
				case EVENT_SUMMON_SWARMING_GUARDIAN:
					...
					if (Is25ManRaid()) // If it is 25, cast this 2 times, 10 seconds = 20 panters.
					{
						if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0, 0.0f, true))
							DoCast(target, SPELL_SUMMON_SWARMING_GUARDIAN);
					}

					events.ScheduleEvent(EVENT_SUMMON_SWARMING_GUARDIAN, urand(28 * IN_MILLISECONDS, 40 * IN_MILLISECONDS));
					return;
				case EVENT_BERSERK:
...
public:
	...
	struct npc_sanctum_sentryAI : public ScriptedAI
	{
		...
		void Reset()
		{
			...
			events.ScheduleEvent(EVENT_RIP, urand(4 * IN_MILLISECONDS, 7 * IN_MILLISECONDS));
		...
		void UpdateAI(uint32 diff)
		{
			...
			while (uint32 eventId = events.ExecuteEvent())
			{
				...
				case EVENT_RIP:
					...
					events.ScheduleEvent(EVENT_RIP, urand(10 * IN_MILLISECONDS, 13 * IN_MILLISECONDS));
					break;
				case EVENT_POUNCE:
					if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0, 0.0f, true))
					{
					...
					events.ScheduleEvent(EVENT_POUNCE, urand(13 * IN_MILLISECONDS, 22 * IN_MILLISECONDS));
					break;
				default:
					break;
				}
...
public:
	...
	struct npc_feral_defenderAI : public ScriptedAI
	{
		...
		void Reset()
		{
			events.ScheduleEvent(EVENT_FERAL_POUNCE, 3 * IN_MILLISECONDS);
			events.ScheduleEvent(EVENT_RUSH, 9 * IN_MILLISECONDS);

            if (me->ToTempSummon())
            {                
                if (Unit* auriaya = me->ToTempSummon()->GetSummoner())
                {
                    if (auriaya->GetVictim())
                        AttackStart(auriaya->GetVictim());
                }
            }
		}
		...
		void UpdateAI(uint32 diff)
		{
			...
			while (uint32 eventId = events.ExecuteEvent())
			{
				...
				case EVENT_FERAL_POUNCE:
					...
					events.ScheduleEvent(EVENT_FERAL_POUNCE, urand(6 * IN_MILLISECONDS, 13 * IN_MILLISECONDS));
					break;
				case EVENT_RUSH:
					if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0, 0.0f, true))
					{
					...
					events.ScheduleEvent(EVENT_RUSH, urand(8 * IN_MILLISECONDS, 15 * IN_MILLISECONDS));
					break;
				default:
					break;
				}
