...
enum Spells
{
	...
	SPELL_SUMMON_VOID_ZONE_VISUAL = 64470, //Este parece más grande pero mejor, revisarlo con retail.
...
enum Events
{
	...
	EVENT_ACTIVATE_LIVING_CONSTELATION = 40,
	EVENT_CHASE_UPDATE = 41,
	EVENT_SECOND_SOUND = 42,
...
{
	/*{1631.666f, -163.35f, 427.25f, 0.0f},
	{ 1631.9899f, -173.8120f, 427.26f, 0.0f },
	{ 1632.2329f, -190.51f, 417.37f, 0.0f },
	{ 1632.00f, -214.120f, 418.47f, 0.0f },
	{ 1632.086f, -227.385f, 418.23f, 0.0f },*/
	{ 1631.666f, -163.35f, 427.25f, 0.0f },
	{ 1631.9899f, -173.8120f, 427.26f, 0.0f },
	{ 1631.9899f, -173.8120f, 427.26f, 0.0f },
...
#define LIVING_CONSTELLATION_COUNT 15
Position const ConstellationPos[LIVING_CONSTELLATION_COUNT] =
{
	{ 1596.208f, -263.2771f, 441.4296f, 5.044002f },
	{ 1675.208f, -329.2771f, 456.4296f, 5.044002f },
	{ 1619.208f, -356.2771f, 441.4296f, 5.044002f },
	{ 1639.208f, -287.2771f, 446.4296f, 5.044002f },
...
public:
	...
	bool Execute(uint64 execTime, uint32 /*diff*/)
	{
		...
		_owner->CastSpell((Unit*)NULL, SPELL_TRIGGER_3_ADDS, TRIGGERED_FULL_MASK); /* Check how to manage this trigger AND the timmers. */
...
public:
	...
	struct boss_algalon_the_observerAI : public BossAI
	{
			...
			void EnterCombat(Unit* /*target*/) override
		{
			...
			The following a super hack, however it does fix every single reset. This is like gold and a very hard to notify thing.
			Algalon has a neutral faction by default, which is 190 on DB, for Alliance and for the Horde.

			His AI, fails a lot of times during the encounter, when trying to manage the aggro tables, such after a Big bang, tank death, Tank phase punch stacks 5,
			or even when his main victim is phased. Algalon tries to attack the next unit on his aggro table, but since via core, this encounter is "ilegal" between factions,
			he enters in conflict with himself and can not attack this player so - resets.

			What I am doing is switching his faction to a normal boss enemy faction, so that he reacts in a normal way and his AI or aggro table trully understand his behaviour.
			After a reset, he will get his database's faction (190), neutral until he is pulled again.
			...
			*/
			...
			me->PlayDirectSound(15878); //  UR_CelestialPlanetariumIntro

			events.ScheduleEvent(EVENT_INTRO_TIMER_DONE, introDelay);
			events.ScheduleEvent(EVENT_ACTIVATE_LIVING_CONSTELATION, 42000 + introDelay);
			events.ScheduleEvent(EVENT_QUANTUM_STRIKE, 2200 + introDelay);
			events.ScheduleEvent(EVENT_PHASE_PUNCH, 15000 + introDelay);
			...
			events.ScheduleEvent(EVENT_COSMIC_SMASH, 22000 + introDelay);
			events.ScheduleEvent(EVENT_SECOND_SOUND, 140 * IN_MILLISECONDS);
			...
			void JustSummoned(Creature* summon) override
		{
			...
			case NPC_COLLAPSING_STAR:
				summon->SetReactState(REACT_PASSIVE); //Setincombatwithzone?
				summon->GetMotionMaster()->MoveRandom(14.0f);
				//summon->CastSpell(summon, SPELL_COLLAPSE, TRIGGERED_FULL_MASK);
				break;
			case NPC_BLACK_HOLE:
				//summon->NearTeleportTo(summon->GetPositionX(), summon->GetPositionY(), summon->GetPositionZ() - 1, summon->GetOrientation());
			...
			void UpdateAI(uint32 diff) override
		{
			...
			while (uint32 eventId = events.ExecuteEvent())
			{
				switch (eventId)
				{
				case EVENT_SECOND_SOUND:
					me->PlayDirectSound(15877); //  UR_CelestialPlanetariumBattle
					events.ScheduleEvent(EVENT_SECOND_SOUND, 120 * IN_MILLISECONDS);
					break;
				case EVENT_INTRO_1:
				...
				case EVENT_INTRO_TIMER_DONE:
				{
					...
					for (uint32 i = 0; i < LIVING_CONSTELLATION_COUNT; ++i)
					{
						...
						{
							summon->SetReactState(REACT_PASSIVE);
							constellationsArray[i] = summon->GetGUID();
						}
					}

					me->SetInCombatWithZone();
					constellationsIndex = 0;

					break;
				}
				case EVENT_ACTIVATE_LIVING_CONSTELATION:
				{

					uint8 tempConstellationsIndex = constellationsIndex;

					if ((constellationsIndex + 3) <= LIVING_CONSTELLATION_COUNT)
					{
						for (uint8 i = tempConstellationsIndex; i < (tempConstellationsIndex + 3); i++)
						{
							if (Creature* livingConstellation = ObjectAccessor::GetCreature(*me, constellationsArray[i])) // Con el ObjectAccessor::GetCreature??
							{
								livingConstellation->AI()->DoAction(ACTION_ACTIVATE_STAR);
								constellationsIndex++;
							}
						}
					}
					...
					{
						for (uint8 i = tempConstellationsIndex; i < LIVING_CONSTELLATION_COUNT; i++)
						{
							if (Creature* livingConstellation = ObjectAccessor::GetCreature(*me, constellationsArray[i])) // Con el ObjectAccessor::GetCreature??
							{
								livingConstellation->AI()->DoAction(ACTION_ACTIVATE_STAR);
					...
					events.ScheduleEvent(EVENT_ACTIVATE_LIVING_CONSTELATION, 50000);
				}
					break;
				case EVENT_QUANTUM_STRIKE:
					DoCastVictim(SPELL_QUANTUM_STRIKE);
					events.ScheduleEvent(EVENT_QUANTUM_STRIKE, urand(1500, 3000));
					break;
				case EVENT_PHASE_PUNCH:
					DoCastVictim(SPELL_PHASE_PUNCH);
					events.ScheduleEvent(EVENT_PHASE_PUNCH, 14000);
					break;
				...
				case EVENT_COSMIC_SMASH:
					...
					DoCastAOE(RAID_MODE(62301, 64598));
					events.ScheduleEvent(EVENT_COSMIC_SMASH, urand(27000, 32000));
					break;
	...
	private:
		std::list<Creature*> livingConstellations;

		ObjectGuid constellationsArray[LIVING_CONSTELLATION_COUNT];
		uint8 constellationsIndex;
		bool _firstPull;
		bool _fedOnTears;
		bool _phaseTwo;
		bool _fightWon;
	...
	struct npc_living_constellationAI : public CreatureAI
	{
			...
			void DoAction(int32 action) override
		{
			...
			case ACTION_ACTIVATE_STAR:
				if (Creature* algalon = ObjectAccessor::GetCreature(*me, instance->GetGuidData(BOSS_ALGALON)))
				{
					me->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
					_isActive = true;
					if (Unit* targetUnit = algalon->AI()->SelectTarget(SELECT_TARGET_RANDOM, 0, NonTankTargetSelector(algalon))) //NonTankTargetSelector(algalon) SelectTarget(SELECT_TARGET_RANDOM, 0, 150.0f, true)
					{
						if (targetUnit->ToPlayer())
							target = targetUnit->ToPlayer();

						me->SetReactState(REACT_AGGRESSIVE);
						DoZoneInCombat();

						_events.ScheduleEvent(EVENT_CHASE_UPDATE, 1 * IN_MILLISECONDS);
						_events.ScheduleEvent(EVENT_ARCANE_BARRAGE, 6000);
					}
			...
			void UpdateAI(uint32 diff) override
		{
			...
			while (uint32 eventId = _events.ExecuteEvent())
			{
				...
				case EVENT_ARCANE_BARRAGE:
					...
					if (Unit* targetBarrage = SelectTarget(SELECT_TARGET_RANDOM, 0, 150.0f, true))
						DoCast(targetBarrage, SPELL_ARCANE_BARRAGE);

					_events.ScheduleEvent(EVENT_ARCANE_BARRAGE, 7000);
					break;
				case EVENT_RESUME_UPDATING:
					_events.SetPhase(0);
					break;
				case EVENT_CHASE_UPDATE:
...
{
public:
	spell_algalon_trigger_3_adds() : SpellScriptLoader("spell_algalon_trigger_3_adds") { } /* Living Constellations triggers. */
