...
enum Yells
{
	...
	// King Llane
	SAY_STORMWIND_ROLEPLAY_5 = 0, rain_li
};
...
enum Spells
{
	...
	SPELL_DOMINATE_MIND = 63042, // Original: 63042 71289
...
enum Events
{
	...
	EVENT_GUARDIAN_DOMINATE_MIND = 48,
	EVENT_DEATH_RAY_MOVE_RANDOM = 49,
	EVENT_GUARDIAN_DOMINATE_MIND_HELPER = 50,

	EVENT_IMMORTAL_REACT_AGGRESSIVE = 51
};
	...
	bool operator()(WorldObject* object)
	{
		if (Unit* unit = object->ToUnit())
			if (Aura* sanity = unit->GetAura(SPELL_SANITY))
				return sanity->GetStackAmount() <= 30;
		return true;
	}
};
...
public:
	...
	struct boss_voice_of_yogg_saronAI : public BossAI
	{
...

		{
			...
			if (who->GetTypeId() == TYPEID_PLAYER && me->GetDistance2d(who) < 60.0f && !me->IsInCombat())
			...
			void EnterEvadeMode() override
		{
			...
			void ModifySanity(Player* target, int8 amount)  /* It does work with everything, but still no with Brain link. */
		{
			if (target)
				if (target->IsAlive())
				{
				int32 newamount;
				if (Aura* aur = target->GetAura(SPELL_SANITY, me->GetGUID())) /* Hacer un if(hasAura(SPELL_SANITY) para evitar crashes */
				{
			...
			void Reset() override
		{
			...
			if (Creature* losHelper = me->FindNearestCreature(225157, 130.0f))
				losHelper->AI()->Reset();
		}
			...
			void EnterCombat(Unit* /*who*/) override
		{
			...
			events.ScheduleEvent(EVENT_SUMMON_GUARDIAN_OF_YOGG_SARON, 15000, 0, PHASE_ONE);
			...
			void UpdateAI(uint32 diff) override
		{
			...
			while (uint32 eventId = events.ExecuteEvent())
			{
				...
				case EVENT_SUMMON_GUARDIAN_OF_YOGG_SARON:
					me->CastCustomSpell(SPELL_SUMMON_GUARDIAN_2, SPELLVALUE_MAX_TARGETS, 1);
					++_guardiansCount;
					if (_guardianTimer >= 14000)//if (_guardiansCount <= 8 && _guardiansCount % 2 == 0)
						_guardianTimer -= 4000;
				...
				case EVENT_SUMMON_CORRUPTOR_TENTACLE:
					...
					events.ScheduleEvent(EVENT_SUMMON_CORRUPTOR_TENTACLE, RAID_MODE(28000, 19000), EVENT_GROUP_SUMMON_TENTACLES, PHASE_TWO);
					break;
				case EVENT_SUMMON_CONSTRICTOR_TENTACLE:
					me->CastCustomSpell(SPELL_CONSTRICTOR_TENTACLE, SPELLVALUE_MAX_TARGETS, 1);
					events.ScheduleEvent(EVENT_SUMMON_CONSTRICTOR_TENTACLE, RAID_MODE(24000, 18000), EVENT_GROUP_SUMMON_TENTACLES, PHASE_TWO);
					break;
	...
	struct boss_saraAI : public ScriptedAI
	{
		...
		void DamageTaken(Unit* /*attacker*/, uint32& damage) override
		{
			if (_events.IsInPhase(PHASE_ONE) && damage >= me->GetHealth())
			{
				...
				me->SetFullHealth(); // Testing, security.
			...
			void EnterCombat(Unit* /*who*/) override
		{
			...
			_events.ScheduleEvent(EVENT_SARAS_BLESSING, urand(10000, 25000), 0, PHASE_ONE);
			_events.ScheduleEvent(EVENT_SARAS_ANGER, urand(13000, 20000), 0, PHASE_ONE);
		}
			...
			void UpdateAI(uint32 diff) override
		{
			...
			while (uint32 eventId = _events.ExecuteEvent())
			{
				...
				case EVENT_SARAS_BLESSING:
					...
					if (Creature *guardian = me->FindNearestCreature(NPC_GUARDIAN_OF_YOGG_SARON, 100.0f, true))
					{
						DoCast(guardian, SPELL_SARAS_BLESSING);
					}
					_events.ScheduleEvent(EVENT_SARAS_BLESSING, urand(6000, 20000), 0, PHASE_ONE);
					break;
				...
				case EVENT_TRANSFORM_4:
					...
					me->SetInCombatWithZone(); // Fixes Psychosis and Malady of the mind target selection - now it does cast it.
					_events.ScheduleEvent(EVENT_DEATH_RAY, 20000, 0, PHASE_TWO);
					_events.ScheduleEvent(EVENT_MALADY_OF_THE_MIND, 10000, 0, PHASE_TWO);
					_events.ScheduleEvent(EVENT_PSYCHOSIS, 1000, 0, PHASE_TWO);
				...
				case EVENT_MALADY_OF_THE_MIND:
					...
					_events.RescheduleEvent(EVENT_MALADY_OF_THE_MIND, urand(13*IN_MILLISECONDS,20*IN_MILLISECONDS), 0, PHASE_TWO);// urand(13000, 20000) PERO PONER A 20000 fijos.
					break;
				case EVENT_PSYCHOSIS:
					...
					_events.RescheduleEvent(EVENT_PSYCHOSIS, Is25ManRaid() ? 2 * IN_MILLISECONDS : 3 * IN_MILLISECONDS, 0, PHASE_TWO);
					break;
				case EVENT_BRAIN_LINK:
					DoCast(SPELL_BRAIN_LINK); /*Has puesto en el SpellMgr.cpp: MaxAffectedTargets, mirar en el dbc spell: 63802 y 63803 */
					_events.ScheduleEvent(EVENT_BRAIN_LINK, 32 * IN_MILLISECONDS, 0, PHASE_TWO);
					break;
				default:
					break;
				}
			...
			void JustSummoned(Creature* summon) override
		{
			...
			case NPC_DEATH_RAY:
				...
				summon->AI()->DoAction(DO_START_MOVEMENT);
				break;
			}
...
public:
	...
	struct boss_yogg_saronAI : public PassiveAI
	{
		...
		void Reset() override
		{
			...
			//std::cout << "\n \n EMPEZANDO YOGG-SARON CON (m_keeper):" << m_keepers << " Keepers  y mi Loot mode es: " << uint16(me->GetLootMode()) << "\n \n ";
		}

		uint32 GetKeeperNumber()
		{
			uint32 count = 0;

			if (Creature* freya_ys = me->FindNearestCreature(NPC_FREYA_YS, 150.0f))
				count++;					
			if (Creature* mimiron_ys = me->FindNearestCreature(NPC_MIMIRON_YS, 150.0f))
				count++;
			if (Creature* hodir_ys = me->FindNearestCreature(NPC_HODIR_YS, 150.0f))
				count++;
			if (Creature* thorim_ys = me->FindNearestCreature(NPC_THORIM_YS, 150.0f))
				count++;		
			...
			void JustDied(Unit* /*killer*/) override
		{
			...
			//std::cout << "\n DROPEANDO LOOT: "<< uint16(me->GetLootMode())<<"\n";
			...
			void UpdateAI(uint32 diff) override
		{
			...
			while (uint32 eventId = _events.ExecuteEvent())
			{
				...
				case EVENT_SHADOW_BEACON:
					...
					if (!me->GetMap()->Is25ManRaid())
						me->CastCustomSpell(SPELL_SHADOW_BEACON, SPELLVALUE_MAX_TARGETS, 2);
					else
						DoCastAOE(SPELL_SHADOW_BEACON);

					Talk(EMOTE_YOGG_SARON_EMPOWERING_SHADOWS);
					_events.ScheduleEvent(EVENT_SHADOW_BEACON, 40000, 0, PHASE_THREE);
					break;
				...
				case EVENT_DEAFENING_ROAR:
					...
					_events.ScheduleEvent(EVENT_DEAFENING_ROAR, urand(17000, 24000), 0, PHASE_THREE);
					break;
				default:
					break;
				}
			...
			void DoAction(int32 action) override
		{
			...
			case ACTION_PHASE_THREE:
				...
				// m_keepers is stored right after Yogg-Saron's reset, it check if the keepers are near.
				...
				//std::cout << "\n  El valor de la variable M_KEEPER es: " << uint32(m_keepers) << " y tengo loot mode: "<< me->GetLootMode() <<"\n";

				if (me->GetMap()->Is25ManRaid() && m_keepers < 4)				
					_events.ScheduleEvent(EVENT_DEAFENING_ROAR, urand(14000, 19000), 0, PHASE_THREE);			
...
public:
	...
	struct boss_brain_of_yogg_saronAI : public PassiveAI
	{
			...
			void DoAction(int32 action) override
		{
			...
			void JustSummoned(Creature* summon) override
		{
			_summons.Summon(summon); // Maybe here, skull counter.
		}
...
public:
	...
	struct npc_guardian_of_yogg_saronAI : public ScriptedAI
	{
			...
			void Reset() override
		{
			...
			_events.ScheduleEvent(EVENT_DARK_VOLLEY, urand(7000, 13000));
			_events.ScheduleEvent(EVENT_GUARDIAN_DOMINATE_MIND, urand(15000, 25000));
		}
			...
			void UpdateAI(uint32 diff) override
		{
			...
			while (uint32 eventId = _events.ExecuteEvent())
			{
				...
				case EVENT_DARK_VOLLEY:
					...
					//me->SummonCreature(219871, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN, 17000);
					_events.ScheduleEvent(EVENT_DARK_VOLLEY, urand(10000, 12000));
					break;
				case EVENT_GUARDIAN_DOMINATE_MIND:

					//if (Unit* player = SelectTarget(SELECT_TARGET_RANDOM, 0, NonTankTargetSelector(me))) //NotCharmedTargetSelector()

					//if (Unit* player = SelectTarget(SELECT_TARGET_RANDOM, 0, NonTankTargetSelector(me)))
					if (Unit* player = SelectTarget(SELECT_TARGET_RANDOM, 0, 100.0f, true, -SPELL_DOMINATE_MIND))
					{
						if (player->ToPlayer())
						{
							me->StopMoving();
							me->CastCustomSpell(SPELL_DOMINATE_MIND, SPELLVALUE_MAX_TARGETS, 1, player); // 71289

							if (player->HasAura(SPELL_SANITY))
							{
...
enum PassengerSpells
{
	SPELL_ICEBLOCK = 45438,
	SPELL_DIVINE_SHIELD = 642,
	SPELL_HANDOF_PROTECTION = 10278,
};
...
public:
	...
	struct npc_constrictor_tentacleAI : public ScriptedAI
	{
			...
			void PassengerBoarded(Unit* passenger, int8 /*seatId*/, bool apply) override
		{
			if (!apply)
			{
				passenger->RemoveAurasDueToSpell(64126);
			...
			if (hasPassenger)
			{
				if (Vehicle* v = me->GetVehicleKit())								
					if (Unit* passenger = v->GetPassenger(0))					
						if (passenger->HasAura(SPELL_ICEBLOCK) || passenger->HasAura(SPELL_HANDOF_PROTECTION) || passenger->HasAura(SPELL_DIVINE_SHIELD))
...
public:
	...
	struct npc_immortal_guardianAI : public ScriptedAI
	{
		...
		void Reset() override
		{
			...
			DoCast(me, 42716);
			me->StopMoving();
			_events.ScheduleEvent(EVENT_IMMORTAL_REACT_AGGRESSIVE, 2000);
			_events.ScheduleEvent(EVENT_DRAIN_LIFE, urand(15000, 19000));
			tickTimer = 1000;
		}
			...
			void UpdateAI(uint32 diff) override
		{
			...
			if (me->HasUnitState(UNIT_STATE_CASTING))
			{
				if (me->FindCurrentSpellBySpellId(SPELL_DRAIN_LIFE))
				{
					if (tickTimer <= diff)
					{
						me->ModifyHealth(10000);
						tickTimer = 1000;
					}
					else
						tickTimer -= diff;
				}
				else if (me->FindCurrentSpellBySpellId(64160))
				{
					if (tickTimer <= diff)
					{
						me->ModifyHealth(33000);
						tickTimer = 1000;
					}
					else
						tickTimer -= diff;
			...
			while (uint32 eventId = _events.ExecuteEvent())
			{
				switch (eventId)
				{
				case EVENT_IMMORTAL_REACT_AGGRESSIVE:
				...
				case EVENT_DRAIN_LIFE:
					...
					if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0, 0.0f, true))
						me->CastCustomSpell(SPELL_DRAIN_LIFE, SPELLVALUE_MAX_TARGETS, 1, target, false);

					_events.ScheduleEvent(EVENT_DRAIN_LIFE, urand(18000, 28000));
					break;
				default:
					break;
				}
...
public:
	...
	class spell_yogg_saron_psychosis_SpellScript : public SanityReduction
	{
		...
		bool Load() override
		{
			_stacks = GetSpellInfo()->Id == SPELL_PSYCHOSIS ? 9 : 12; /* 10 OR 25 MODES CHECK THIS */
			return true;
		}
...
public:
	...
	class spell_brain_link_periodic_dummy_SpellScript : public SpellScript
	{
		...
		{
			targets.remove_if(BrainLinkTargetSelector()); /* REVISAR TAMBIÉN EL SPELL ILLUSION */
							...
							if (player->HasAura(SPELL_BRAIN_LINK) && player->GetGUID() != trigger->GetGUID())
							{
								if (Unit* caster = GetCaster())
								{
									if (!player->IsWithinDist(trigger, float(aurEff->GetAmount())))
									{
										...
										if (caster->ToCreature() && caster->GetGUID() == pInstance->GetGuidData(DATA_VOICE_OF_YOGG_SARON))
										CAST_AI(boss_voice_of_yogg_saron::boss_voice_of_yogg_saronAI, caster->ToCreature()->AI())->ModifySanity(player, -2);*/
									}
									else
										trigger->CastSpell(player, SPELL_BRAIN_LINK_NO_DAMAGE, true);
								}
							}
						}
					}
...
public:
	...
	class spell_yogg_saron_boil_ominously_SpellScript : public SpellScript
	{
		...
		{
			if (Unit* target = GetHitUnit())
				if (!target->HasAura(SPELL_DOMINATE_MIND) && !target->HasAura(SPELL_FLASH_FREEZE) && !GetCaster()->HasAura(SPELL_SUMMON_GUARDIAN_1) && !GetCaster()->HasAura(SPELL_SUMMON_GUARDIAN_2))
				{
...
public:
	...
	bool OnUse(Player* pPlayer, Item* pItem, SpellCastTargets const& /*targets*/)
	{
		...
		{
			if (yogg->FindCurrentSpellBySpellId(SPELL_DEAFENING_ROAR)) /* Agregar 45897 a player ? */
				return false;
		}
...
public:
	...
	struct npc_death_rayAI : public ScriptedAI
	{
		...
		{
			//_events.ScheduleEvent(EVENT_DEATH_RAY_MOVE_RANDOM, 1500);
		}
		...
		void DoAction(int32 action) override
		{
			switch (action)
			{
			case DO_START_MOVEMENT:
				_events.ScheduleEvent(EVENT_DEATH_RAY_MOVE_RANDOM, 6000);
				me->GetMotionMaster()->MoveIdle();
				break;
			}
		}
			...
			void UpdateAI(uint32 diff) override
		{
			...
			while (uint32 eventId = _events.ExecuteEvent())
			{
				...
				case EVENT_DEATH_RAY_MOVE_RANDOM:
				{
					bool rand = urand(0, 1);
					...
					{
					case 0:
						me->GetMotionMaster()->MovePoint(0, me->GetPositionX() - urand(2, 6), me->GetPositionY() - urand(2, 7), me->GetPositionZ());
						break;
					case 1:
						me->GetMotionMaster()->MovePoint(0, me->GetPositionX() + urand(2, 6), me->GetPositionY() + urand(2, 7), me->GetPositionZ());
						break;
					...
					_events.ScheduleEvent(EVENT_DEATH_RAY_MOVE_RANDOM, 2500);
				}
					break;
				default:
					break;
	...
	CreatureAI* GetAI(Creature* creature) const override
	{
		return GetUlduarAI<npc_death_rayAI>(creature);
	}
};


class npc_yogg_saron_los_helper : public CreatureScript
{
public:
	npc_yogg_saron_los_helper() : CreatureScript("npc_yogg_saron_los_helper") { }

	struct npc_yogg_saron_los_helperAI : public ScriptedAI
	{
		npc_yogg_saron_los_helperAI(Creature* creature) : ScriptedAI(creature), _instance(creature->GetInstanceScript())
		{
		...
		{
			if (!activated && who->GetTypeId() == TYPEID_PLAYER && me->GetDistance2d(who) < 20.0f)
			if (Creature* voice = ObjectAccessor::GetCreature(*me, _instance->GetGuidData(DATA_VOICE_OF_YOGG_SARON)))
			{
				voice->SetInCombatWithZone();
				activated = true;
			}
	...
	CreatureAI* GetAI(Creature* creature) const override
	{
		return GetUlduarAI<npc_yogg_saron_los_helperAI>(creature);
	}
};
