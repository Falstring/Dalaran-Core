...
// Achievements
#define ACHIEVEMENT_RUBBLE_AND_ROLL                     RAID_MODE(ACHIEVEMENT_RUBBLE_AND_ROLL_10, ACHIEVEMENT_RUBBLE_AND_ROLL_25)
#define ACHIEVEMENT_WITH_OPEN_ARMS						RAID_MODE(ACHIEVEMENT_WITH_OPEN_ARMS_10, ACHIEVEMENT_WITH_OPEN_ARMS_25)
...
enum Events
{
	...
	EVENT_START_CHECKING_STONEGRIP,
	EVENT_CANCEL_CHECKING_STONEGRIP
};
...
public:
	...
	struct boss_kologarnAI : public BossAI
	{
		...
		void EnterCombat(Unit* /*who*/)
		{
			...
			events.ScheduleEvent(EVENT_SWEEP, 17 * IN_MILLISECONDS);
			...
			if (rubbleCount >= 25)
			{
				...
				if (map && map->IsDungeon())
				{
					...
					for (Map::PlayerList::const_iterator itr = players.begin(); itr != players.end(); ++itr)
					{
						...
						{
							if (AchievementEntry const* rubbleAndRoll = sAchievementMgr->GetAchievement(ACHIEVEMENT_RUBBLE_AND_ROLL))
								player->CompletedAchievement(rubbleAndRoll);
						}
					}
				}
			}
		...
		void PassengerBoarded(Unit* who, int8 /*seatId*/, bool apply)
		{
			...
			if (who->GetEntry() == NPC_LEFT_ARM)
			{
				...
				if (!apply && isEncounterInProgress)
				{
					...
					events.ScheduleEvent(EVENT_RESPAWN_LEFT_ARM, 42 * IN_MILLISECONDS);
				}
			}
			else if (who->GetEntry() == NPC_RIGHT_ARM)
			{
				...
				if (!apply && isEncounterInProgress)
				{
					/*Unit* player = who->GetVehicleKit()->GetPassenger(0);
					player->GetMotionMaster()->MoveJump(1753.1907f, -23.8603f, 452.8268f, 448.8f, 30.0f, 10.0f);*/

					who->ToCreature()->DisappearAndDie();

					//Talk(SAY_RIGHT_ARM_GONE);
					events.ScheduleEvent(EVENT_RESPAWN_RIGHT_ARM, 42 * IN_MILLISECONDS);
				}
			}
			...
			if (!apply)
			{
				...
				if (me->GetHealthPct() <= 17)
					me->DealDamage(me, me->GetHealth(), NULL, NODAMAGE, SPELL_SCHOOL_MASK_NORMAL, NULL, false);
				else
				{
					uint32 _hPct = (me->GetMaxHealth() * 15) / 100;
					me->LowerPlayerDamageReq(_hPct);   // Reduce requirements for players to get the loot - since the spell does a lot of damage
					me->ModifyHealth(-int(_hPct));
				}

				switch (who->GetEntry())
				{
				case NPC_RIGHT_ARM:
					if (Creature* rubbleStalker = who->FindNearestCreature(198765, 100.0f)) // Hack, for now.
					{
					...
					break;
				case NPC_LEFT_ARM:
					if (Creature* rubbleStalker = who->FindNearestCreature(NPC_RUBBLE_STALKER, 100.0f))
					{
						rubbleStalker->CastSpell(rubbleStalker, SPELL_FALLING_RUBBLE, true);
						rubbleStalker->CastSpell(rubbleStalker, SPELL_SUMMON_RUBBLE, true);
					}
		...
		void JustSummoned(Creature* summon)
		{
			...
			case NPC_FOCUSED_EYEBEAM_RIGHT:
				/*if (Unit* otherBeam = summon->FindNearestCreature(33632, 15.0f))
				summon->GetMotionMaster()->MoveChase(otherBeam);*/
				summons.Summon(summon);
			...
			summon->CastSpell(summon, SPELL_FOCUSED_EYEBEAM_PERIODIC, true); // Daño, spell con daño periódico y efecto visual de ruptura sísmica.
			...
			void UpdateAI(uint32 diff)
		{
			...
			while (uint32 eventId = events.ExecuteEvent())
			{
				...
				case EVENT_SWEEP:           // Cast for left arm
					...
					events.ScheduleEvent(EVENT_SWEEP, urand(16 * IN_MILLISECONDS, 23 * IN_MILLISECONDS));
					return;
				case EVENT_STONE_GRIP:
					...
					if (haveRightArm)
					{
						...
						/*if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 1, 40.f, true)) // O hacer un RANDOM y si el target = me->GetVictim() o tiene un aura de STONE GRIP, pues i++;
						{
						target->CastSpell(target, SPELL_STONE_GRIP);
						//target->Kill(target);
						}	*/
						...
						events.ScheduleEvent(EVENT_START_CHECKING_STONEGRIP, 2 * IN_MILLISECONDS);
						events.ScheduleEvent(EVENT_CANCEL_CHECKING_STONEGRIP, 14 * IN_MILLISECONDS);

					}

					if (GetDifficulty() == RAID_DIFFICULTY_25MAN_NORMAL)
						events.ScheduleEvent(EVENT_STONE_GRIP2, 1500);

					events.ScheduleEvent(EVENT_STONE_GRIP, 25 * IN_MILLISECONDS);
					return;
				case EVENT_START_CHECKING_STONEGRIP:
					// Incluso por Aura, hacer un checker de todos los jugadores cada segundo una vez el stone grip sea casteado y si alguno tiene el aura, forzar el poner el spell del daño.
					// Parece que él se lo tiene que castear a sí mismo. ¿Quizá el aura stun está interfiriendo?
					if (Creature* rightArm = me->FindNearestCreature(NPC_RIGHT_ARM, 30.f))
					{
						for (uint8 i = 0; i < RAID_MODE(1, 3); i++)
						{
							if (Unit* passenger = rightArm->GetVehicleKit()->GetPassenger(i))
							{
								if (Player* player = passenger->ToPlayer())
								{

									if (!player->HasAura(RAID_MODE(64290, 64292)))
									{
										//std::cout << "Player " << std::string(player->GetName()) << " does not have the Aura, applying it. \n";
										player->CastSpell(player, RAID_MODE(64290, 64292), true);
									}
									/*else
									std::cout << "Player " << std::string(player->GetName()) << " HAS the Aura so, doing nothing. \n";*/
								}
							}
						}
					}
					events.ScheduleEvent(EVENT_START_CHECKING_STONEGRIP, 1 * IN_MILLISECONDS);
					break;
				case EVENT_CANCEL_CHECKING_STONEGRIP:
					events.CancelEvent(EVENT_START_CHECKING_STONEGRIP);
					break;
				case EVENT_STONE_GRIP2:
					if (haveRightArm)
					{
						DoCast(SPELL_STONE_GRIP);
						/*bool notValidTarget = true;

						while (notValidTarget)
						{
						if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 1, 90.f, true)) // O hacer un RANDOM y si el target = me->GetVictim() o tiene un aura de STONE GRIP, pues i++;
						{
						if (!target->HasAura(SPELL_STONE_GRIP_25))
						{
						target->CastSpell(target, SPELL_STONE_GRIP);
					...
					if (GetDifficulty() == RAID_DIFFICULTY_25MAN_NORMAL)
						events.ScheduleEvent(EVENT_STONE_GRIP3, 1500);

					//events.ScheduleEvent(EVENT_STONE_GRIP3, 1500);
					return;
				case EVENT_STONE_GRIP3:
					if (haveRightArm)
					{
						DoCast(SPELL_STONE_GRIP);
						/*
				...
				case EVENT_SMASH:
					...
					events.ScheduleEvent(EVENT_SMASH, 13 * IN_MILLISECONDS);
					return;
				...
				case EVENT_FOCUSED_EYEBEAM:
					...
					if (Player* eyebeamTargetUnit = GetEyeBeamTarget())
					{
						...
						Talk(WHISPER_EYEBEAM, eyebeamTargetUnit);

						eyebeamTargetUnit->CastSpell(eyebeamTargetUnit, 63343, true, NULL, NULL, me->GetGUID()); // Summon 1, castea el player.
						eyebeamTargetUnit->CastSpell(eyebeamTargetUnit, 63701, true, NULL, NULL, me->GetGUID()); // Summon 2, castea el player.
					}

					// Trying to reset here the Eye Beam Target.

					events.ScheduleEvent(EVENT_FOCUSED_EYEBEAM, urand(15 * IN_MILLISECONDS, 25 * IN_MILLISECONDS));
					return;
				default:
					return;
				}
...
public:
	...
	struct npc_kologarn_armAI : public ScriptedAI
	{
		...
		void Reset()
		{
			...
			if (GetDifficulty() == RAID_DIFFICULTY_25MAN_NORMAL)
				absorptionNeeded = 550000;

			else if (GetDifficulty() == RAID_DIFFICULTY_10MAN_NORMAL)
				absorptionNeeded = 210000;
		}
		...
		void JustDied(Unit* /*victim*/)
		{
			...
			passenger->GetMotionMaster()->MoveJump(1753.1907f, -23.8603f, 452.8268f, 448.8f, 30.0f, 10.0f);
			}
			}*/
		...
		void DamageTaken(Unit* pDone, uint32 &damage)
		{
			...
			{
				if (me->GetVehicleKit()->GetPassenger(0) || me->GetVehicleKit()->GetPassenger(1) || me->GetVehicleKit()->GetPassenger(2))
				{
					damageAbsorbed += damage;

					if (damageAbsorbed >= absorptionNeeded)
					{
						...
						for (uint8 i = 0; i < 3; i++)
						{
							...
							{
								me->AI()->DoCast(passenger, SPELL_STONE_GRIP_CANCEL);
								passenger->ExitVehicle();
								/*			exitPosition.m_positionX = 1769.08f;
								exitPosition.m_positionY = -9.85f;
								exitPosition.m_positionZ = 448.80f;*/
								passenger->GetMotionMaster()->MoveJump(1769.08f, -9.85f, 448.80f, 30.0f, 30.0f, 10.0f);
								passenger->RemoveAurasDueToSpell(64708);
								passenger->RemoveAura(RAID_MODE(64290, 64292));
							}
						}
			...
			if (damage && me->GetHealthPct() <= 8)
			{
				if (me->GetEntry() == NPC_RIGHT_ARM)
				{
					if (me->GetVehicleKit())
					{
						/*if (Unit* passenger = me->GetVehicleKit()->GetPassenger(0))
						{
						me->AI()->DoCast(SPELL_STONE_GRIP_CANCEL);
						passenger->ExitVehicle();
						passenger->GetMotionMaster()->MoveJump(1753.1907f, -23.8603f, 452.8268f, 448.8f, 30.0f, 10.0f);
						}*/
	...
	private:
		uint32 damageAbsorbed;
		uint32 absorptionNeeded;
	};
...
public:
	...
	class spell_ulduar_stone_grip_cast_target_SpellScript : public SpellScript
	{
		...
		void HandleForceCast(SpellEffIndex i)
		{
			...
			// Echar un ojo aquí, parece que falla. Quizá tirarlo tú de alguna manera.
...
public:
	...
	class spell_ulduar_squeezed_lifeless_SpellScript : public SpellScript
	{
		...
		void HandleInstaKill(SpellEffIndex /*effIndex*/)
		{
			...
			*/
			Position pos;
			pos.m_positionX = 1769.08f;
			pos.m_positionY = -9.85f;
			pos.m_positionZ = 448.80f;
		...
		void MoveCorpse()
		{
			...
			//GetHitUnit()->RemoveAurasDueToSpell(64708);
			// Remove Stone Grip.
			GetHitUnit()->RemoveAura(GetHitUnit()->GetMap()->GetDifficulty() == DUNGEON_DIFFICULTY_NORMAL ? 64290 : 64292);
			//if (Unit* target = SelectTarget(SELECT_TARGET_BOTTOMAGGRO))
			GetHitUnit()->GetMotionMaster()->MoveJump(1769.08f, -9.85f, 448.80f, 30.0f, 30.0f, 10.0f);
...
public:
	...
	class spell_ulduar_stone_grip_AuraScript : public AuraScript
	{
		...
		void OnEnterVehicle(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
		{
			...
			// Forzar que lo castee aquí por si acaso?
		}
		...
		void OnRemoveVehicle(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
			...
			/* REVISAR TODO ESTO, ACABAS DE FUSIONAR AMBOS SCRIPTS, METIDO EL ONREMOVEHICLE CUANDO EN EL ANTIGUO TAN SOLO ESTABA EL ONENTER, FIJATE

			SI EN EL ANTIGUO/ORI SCRIPT, COMO GESTIONA EL GRIP A VARIOS EN 25. */
		{
			...
			/*
			pos.m_positionX = 1753.190796f;
			pos.m_positionY = -23.860395f;
			pos.m_positionZ = 452.826813f;
			*/
...
class npc_focused_eye_beam : public CreatureScript
{
public:
	npc_focused_eye_beam() : CreatureScript("npc_focused_eye_beam") { }

	struct npc_focused_eye_beamAI : public ScriptedAI
	{
		npc_focused_eye_beamAI(Creature* creature) : ScriptedAI(creature),
			_instance(creature->GetInstanceScript())
		{
			me->SetReactState(REACT_PASSIVE);
			DoZoneInCombat(me, 150.00f);
		...
		void Reset() override
		{
			if (me->GetEntry() == NPC_FOCUSED_EYEBEAM_RIGHT)
			{
				//me->SetReactState(REACT_PASSIVE);
				me->SetSpeed(MOVE_RUN, 1.2f);
				events.ScheduleEvent(EVENT_CHASE_RIGHT, 1000);
			}

			if (me->GetEntry() == NPC_FOCUSED_EYEBEAM)
			{
				//me->SetReactState(REACT_PASSIVE);
				if (targetPlayer = me->SelectNearestPlayer(10.0f))
				{
					events.ScheduleEvent(EVENT_UPDATE_TARGET, 1000);
				}
			}
		}
			...
			void EnterCombat(Unit* /*who*/) override
		{
			...
			{
			case NPC_FOCUSED_EYEBEAM:
			me->AI()->DoCast(SPELL_FOCUSED_EYEBEAM_VISUAL_LEFT);
			break;
			case NPC_FOCUSED_EYEBEAM_RIGHT:
			me->AI()->DoCast(SPELL_FOCUSED_EYEBEAM_VISUAL_RIGHT);
			break;
			default:
			break;
			}
			me->AI()->DoCast(SPELL_FOCUSED_EYEBEAM_VISUAL_LEFT);*/
			me->AI()->DoCast(SPELL_FOCUSED_EYEBEAM_PERIODIC);

			//events.ScheduleEvent(EVENT_CHASE_RIGHT, 500);
			/*
	...
	{
		return GetInstanceAI<npc_focused_eye_beamAI>(creature);
	}
};
...
public:
	...
	struct npc_rubbleAI : public ScriptedAI
	{
			...
			void UpdateAI(uint32 Diff) override
		{
			...
			if (rumble <= Diff)
			{
				DoCastVictim(RAID_MODE(63818, 63978));
				rumble = urand(8000, 15000);
			}
			else
				rumble -= Diff;
