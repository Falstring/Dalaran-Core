...
enum FreyaSpells
{
	...
	SPELL_STORMLASHER_VISUAL = 62639,
...
public:
	...
	struct boss_freyaAI : public BossAI
	{
		...
		void Reset() override
		{
			...
			gotEncounterFinished = gotEncounterFinished || (instance->GetBossState(BOSS_FREYA) == DONE);
			...
			// Despawn Nature bombs.
			std::list<Creature*> pCreatureList;

			Trinity::AllCreaturesOfEntryInRange checker(me, 34129, 200.0f);
			...
			void EnterCombat(Unit* who) override
		{
			...
			/* Reduce el radius y que la castee freya sobre sí misma. */

			if (Elder[0] && Elder[0]->IsAlive())
			{
				Elder[0]->CastSpell(me, SPELL_IRONBRANCH_ESSENCE, true); // 
				//Elder[0]->CastSpell(me, 65588, true);
				//me->CastSpell(Elder[0], 65588, true);
				events.ScheduleEvent(EVENT_STRENGTHENED_IRON_ROOTS, urand(10000, 20000));
				elderCount++;
			}

			if (Elder[1] && Elder[1]->IsAlive()) // ESTE DA BRIGHTLEAF
			{
				Elder[1]->CastSpell(me, SPELL_BRIGHTLEAF_ESSENCE, true);
				//Elder[0]->CastSpell(me, 65587, true);
			...
			if (Elder[2] && Elder[2]->IsAlive())
			{
				...
				//me->CastSpell(Elder[0], 65589, true);
			...
			//std::cout << "Elder count: " << uint32(elderCount) << " - \n";
			me->CastCustomSpell(SPELL_ATTUNED_TO_NATURE, SPELLVALUE_AURA_STACK, 150, me, true);

			events.ScheduleEvent(EVENT_WAVE, 10000);
			events.ScheduleEvent(EVENT_EONAR_GIFT, 23000);
			events.ScheduleEvent(EVENT_ENRAGE, 600000);
			events.ScheduleEvent(EVENT_SUNBEAM, urand(5000, 12000));

			me->CallForHelp(115.0f);
		}

			void CastNatureBombs()
			{
			...
			void UpdateAI(uint32 diff) override
		{
			...
			while (uint32 eventId = events.ExecuteEvent())
			{
				...
				case EVENT_SUNBEAM:
					...
					events.ScheduleEvent(EVENT_SUNBEAM, urand(10000, 16000));
					break;
				case EVENT_NATURE_BOMB: /* Once scheduled, waves should not appear anymore */
					CastNatureBombs();
					events.RescheduleEvent(EVENT_NATURE_BOMB, 18000);
					break;
					break;
				case EVENT_UNSTABLE_ENERGY:
					if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0, 100.0f, true))
						me->SummonCreature(33170, target->GetPositionX(), target->GetPositionY(), target->GetPositionZ(), target->GetOrientation());
					//DoCast(target, 62866, true); //SPELL_FREYA_UNSTABLE_SUNBEAM
					events.ScheduleEvent(EVENT_UNSTABLE_ENERGY, 13000);
					break;
				case EVENT_WAVE:
					...
					if (waveCount < 6)
						...
						events.ScheduleEvent(EVENT_NATURE_BOMB, urand(45000, 55000));
					break;
				case EVENT_EONAR_GIFT:
					Talk(EMOTE_LIFEBINDERS_GIFT);
					DoCast(me, SPELL_SUMMON_EONAR_GIFT);
					events.ScheduleEvent(EVENT_EONAR_GIFT, urand(34000, 46000));
					break;
				...
				case EVENT_GROUND_TREMOR:
					...
					events.ScheduleEvent(EVENT_GROUND_TREMOR, urand(20000, 30000));
					break;
				}
			}

			if (!me->HasAura(RAID_MODE(62528, 62892)))
							...
							if (Elemental[0][i] && Elemental[1][i] && Elemental[2][i])
							{
								for (uint8 n = 0; n < 3; ++n)
								{
									...
									else
									{
										...
										if (Elemental[n][i]->GetEntry() == NPC_STORM_LASHER)
											Elemental[n][i]->AI()->DoCast(62851);

										if (Elemental[n][i]->GetEntry() == NPC_SNAPLASHER)
											Elemental[n][i]->AI()->DoCast(62848);

										if (Elemental[n][i]->GetEntry() == NPC_ANCIENT_WATER_SPIRIT)
											Elemental[n][i]->AI()->DoCast(62849);
		...
		void EncounterPostProgress()
		{
			...
			//std::cout << "FREYA JUST DIED WITH " << uint32(elderCount) << " ALIVE  \n";

			switch (elderCount)
			{
			case 3: // 194327 
				me->SummonGameObject(RAID_MODE(194327, 194331), me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), 1.55988f, 0.0f, 0.0f, 1.0f, 1.0f, WEEK);
				break;
			case 2:
			case 1:
			case 0:
				me->SummonGameObject(RAID_MODE(194326, 194325), me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), 1.55988f, 0.0f, 0.0f, 1.0f, 1.0f, WEEK);
				break;
			default:
				break;
			}
			...
			void UpdateAI(uint32 diff) override
		{
			...
			while (uint32 eventId = events.ExecuteEvent())
			{
				...
				case EVENT_BARK:
					...
					events.ScheduleEvent(EVENT_BARK, urand(50000, 60000));
					break;
				case EVENT_FISTS:
...
public:
	...
	struct npc_detonating_lasherAI : public ScriptedAI
	{
		...
		void Reset() override
		{
			lashTimer = urand(15000, 20000);
			changeTargetTimer = 2500;
	...
	struct npc_ancient_water_spiritAI : public ScriptedAI
	{
			...
			void UpdateAI(uint32 diff) override
		{
			...
			if (tidalWaveTimer <= diff)
			{
				...
				tidalWaveTimer = urand(18000, 26000);
			}
			else
				tidalWaveTimer -= diff;
...
public:
	...
	struct npc_storm_lasherAI : public ScriptedAI
	{
		...
		void Reset() override
		{
			...
			stormboltTimer = 7000;
			DoCast(SPELL_STORMLASHER_VISUAL);
		}
			...
			void UpdateAI(uint32 diff) override
		{
			...
			if (lightningLashTimer <= diff)
			{
				...
				lightningLashTimer = urand(12000, 20000);
			}
			else
			...
			if (stormboltTimer <= diff)
			{
				...
				stormboltTimer = urand(6000, 10000);
			}
			else
				stormboltTimer -= diff;
...
public:
	...
	struct npc_snaplasherAI : public ScriptedAI
	{
		...
		void UpdateAI(uint32 /*diff*/) override
		{
			...
			if (!me->HasAura(RAID_MODE(62664, 64190)))
				DoCast(RAID_MODE(62664, 64191));
...
public:
	...
	struct npc_sun_beamAI : public ScriptedAI
	{
		npc_sun_beamAI(Creature* creature) : ScriptedAI(creature)
		{
			...
			/*if (Creature* Freya = ObjectAccessor::GetCreature(*me, instance->GetGuidData(BOSS_FREYA)))
			{
			if (Freya->HasAura(62968))
			me->AddAura(62968,me);
			}*/
			me->AddAura(SPELL_FREYA_UNSTABLE_ENERGY_VISUAL, me);
...
{
public:
	npc_unstable_sun_beam() : CreatureScript("npc_unstable_sun_beam") { } /* modelId1 - 169, 2 -> 23258*/
