...
Position const AllianceLandingCampSpiritGuidePosition = { 5140.79f, 2179.12f, 390.951f, 1.97222f };
Position const HordeChilledQuagmireSpiritGuidePosition = { 5031.84f, 3710.75f, 372.483f, 3.3827f };

enum IDdata
{
	WINTERGRASP_ZONEID	= 4197,
	SPELL_GHOST_AURA	= 8326,
};
        ...
        struct npc_wg_spirit_guideAI : public ScriptedAI
        {
            ...
            void UpdateAI(uint32 diff) override
            {
				if (!me->HasUnitState(UNIT_STATE_CASTING))
				{
					...
					if ((me->GetPosition() == AllianceLandingCampSpiritGuidePosition || me->GetPosition() == HordeChilledQuagmireSpiritGuidePosition) && me->GetZoneId() == WINTERGRASP_ZONEID)
						events.RescheduleEvent(EVENT_REVIVE_PLAYERS, 23 * IN_MILLISECONDS);						
				}
						...
						case EVENT_REVIVE_PLAYERS:														
							...
							{								
								...
								for (Map::PlayerList::const_iterator itr = Players.begin(); itr != Players.end(); ++itr)
								{
									...
									{
										if (!player->HasAura(SPELL_GHOST_AURA))																		
											continue;																				
...
enum eWintergraspBattleMage
{
	...
	GO_WINTERGRASP_ALLIANCE_CAMP = 305205,
	GO_WINTERGRASP_HORDE_CAMP = 305206,
...
class npc_wg_queue : public CreatureScript
{
    ...
    struct npc_wg_queueAI : public ScriptedAI
    {
        ...
        {
            FrostArmor_Timer = 0;
			CheckWGTimer = 60 * IN_MILLISECONDS;
			
			needToDespawnPortal = false;
			portalSummoned = false;
			keepAnnouncements = false;
        }

        uint32 FrostArmor_Timer;
		uint32 CheckWGTimer;

		bool needToDespawnPortal;
		bool portalSummoned;
		uint32 despawnPortalTimer;
		
		uint32 announceWhileWarTimer;

		bool keepAnnouncements;
        ...
        void UpdateAI(uint32 diff) override
        {
			...
			if (keepAnnouncements && announceWhileWarTimer <= diff)
			{
				Talk(0);
				announceWhileWarTimer = 300 * IN_MILLISECONDS;
			}
			else announceWhileWarTimer -= diff;

			if (needToDespawnPortal && despawnPortalTimer <= diff)
            {				
					if (GameObject* portal = me->FindNearestGameObject(GO_WINTERGRASP_PORTAL, 25.f))					
						portal->RemoveFromWorld();											
					else if (GameObject* portal = me->FindNearestGameObject(GO_WINTERGRASP_ALLIANCE_CAMP, 25.f))					
						portal->RemoveFromWorld();											
					else if (GameObject* portal = me->FindNearestGameObject(GO_WINTERGRASP_HORDE_CAMP, 25.f))					
						portal->RemoveFromWorld();											
			...
			else despawnPortalTimer -= diff;

			if (CheckWGTimer <= diff)
			{								
				Battlefield* wintergrasp = sBattlefieldMgr->GetBattlefieldByBattleId(BATTLEFIELD_BATTLEID_WG);
				if (wintergrasp)
				{
					...
					if (timer >= 14 * MINUTE && timer <= 16 * MINUTE)
					{
						Talk(3); // 15 minutes.
					}
					else if (timer >= 9 * MINUTE && timer <= 11 * MINUTE)
					{
						Talk(2); // 10 minutes.
					}
					else if (timer >= 4 * MINUTE && timer <= 6 * MINUTE)
					{
						Talk(1); // 5 minutes. 
					}					
					else if (!portalSummoned && timer <= 2 * MINUTE)
					{
						
						if (me->GetEntry() == NPC_MAGISTER_SURDIEL) // Horde
						{														
							me->AI()->DoCast(SPELL_PORTAL_VISUAL);
							Talk(0);

							switch (wintergrasp->GetDefenderTeam())
							{
							case TEAM_HORDE:
								me->SummonGameObject(GO_WINTERGRASP_PORTAL, 5924.042969f, 570.354492f, 661.087280f, 5.930885f, 0, 0, 0.324484f, -0.945891f, 0);
								break;
							case TEAM_ALLIANCE:								
								me->SummonGameObject(GO_WINTERGRASP_HORDE_CAMP, 5924.042969f, 570.354492f, 661.087280f, 5.930885f, 0, 0, 0.324484f, -0.945891f, 0);
								break;
							default:
								break;
							}
							// Horda.							
							needToDespawnPortal = true;
							despawnPortalTimer = 1800000;


							portalSummoned = true;
						}
						else if (me->GetEntry() == NPC_ARCANIST_BRAEDIN) // Alliance
						{
							...
							{
							case TEAM_HORDE:
								me->SummonGameObject(GO_WINTERGRASP_ALLIANCE_CAMP, 5686.8f, 773.175f, 647.752f, 1.83259f, 0, 0, 0, 0, 0);
								break;
							case TEAM_ALLIANCE:
								me->SummonGameObject(GO_WINTERGRASP_PORTAL, 5686.8f, 773.175f, 647.752f, 1.83259f, 0, 0, 0, 0, 0);
								break;
							default:
								break;
							}
						...
						keepAnnouncements = true;
						announceWhileWarTimer = 180 * IN_MILLISECONDS;						
					}
				}
					
				CheckWGTimer = 60000;
			}
			else CheckWGTimer -= diff;
...
public:
    ...
    class spell_wintergrasp_rp_gg_SpellScript : public SpellScript
    {
        ...
        void HandleFinish()
        {
            ...
            if(GetCaster()->IsWithinLOS(GetExplTargetDest()->GetPositionX(), GetExplTargetDest()->GetPositionY(), GetExplTargetDest()->GetPositionZ()))
    ...
    class spell_wintergrasp_create_vehicle_SpellScript : public SpellScript
    {
        ...
        void HandleSummon(SpellEffIndex effIndex)
        {
            ...
            if (GetCaster() && GetCaster()->GetMap())
                ...
                {
                    // This should be handled by summon->HandleSpellClick(GetCaster()) but we got some crashes on Algalon
                    // Maybe it was the nullptr check from GetCaster() and GetCaster()->GetMap()
                    GetCaster()->CastSpell(summon, 66668, true);
                    GetCaster()->CastSpell(summon, 65635, true);
                    GetCaster()->EnterVehicle(summon, 0);                    
                }                    
        }
