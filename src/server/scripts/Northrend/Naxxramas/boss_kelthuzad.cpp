...
MODEL INFO 
 4 Y 4 PARA GUARDIANS
 select * from world_test.creature_model_info where modelid=16586
				...
				while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_WASTE:
							...
							events.ScheduleEvent(EVENT_WASTE, urand(1000, 4000));
                            break;
                        case EVENT_ABOMIN:
                            if (nAbomination < 8)
                            {
                                ...
                                nAbomination++;
								events.ScheduleEvent(EVENT_ABOMIN,19000);
                            }
                            break;
                        case EVENT_WEAVER:
                            if (nWeaver < 8)
                            {
                                ...
                                nWeaver++;
                                events.ScheduleEvent(EVENT_WEAVER, 19000);
                            }
                        ...
                        case EVENT_PHASE:
                            ...
                            events.ScheduleEvent(EVENT_BOLT, urand(4000, 8000));
                            events.ScheduleEvent(EVENT_NOVA, 15000);
                            events.ScheduleEvent(EVENT_DETONATE, 30000);
                            events.ScheduleEvent(EVENT_FISSURE, urand(10000, 18000));
                            events.ScheduleEvent(EVENT_BLAST, urand(45000, 48000));
                            if (GetDifficulty() == RAID_DIFFICULTY_25MAN_NORMAL)
                                events.ScheduleEvent(EVENT_CHAIN, 30000);
                            Phase = 2;
                            break;
                        default:                            
                            break;
            ...
            else
            {
                ...
                else if (nGuardiansOfIcecrownCount < RAID_MODE(2, 4))
                {
                    if (uiGuardiansOfIcecrownTimer <= diff)
                    {
                        ...
                        if (Creature* pGuardian = DoSummon(NPC_ICECROWN, Pos[RAND(1, 4, 7, 10)]))
						{
							pGuardian->SetFloatValue(UNIT_FIELD_COMBATREACH, 2);
							pGuardian->SetFloatValue(UNIT_FIELD_BOUNDINGRADIUS, 3);
						}
                ...
                if (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_NOVA:
                            ...
                            events.ScheduleEvent(EVENT_NOVA, urand(15000, 24000));
                            break;
                        case EVENT_CHAIN:
                        {
                            uint32 count = 3;
							/*
							Idea: while, y hasta que no sean los 3 exitosos, nada, para testear ponerlo a 20 segundos o algo asi
							asi igual entra en bucle infinito, cuidado si hay menos de 3 jugadores
							*/
                        ...
                        case EVENT_FISSURE:
                            ...
                            events.ScheduleEvent(EVENT_FISSURE, urand(10000, 22000));
                            break;
                        case EVENT_BLAST:
                            ...
                            events.ScheduleEvent(EVENT_BLAST, 45000);
                            break;
                        default:                            
                            break;
                    }
    ...
    public:
        ...
        struct npc_kelthuzad_abominationAI : public ScriptedAI
        {
            ...
            void UpdateAI(uint32 diff) override
            {
				...
				if(HealthBelowPct(50) && !FrenzyCasted)
				{
                ...
                while (uint32 eventId = _events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_MORTAL_WOUND:
                            ...
                            _events.ScheduleEvent(EVENT_MORTAL_WOUND, urand(7000, 13000));
                            break;
                        default:
                            break;
                    }
...
class npc_kelthuzad_guardian_icecrown : public CreatureScript
{
    public:
        npc_kelthuzad_guardian_icecrown() : CreatureScript("npc_kelthuzad_guardian_icecrown") { }

        struct npc_kelthuzad_guardian_icecrownAI : public ScriptedAI
        {
            npc_kelthuzad_guardian_icecrownAI(Creature* creature) : ScriptedAI(creature)
            {               
            }
            ...
            {
                _events.Reset();				
                _events.ScheduleEvent(EVENT_BLOOD_TAP, urand(8000, 14000));                
            }
                ...
                while (uint32 eventId = _events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_BLOOD_TAP:
                            ...
                            _events.ScheduleEvent(EVENT_BLOOD_TAP, urand(8000, 13000));
                            break;
                        default:
                            break;
                    }
        ...
        CreatureAI* GetAI(Creature* creature) const override
        {
            return GetInstanceAI<npc_kelthuzad_guardian_icecrownAI>(creature);
        }
};
...
public:
    ...
    struct npc_shadow_fissureAI : public ScriptedAI
    {
        npc_shadow_fissureAI(Creature* creature) : ScriptedAI(creature)
        {
...
void AddSC_boss_kelthuzad()
{
	...
	new npc_kelthuzad_guardian_icecrown();
