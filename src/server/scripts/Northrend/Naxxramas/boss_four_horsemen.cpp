...
class npc_four_horsemen_intro : public CreatureScript
{
    public:
        npc_four_horsemen_intro() : CreatureScript("npc_four_horsemen_intro") { }

        struct npc_four_horsemen_introAI : public ScriptedAI
        {
            npc_four_horsemen_introAI(Creature* creature) : ScriptedAI(creature),
                instance(creature->GetInstanceScript()), introDone(false), introInProcess(false)
            {					
		...
		void UpdateAI(uint32 diff) override
		{
			...
			if(!introInProcess)
			{
				if(Player* player=me->SelectNearestPlayer(25.0f))
				{
					...
					{	
						events.ScheduleEvent(INTRO_ZELIEK_1, 2000);
						introInProcess=true;
					}
				}
			}
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
				...
				{
					case INTRO_ZELIEK_1:
						if(Creature* Sir = ObjectAccessor::GetCreature(*me, instance->GetGuidData(DATA_SIR)))
						{
							Sir->AI()->Talk(5);
							events.ScheduleEvent(INTRO_BLAUMEUX_2, 7000);
						}
						break;
					case INTRO_BLAUMEUX_2:
						if(Creature* Lady = ObjectAccessor::GetCreature(*me, instance->GetGuidData(DATA_LADY)))
						{
							Lady->AI()->Talk(5);
							events.ScheduleEvent(INTRO_RIVENDARE_3, 6000);
						}
						break;
					case INTRO_RIVENDARE_3:
						if(Creature* Baron = ObjectAccessor::GetCreature(*me, instance->GetGuidData(DATA_BARON)))
						{	
							Baron->AI()->Talk(5);
							events.ScheduleEvent(INTRO_BLAUMEUX_4, 7000);
						}
						break;
					case INTRO_BLAUMEUX_4:
						if(Creature* Lady = ObjectAccessor::GetCreature(*me, instance->GetGuidData(DATA_LADY)))
						{
							Lady->AI()->Talk(6);
							events.ScheduleEvent(INTRO_ZELIEK_5, 6000);
						}
						break;
					case INTRO_ZELIEK_5:
						if(Creature* Sir = ObjectAccessor::GetCreature(*me, instance->GetGuidData(DATA_SIR)))
						{
							Sir->AI()->Talk(6);
							events.ScheduleEvent(INTRO_THANE_6, 7000);							
						}
						break;
					case INTRO_THANE_6:
						if(Creature* Thane = ObjectAccessor::GetCreature(*me, instance->GetGuidData(DATA_THANE)))
						{
							Thane->AI()->Talk(5);
							events.ScheduleEvent(INTRO_RIVENDARE_7, 6000);							
						}
						break;
					case INTRO_RIVENDARE_7:
						if(Creature* Baron = ObjectAccessor::GetCreature(*me, instance->GetGuidData(DATA_BARON)))
						{	
							Baron->AI()->Talk(6);
						}
        ...
        CreatureAI* GetAI(Creature* creature) const override
        {
            return GetInstanceAI<npc_four_horsemen_introAI>(creature);
        }
};
