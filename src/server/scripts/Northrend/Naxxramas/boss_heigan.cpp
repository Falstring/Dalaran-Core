...
static const uint8 numEntranceTiles = 5;
static const uint32 cheatersEruptionsDBGUIDS[5] = {
	65811,
	65802,
	65808,
	65809,
...
public:
	...
	struct boss_heiganAI : public BossAI
	{
		...
		void EnterCombat(Unit* /*who*/) override
		{
			...
			//Entrance tiles, they have different GUID on DB, not consecutives so, hardcoded here.
			_entranceTiles.clear();
			for (uint8 entranceTile = 0; entranceTile < numEntranceTiles; ++entranceTile)
			{				
				std::pair<std::unordered_multimap<uint32, GameObject*>::const_iterator, std::unordered_multimap<uint32, GameObject*>::const_iterator> entranceTileIt = mapGOs.equal_range(cheatersEruptionsDBGUIDS[entranceTile]);
				std::unordered_multimap<uint32, GameObject*>::const_iterator it = entranceTileIt.first;
				_entranceTiles.push_back(it->second->GetGUID());
			}
		}
		...
		void UpdateAI(uint32 diff) override
		{
			...
			while (uint32 eventId = events.ExecuteEvent())
			{
				...
				case EVENT_FEVER:
					...
					events.Repeat(urand(20*IN_MILLISECONDS, 25*IN_MILLISECONDS));
					break;
				case EVENT_DANCE:
				...
				case EVENT_DANCE_END:
					...
					_safeSection = 0;
					events.ScheduleEvent(EVENT_DISRUPT, urand(10*IN_MILLISECONDS, 25*IN_MILLISECONDS), 0, PHASE_FIGHT);
					events.ScheduleEvent(EVENT_FEVER, urand(15*IN_MILLISECONDS, 20*IN_MILLISECONDS), 0, PHASE_FIGHT);
					events.RescheduleEvent(EVENT_DANCE, 90*IN_MILLISECONDS, 0, PHASE_FIGHT);
					events.RescheduleEvent(EVENT_ERUPT, 15*IN_MILLISECONDS, 0, PHASE_FIGHT);
					me->CastStop();
					me->SetReactState(REACT_AGGRESSIVE);
					DoZoneInCombat();
					break;
				case EVENT_ERUPT:
					...
					{					
						...
						{							
							...
							if (section == 0)
							{
								for (uint8 i = 0; i < numEntranceTiles; i++)
									if (GameObject* tile = ObjectAccessor::GetGameObject(*me, _entranceTiles[i]))
									{
										tile->SendCustomAnim(0);
										tile->CastSpell(nullptr, SPELL_ERUPTION);
									}
					...
					events.RescheduleEvent(EVENT_ERUPT, events.IsInPhase(PHASE_DANCE) ? Seconds(4) : Seconds(10));
					break;
				}
			}
	...
	private:
		...
		std::vector<ObjectGuid> _entranceTiles; // populated on encounter start
