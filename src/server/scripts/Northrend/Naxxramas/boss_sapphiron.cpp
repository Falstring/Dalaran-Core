...
public:
	...
	struct boss_sapphironAI : public BossAI
	{
		...
		{
			me->SetReactState(REACT_AGGRESSIVE);
			events.ScheduleEvent(EVENT_CLEAVE, urand(5000, 15000), 0, PHASE_GROUND);
			events.ScheduleEvent(EVENT_TAIL, urand(70000, 10000), 0, PHASE_GROUND);
			events.ScheduleEvent(EVENT_BLIZZARD, urand(5000, 10000), 0, PHASE_GROUND);
			if (initial)
			{
				events.ScheduleEvent(EVENT_DRAIN, urand(22000, 28000));
			...
			if (events.IsInPhase(PHASE_GROUND))
			{
				while (uint32 eventId = events.ExecuteEvent())
				{
					...
					case EVENT_TAIL:
						...
						events.ScheduleEvent(EVENT_TAIL, urand(7000, 10000), 0, PHASE_GROUND);
						return;
					case EVENT_DRAIN:
						if (events.IsInPhase(PHASE_FLIGHT))
							_delayedDrain = true;
					...
					case EVENT_BLIZZARD:
						...
						events.ScheduleEvent(EVENT_BLIZZARD, RAID_MODE(Seconds(21), Seconds(11)), 0, PHASE_GROUND);
						break;
					case EVENT_FLIGHT:
