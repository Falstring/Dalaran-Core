...
public:
    ...
    struct boss_ymironAI : public BossAI
    {
        boss_ymironAI(Creature* creature) : BossAI(creature, DATA_KING_YMIRON)
        {
            ...
            me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE | UNIT_FLAG_IMMUNE_TO_PC | UNIT_FLAG_DISABLE_MOVE);
            Initialize();
        ...
        void MovementInform(uint32 type, uint32 pointId) override
        {
            ...
            if (pointId == POINT_BOAT) // Check might not be needed.
            {
                ...
                {
                    if (Creature *cr = me->SummonTrigger(ActiveBoat[ActiveOrder[ActivedNumber]].SpawnX, ActiveBoat[ActiveOrder[ActivedNumber]].SpawnY, ActiveBoat[ActiveOrder[ActivedNumber]].SpawnZ, 0, 1800000))
                    {
