...
{
	{ 335.80f,  -511.89f, 117.82f },
	{ 409.94f,  -513.97f, 122.00f },
	{ 509.86f,  -519.56f, 126.31f },
	{ 514.68f,  -528.33f, 126.51f }
};
	...
	struct boss_skadiAI : public ScriptedAI
	{
		...
		void EnterCombat(Unit* pWho)
		{
			if (!EventStarted)
			{
				...
				events.RescheduleEvent(EVENT_SKADI_START, 0);
			}
		}
		...
		void UpdateAI(uint32 diff)
		{
			...
			case EVENT_SKADI_CRUSH:
			{
				...
				events.RescheduleEvent(EVENT_SKADI_CRUSH, 8000);
				break;
			}
			case EVENT_SKADI_SPEAR:
			{
				...
				events.RescheduleEvent(EVENT_SKADI_SPEAR, 10000);
				break;
			}
			case EVENT_SKADI_WHIRLWIND:
			{
				me->CastSpell(me, IsHeroic() ? SPELL_WHIRLWIND_H : SPELL_WHIRLWIND_N, false);
				events.RescheduleEvent(EVENT_SKADI_WHIRLWIND, 15000 + rand() % 5000);
	...
	struct boss_skadi_graufAI : public VehicleAI
	{
		...
		void DoAction(int32 param)
		{
			...
			else if (param == ACTION_REMOVE_SKADI)
			{
				...
				events.ScheduleEvent(EVENT_GRAUF_REMOVE_SKADI, 4000);
			}
		...
		void MovementInform(uint32 uiType, uint32 Id)
		{
			...
			case 3:
				...
				me->TextEmote(EMOTE_IN_RANGE, 0, true);
				me->SetFacingTo(M_PI);
				break;
			}
		}
		...
		void UpdateAI(uint32 diff)
		{
			...
			{
				CheckPlayers();
				events.RescheduleEvent(EVENT_GRAUF_CHECK, 2000);
				break;
			}
			case EVENT_GRAUF_START:
			{
				me->GetMotionMaster()->Clear(true);
				me->GetMotionMaster()->MoveTakeoff(10, SkadiPosition[0]);
			...
			case EVENT_GRAUF_MOVE:
			{
				...
				events.RescheduleEvent(EVENT_GRAUF_MOVE, 25000);
				break;
			}
			case EVENT_GRAUF_SUMMON_HELPERS:
			{
				SpawnHelpers(1);
				events.RescheduleEvent(EVENT_GRAUF_SUMMON_HELPERS, 15000);
				break;
			}
		...
		if (m_pInstance)
			if (m_pInstance->GetData(SKADI_IN_RANGE) == 1)
			{
				...
				if (Creature *grauf = ObjectAccessor::GetCreature(*pPlayer, m_pInstance->GetGuidData(DATA_GRAUF)))
				{
					...
					pPlayer->CastSpell(grauf, SPELL_RAPID_FIRE, true);
