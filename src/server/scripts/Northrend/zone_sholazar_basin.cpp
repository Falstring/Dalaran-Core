...
class npc_warlord_tartek : public CreatureScript
{
public:
	npc_warlord_tartek() : CreatureScript("npc_warlord_tartek") { }

	struct npc_warlord_tartekAI : public ScriptedAI
	{
		npc_warlord_tartekAI(Creature* pCreature) : ScriptedAI(pCreature)
		{
		...
		void Reset() override
		{
			Talk(0);
			//me->setFaction(2061);
			if (Creature* zeppek = me->SummonCreature(28399, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), 0, TEMPSUMMON_MANUAL_DESPAWN))
			{
				me->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE);
				ZeppekGUID = zeppek->GetGUID();
			}
		}
		...
		{
			if (me->GetVictim() || me->GetDistance(who) > 40.0f)
				return;
			...
			if (who->GetTypeId() == TYPEID_PLAYER)
			{
				...
				if (ZeppekGUID)
				{
					if (Creature* zeppek = ObjectAccessor::GetCreature(*me, ZeppekGUID))
					{
						zeppek->Attack(who, true);
					}
				}
			}
		}
		...
		void DamageTaken(Unit* pDoneBy, uint32& uiDamage) override
		{
			if (HealthBelowPct(25))
			{
				if (pDoneBy->GetTypeId() == TYPEID_PLAYER)
				{
					...
					{
							if (player->GetQuestStatus(12575) != QUEST_STATUS_COMPLETE)
							{
								player->KilledMonsterCredit(28105);
								player->CompleteQuest(12575);
							}
						}
		...
		EventMap events;
		ObjectGuid ZeppekGUID;
	};

	CreatureAI* GetAI(Creature* pCreature) const
	{
		return new npc_warlord_tartekAI(pCreature);
	}
};
...
enum Lafoo
{
	...
	SPELL_SELF_HEAL = 43978,
	SPELL_GLINTING_BUG = 51444,
...
{
public: npc_lafoo() : CreatureScript("npc_lafoo") { }

        struct npc_lafooAI : public ScriptedAI
        {
            npc_lafooAI(Creature* creature) : ScriptedAI(creature)
            {
                DigTimer = 1.5 * IN_MILLISECONDS;
                SelfHealTimer = 5 * IN_MILLISECONDS;
                RandomTalkTimer = 2 * IN_MILLISECONDS;
                MoveToGO = false;
            ...
            void UpdateAI(uint32 diff) override
            {
                if (SelfHealTimer <= diff)
                {
                    me->CastSpell(me, SPELL_SELF_HEAL);
                    SelfHealTimer = 5 * IN_MILLISECONDS;
                }
                else
                    SelfHealTimer -= diff;

                if (RandomTalkTimer <= diff)
                {
                    Talk(SAY_RANDOM);
                    RandomTalkTimer = 45 * IN_MILLISECONDS;
                }
                else
                    RandomTalkTimer -= diff;
        ...
        private:
            uint32 DigTimer;
            uint32 SelfHealTimer;
            uint32 RandomTalkTimer;
            bool MoveToGO;
...
public:
    ...
    {
        npc_shiny_thing_visualAI(Creature* creature) : ScriptedAI(creature) { }

        void Reset() override
        {
            if (GameObject* shinyGO = me->FindNearestGameObject(GO_SHINY_BAUBLE, 1.0f))
                shinyGO->SetPhaseMask(1, true);
            me->AddAura(SPELL_GLINTING_BUG, me);
        }
    };

    CreatureAI* GetAI(Creature* creature) const override
    {
        return new npc_shiny_thing_visualAI(creature);
    }
};
...
public:
	...
	{
		if (Creature *cr = pGo->SummonCreature(28105, 6711.7f, 5145.45f, -19.3f, 0.7f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 30000))
		{
...
/*######
# Quest The Lost Mistwhisper Treasure
########*/

#define NPC_WARLORD_TARTEK  28105
#define QUEST_THE_LOST_MISTWHISPER_TREASURE  12575
...
class go_mistwhisper_treasure : public GameObjectScript
{
    public:
        ...
        {
            if (!player->FindNearestCreature(NPC_WARLORD_TARTEK, 25.0f, true))
                if (player->GetQuestStatus(QUEST_THE_LOST_MISTWHISPER_TREASURE) == QUEST_STATUS_INCOMPLETE)
                {
                    go->AddUse();
                    if (Creature* creature = go->SummonCreature(NPC_WARLORD_TARTEK, 6709.02f, 5169.21f, -20.8878f, 4.91029f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 300000))
                    {
...
class npc_warlord_tartek_treasure : public CreatureScript
{
    public:
        npc_warlord_tartek_treasure() : CreatureScript("npc_warlord_tartek_treasure") { }
    
        struct npc_warlord_tartek_treasureAI : public SmartAI
        {
            npc_warlord_tartek_treasureAI(Creature* creature) : SmartAI(creature) { }
    
            void JustDied(Unit* killer)
            {
                if (killer && killer->ToPlayer())
                    killer->ToPlayer()->CompleteQuest(QUEST_THE_LOST_MISTWHISPER_TREASURE);
        ...
        {
            return new npc_warlord_tartek_treasureAI(creature);
        }
};
...
void AddSC_sholazar_basin()
{
	...
	new npc_warlord_tartek();
    new npc_lafoo();
    new npc_shiny_thing_visual();
    new npc_warlord_tartek_treasure();
