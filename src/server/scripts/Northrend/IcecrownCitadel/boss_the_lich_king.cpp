...
#define HARVEST_SOUL_RM      RAID_MODE<uint32>(68980, 74325, 74296, 74297)
#define HARVESTED_SOUL_BUFF  RAID_MODE<uint32>(72679, 74318, 74319, 74320)
#define SOUL_SHRIEK          RAID_MODE<uint32>(69242, 73800, 73801, 73802)
...
enum Events
{
	...
	EVENT_SET_AMBIENCE_MUSIC,

	EVENT_REMOVE_DEFENSIVE_AFTER_VILE_SPIRITS,
...
void SendPacketToPlayers(WorldPacket* data, Unit* source)
{
...
public:
	...
	struct boss_the_lich_kingAI : public BossAI
	{
        ...
        uint32 _summonedRagingSpiritT2;
		...
		void EnterCombat(Unit* target)
		{
			...
			events.ScheduleEvent(EVENT_INFEST, 5000);
		...
		void DamageTaken(Unit* attacker, uint32& damage)
		{
			...
			if (_phase == PHASE_ONE && !HealthAbovePct(70) && !me->HasUnitState(UNIT_STATE_CASTING))
			{
				...
				events.CancelEvent(EVENT_INFEST);
			...
			if (_phase == PHASE_TWO && !HealthAbovePct(40) && !me->HasUnitState(UNIT_STATE_CASTING))
			{
				...
				events.CancelEvent(EVENT_SUMMON_VALKYR);
				events.CancelEvent(EVENT_INFEST);
				me->InterruptNonMeleeSpells(false);
			...
			if (_phase == PHASE_OUTRO)
			{
				...
				if (!_bFordringMustFallYell && me->GetHealth() < 500000)
				{
					...
					if (Creature* tirion = ObjectAccessor::GetCreature(*me, instance->GetGuidData(DATA_HIGHLORD_TIRION_FORDRING)))
					{
						tirion->Yell("The Lich King must fall!", LANG_UNIVERSAL, 0);
        ...
        void ManageValkyrs()
        {
            ...
            Trinity::AllCreaturesOfEntryInRange checker(me, NPC_VALKYR_SHADOWGUARD, 150.0f);
            ...
            if (!pCreatureList.empty())
            {
                for (std::list<Creature*>::iterator i = pCreatureList.begin(); i != pCreatureList.end(); ++i)
                    (*i)->AI()->DoAction(0); // Despawn if they need to
            }
        }
		...
		void MovementInform(uint32 type, uint32 pointId)
		{
			...
			case POINT_CENTER_1:
				...
				me->PlayDirectSound(MUSIC_SPECIAL);
				me->CastSpell(me, SPELL_REMORSELESS_WINTER_1, false);
				//events.DelayEvents62500, EVENT_GROUP_BERSERK); // delay berserk timer, its not ticking during phase transitions, bullshit, 15mins on movies
			...
			case POINT_CENTER_2:
				...
				//events.DelayEvents62500, EVENT_GROUP_BERSERK); // delay berserk timer, its not ticking during phase transitions, bullshit, 15 mins on movies
		...
		void UpdateAI(uint32 diff)
		{
			...
			case EVENT_START_ATTACK:
				...
				if (_phase == PHASE_FROSTMOURNE)
				{
					_bFrostmournePhase = false;
					_phase = PHASE_THREE;
					events.RescheduleEvent(EVENT_DEFILE, 500);
					events.RescheduleEvent(EVENT_VILE_SPIRITS, urand(7000, 10000));
					events.RescheduleEvent(EVENT_SOUL_REAPER, urand(12000, 14000));
			...
			case EVENT_QUAKE:
				...
				_phase = PHASE_TWO;
				events.CancelEventGroup(EVENT_GROUP_ABILITIES);
				events.ScheduleEvent(EVENT_INFEST, 14000);
				events.ScheduleEvent(EVENT_SUMMON_VALKYR, 18000);
				events.ScheduleEvent(EVENT_SOUL_REAPER, 22000, EVENT_GROUP_ABILITIES);
				events.ScheduleEvent(EVENT_DEFILE, 32500);
				events.ScheduleEvent(EVENT_SET_AMBIENCE_MUSIC, 5000);
			...
			case EVENT_QUAKE_2:
				...
				events.RescheduleEvent(EVENT_VILE_SPIRITS, 18000);
				events.ScheduleEvent(IsHeroic() ? EVENT_HARVEST_SOULS : EVENT_HARVEST_SOUL, 11000);
				events.ScheduleEvent(EVENT_SET_AMBIENCE_MUSIC, 7000);
			...
			case EVENT_INFEST:
				...
				events.ScheduleEvent(EVENT_INFEST, urand(21000, 22000));
				break;
            ...
            {
                if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0, 100.0f, true, -SOUL_SHRIEK))
                    me->CastSpell(target, SPELL_RAGING_SPIRIT, false);

                uint32 timer = 19000;
                if (!HealthAbovePct(40))
                {
                    ++_summonedRagingSpiritT2;
                    if (_summonedRagingSpiritT2 == 3)
                        timer = 10000;
                    else
                        timer = 13000;
                }

                events.ScheduleEvent(EVENT_SUMMON_RAGING_SPIRIT, timer, EVENT_GROUP_ABILITIES);
            }
				break;
			case EVENT_DEFILE:
			{
				...
				events.ScheduleEvent(EVENT_DEFILE, urand(32000, 35000));
				*/
				...
				if (evTime && (events.GetTimer() > evTime || evTime - events.GetTimer() < 5000)) // defile cast 2sec -> valkyr in less than 3 secs after defile appears
				{
					if (events.GetTimer() > evTime || evTime - events.GetTimer() < 3500) // valkyr is less than 1.5 secs after defile - reschedule defile
					{
						...
						events.ScheduleEvent(EVENT_DEFILE, t + (Is25ManRaid() ? 5000 : 4000));
						break;
					}
					// delay valkyr just a bit
					events.RescheduleEvent(EVENT_SUMMON_VALKYR, 5000);
				}
				if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0, DefileTargetSelector(me)))
				{
					Talk(EMOTE_DEFILE_WARNING);
					me->CastSpell(target, SPELL_DEFILE, false);
					events.ScheduleEvent(EVENT_DEFILE, urand(31000, 34000));
				}
				else
					events.ScheduleEvent(EVENT_DEFILE, 2000);
			}
			break;
			case EVENT_SOUL_REAPER:
				if (me->IsWithinMeleeRange(me->GetVictim()))
				{
					...
					events.DelayEvents(12000, EVENT_GROUP_ABILITIES);
			...
			case EVENT_SUMMON_VALKYR:
			{
				...
				events.ScheduleEvent(EVENT_SUMMON_VALKYR, urand(45000, 50000));*/


					me->PlayDirectSound(MUSIC_SPECIAL);
					Talk(SAY_LK_SUMMON_VALKYR);
					DoCastAOE(SUMMON_VALKYR, true);
					events.ScheduleEvent(EVENT_SUMMON_VALKYR, urand(45000, 48000));
					...
					if (uint32 evTime = events.GetNextEventTime(EVENT_DEFILE))
						if (events.GetTimer() > evTime || evTime - events.GetTimer() < minTime)
							events.RescheduleEvent(EVENT_DEFILE, minTime);
			...
			case EVENT_VILE_SPIRITS:
				...
				events.ScheduleEvent(EVENT_VILE_SPIRITS, 30000);
				events.ScheduleEvent(EVENT_REMOVE_DEFENSIVE_AFTER_VILE_SPIRITS, 5000);
				// 5 segundos en de
				break;
			case EVENT_REMOVE_DEFENSIVE_AFTER_VILE_SPIRITS:
				me->SetReactState(REACT_AGGRESSIVE);
				break;
			...
			case EVENT_HARVEST_SOULS:
				...
				events.ScheduleEvent(EVENT_FROSTMOURNE_HEROIC, 6000);
				events.ScheduleEvent(EVENT_HARVEST_SOULS, urand(100000, 110000));
...
public:
	...
	struct npc_tirion_fordringAI : public ScriptedAI
	{
		...
		{
			_events.Reset();
			if (_instance->GetBossState(DATA_THE_LICH_KING) == DONE || me->GetMap()->IsHeroic() && !_instance->GetData(DATA_HEROIC_ATTEMPTS))
		...
		void sGossipSelect(Player* /*player*/, uint32 sender, uint32 action)
		{
			...
			{
				if (me->GetMap()->IsHeroic() && !_instance->GetData(DATA_HEROIC_ATTEMPTS))
					return;
				//bandera - Comprobar Sindragosa, Putricidio y Blood queen.
				if (me->GetMap()->IsHeroic())
					if (_instance->GetData(DATA_IS_LK_HEROIC_ENGAGEABLE) != 3)
						return;
		...
		void UpdateAI(uint32 diff)
		{
			...
			case EVENT_OUTRO_SOUL_BARRAGE:
				if (Creature* theLichKing = ObjectAccessor::GetCreature(*me, _instance->GetGuidData(DATA_THE_LICH_KING)))
				{
					...
					sCreatureTextMgr->SendSound(theLichKing, SOUND_PAIN, CHAT_MSG_MONSTER_YELL, 0, TEXT_RANGE_NORMAL, TEAM_OTHER, false);
					...
					theLichKing->SetInhabitType(INHABIT_GROUND | INHABIT_AIR);
					theLichKing->SetCanFly(true);
					theLichKing->SetDisableGravity(true);
...
public:
	...
	class spell_the_lich_king_infest_AuraScript : public AuraScript
	{
		...
		void OnUpdate(AuraEffect* aurEff)
		{
			...
			aurEff->SetAmount(int32(aurEff->GetAmount() * 1.25f));
		}
...
public:
	...
	struct npc_icc_ice_sphereAI : public ScriptedAI
	{
		...
		void SelectNewTarget()
		{
			...
			if (Unit* p = SelectTarget(SELECT_TARGET_RANDOM, 0, 120.0f, true))
			{
...
public:
	...
	{
		npc_valkyr_shadowguardAI(Creature* creature) : ScriptedAI(creature), _grabbedPlayer(ObjectGuid::Empty), didbelow50pct(false), dropped(false), _instance(creature->GetInstanceScript())
		{
			_events.Reset();
		...
		void GoSiphon()
		{
            if (Creature* theLichKing = ObjectAccessor::GetCreature(*me, _instance->GetGuidData(DATA_THE_LICH_KING)))
            {
                if (theLichKing->AI()->GetData(0) == PHASE_TRANSITION)
                {
                    me->DespawnOrUnsummon(1000);
                    return;
                }                    
		...
		void UpdateAI(uint32 diff)
		{
			...
			case EVENT_MOVE_TO_DROP_POS:
				...
				{
					me->SetSpeed(MOVE_FLIGHT, 0.29f);
					me->SetSpeed(MOVE_WALK, 0.29f);
					me->SetSpeed(MOVE_RUN, 0.29f);
				}
			...
			case EVENT_INCREASE_SPEED:
				if (!me->HasAuraType(SPELL_AURA_MOD_DECREASE_SPEED) && !me->HasAura(SPELL_DK_DESECRATION) && !me->HasAura(SPELL_HUNTER_FROST_TRAP))
				{
					...
					me->SetSpeed(MOVE_WALK, 0.28f);
					me->SetSpeed(MOVE_RUN, 0.28f);
				}
				else
					_events.RescheduleEvent(EVENT_INCREASE_SPEED, 100);
				break;
...
public:
	...
	class spell_the_lich_king_vile_spirit_move_target_search_SpellScript : public SpellScript
	{
		...
		void HandleScript(SpellEffIndex effIndex)
		{
			...
			GetCaster()->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PACIFIED | UNIT_FLAG_IMMUNE_TO_PC | UNIT_FLAG_IMMUNE_TO_NPC);
			GetCaster()->ForceValuesUpdateAtIndex(UNIT_FIELD_FLAGS);
	...
	class spell_the_lich_king_harvest_soul_AuraScript : public AuraScript
	{
		...
		void OnRemove(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
		{
			...
			if (GetTargetApplication()->GetRemoveMode() == AURA_REMOVE_BY_DEATH)
				GetTarget()->CastSpell((Unit*)NULL, SPELL_HARVESTED_SOUL_LK_BUFF, true, NULL, NULL, GetTarget()->GetInstanceScript()->GetGuidData(DATA_THE_LICH_KING));
		}
		...
		void DoAction(int32 action)
		{
			...
			if (TempSummon* summ = me->ToTempSummon())
			{
				if (Unit* summoner = summ->GetSummoner())
				{
					...
					{
						if (buff)
							summoner->CastSpell((Unit*)NULL, SPELL_HARVESTED_SOUL_LK_BUFF, true, NULL, NULL, _instance->GetGuidData(DATA_THE_LICH_KING));

						me->CastSpell(summoner, SPELL_HARVEST_SOUL_TELEPORT_BACK, false);
					}
					else if (buff)
						me->CastSpell((Unit*)NULL, SPELL_HARVESTED_SOUL_LK_BUFF, true, NULL, NULL, _instance->GetGuidData(DATA_THE_LICH_KING));
					...
					if (Unit* summoner = summ->GetSummoner())
					{
						if (summoner->IsAlive() && summoner->GetTypeId() == TYPEID_PLAYER)
						{
							...
							else
							{
								...
								summoner->RemoveAurasDueToSpell(HARVEST_SOUL_RM, ObjectGuid::Empty, 0, AURA_REMOVE_BY_EXPIRE);
							}
						}
						else
						{
