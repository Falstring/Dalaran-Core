...
struct gunship_npc_AI : public ScriptedAI
{
    ...
    virtual Unit* SelectTargetOnOtherShip()
    {
        return 0LL;
    }
    ...
    bool SelectVictim()
    {
        ...
        if (!me->HasReactState(REACT_PASSIVE))
        {
            if (Unit* victim = me->SelectVictim())
                AttackStart(victim);
            else if (Unit* victim = SelectTargetOnOtherShip())
                AttackStart(victim);
                ...
                if (isVictory)
                {
                    ...
                    if (Map* map = me->GetMap())
                    {
                        ...
                        for (Map::PlayerList::const_iterator i = PlayerList.begin(); i != PlayerList.end(); ++i)
                        {
                            ...
                            {
                                player->RemoveAura(SPELL_ROCKET_PACK);
    ...
    public:
        ...
        struct npc_high_overlord_saurfang_igbAI : public ScriptedAI
        {
            ...
            void sGossipSelect(Player* /*player*/, uint32 /*menuId*/, uint32 /*gossipListId*/) override
            {
                ...
                _events.RescheduleEvent(EVENT_INTRO_H_1, 5000, 0, PHASE_INTRO);
                _events.RescheduleEvent(EVENT_INTRO_H_2, 16000, 0, PHASE_INTRO);
                _events.RescheduleEvent(EVENT_INTRO_SUMMON_SKYBREAKER, 24600, 0, PHASE_INTRO);
                _events.RescheduleEvent(EVENT_INTRO_H_3, 29600, 0, PHASE_INTRO);
                _events.RescheduleEvent(EVENT_INTRO_H_4, 39200, 0, PHASE_INTRO);
                const CreatureTemplate* cinfo = me->GetCreatureTemplate();
                CreatureBaseStats const* stats = sObjectMgr->GetCreatureBaseStats(me->getLevel(), cinfo->unit_class);
                me->SetBaseWeaponDamage(BASE_ATTACK, MINDAMAGE, (((stats->BaseDamage[cinfo->expansion] + (stats->AttackPower / 14)) * cinfo->ModDamage) * (cinfo->BaseAttackTime / 1000)) * 0.01f);
                me->SetBaseWeaponDamage(BASE_ATTACK, MAXDAMAGE, ((((stats->BaseDamage[cinfo->expansion] * 1.5) + (stats->AttackPower / 14)) * cinfo->ModDamage) * (cinfo->BaseAttackTime / 1000)) * 0.01f);
                isOwnBoss = true;
            }

            void DamageTaken(Unit* , uint32& damage) override
            {
                if (!isOwnBoss && me->HealthBelowPctDamaged(65, damage) && !me->HasAura(SPELL_TASTE_OF_BLOOD))
    ...
    public:
        ...
        struct npc_muradin_bronzebeard_igbAI : public ScriptedAI
        {
            ...
            void sGossipSelect(Player* /*player*/, uint32 /*menuId*/, uint32 /*gossipListId*/) override
            {
                ...
                me->GetTransport()->EnableMovement(true);
                _events.SetPhase(PHASE_INTRO);
                _events.RescheduleEvent(EVENT_INTRO_A_1, 5000);
                _events.RescheduleEvent(EVENT_INTRO_A_2, 10000, 0, PHASE_INTRO);
                _events.RescheduleEvent(EVENT_INTRO_SUMMON_ORGRIMS_HAMMER, 28000, 0, PHASE_INTRO);
                _events.RescheduleEvent(EVENT_INTRO_A_3, 33000, 0, PHASE_INTRO);
                _events.RescheduleEvent(EVENT_INTRO_A_4, 39000, 0, PHASE_INTRO);
                _events.RescheduleEvent(EVENT_INTRO_A_5, 45000, 0, PHASE_INTRO);
                const CreatureTemplate* cinfo = me->GetCreatureTemplate();
                CreatureBaseStats const* stats = sObjectMgr->GetCreatureBaseStats(me->getLevel(), cinfo->unit_class);
                me->SetBaseWeaponDamage(BASE_ATTACK, MINDAMAGE, (((stats->BaseDamage[cinfo->expansion] + (stats->AttackPower / 14)) * cinfo->ModDamage) * (cinfo->BaseAttackTime / 1000)) * 0.01f);
                me->SetBaseWeaponDamage(BASE_ATTACK, MAXDAMAGE, ((((stats->BaseDamage[cinfo->expansion] * 1.5) + (stats->AttackPower / 14)) * cinfo->ModDamage) * (cinfo->BaseAttackTime / 1000)) * 0.01f);
...
class npc_gunship_gunner : public CreatureScript
{
    public:
        ...
        struct npc_gunship_gunnerAI : public gunship_npc_AI
        {
            ...
            Unit* SelectTargetOnOtherShip()
            {
                std::list<Unit*>PlayerOnOtherShip;
                Map::PlayerList const &players = me->GetMap()->GetPlayers();
                for (Map::PlayerList::const_iterator itr = players.begin(); itr != players.end(); ++itr)
                    if (Player* player = itr->GetSource()->ToPlayer())
                        if (player->HasAura(Instance->GetData(DATA_TEAM_IN_INSTANCE) == HORDE ? SPELL_ON_ORGRIMS_HAMMER_DECK : SPELL_ON_SKYBREAKER_DECK))
                            PlayerOnOtherShip.push_back(player);

                if (PlayerOnOtherShip.empty())
                    return 0LL;
                return Trinity::Containers::SelectRandomContainerElement(PlayerOnOtherShip);
            }
    ...
    public:
        ...
        struct npc_gunship_rocketeerAI : public gunship_npc_AI
        {
            ...
            void UpdateAI(uint32 /*diff*/) override
            {
                ...
                me->GetSpellHistory()->AddCooldown(spellId, 0, std::chrono::seconds(1));
            }
        };
...
public:
	...
	struct npc_gunship_mageAI : public gunship_npc_AI
	{
		npc_gunship_mageAI(Creature* creature) : gunship_npc_AI(creature)
		{
            ...
            me->ApplySpellImmune(0, IMMUNITY_ID, 57994, true); // Wind Shear
		}
...
class spell_igb_on_gunship_deck : public SpellScriptLoader
{
    public:
        ...
        private:
            ...
            void HandleApply(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
            {
                bool enemy = GetSpellInfo()->Id == uint32(_teamInInstance == TEAM_HORDE ? SPELL_ON_SKYBREAKER_DECK : SPELL_ON_ORGRIMS_HAMMER_DECK);
                if (Creature* gunship = GetOwner()->FindNearestCreature(_teamInInstance == TEAM_HORDE ? NPC_ORGRIMS_HAMMER : NPC_THE_SKYBREAKER, 200.0f))
    ...
    public:
        ...
        class spell_igb_cannon_blast_SpellScript : public SpellScript
        {
            ...
            void CheckEnergy()
            {
                ...
                {
					GetCaster()->AddAura(SPELL_OVERHEAT, GetCaster());
