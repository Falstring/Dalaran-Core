...
enum Events
{
	...
	EVENT_STICKY_OOZE,
	EVENT_SLIME_SPRAY_START,
	EVENT_SLIME_SPRAY_END,
    EVENT_ENRAGE,
...
public:
	...
	struct boss_rotfaceAI : public BossAI
	{
		...
		void EnterCombat(Unit* who)
		{
			...
			// schedule events
			events.Reset();
			events.ScheduleEvent(EVENT_SLIME_SPRAY_START, 16000);
		...
		void UpdateAI(uint32 diff)
		{
			...
			case EVENT_UNROOT:
				...
				break;
			case EVENT_SLIME_SPRAY_START:
				me->SetReactState(REACT_PASSIVE);
				me->AttackStop();
				me->CastStop();
				events.ScheduleEvent(EVENT_SLIME_SPRAY, 0.1 * IN_MILLISECONDS);
				me->ApplySpellImmune(0, IMMUNITY_EFFECT, SPELL_EFFECT_ATTACK_ME, true);
				me->ApplySpellImmune(0, IMMUNITY_STATE, SPELL_AURA_MOD_TAUNT, true);
				events.ScheduleEvent(EVENT_SLIME_SPRAY_END, 7 * IN_MILLISECONDS);
			...
			case EVENT_SLIME_SPRAY:
				...
				{
					if (Creature* stalker = me->SummonCreature(NPC_OOZE_SPRAY_STALKER, target->GetPositionX(), target->GetPositionY(), target->GetPositionZ(), target->GetOrientation(), TEMPSUMMON_TIMED_DESPAWN, 9 * IN_MILLISECONDS))
					{
				...
				events.ScheduleEvent(EVENT_UNROOT, 1);
				break;
