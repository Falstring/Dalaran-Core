...
public:
	...
	struct boss_valithria_dreamwalkerAI : public ScriptedAI
	{
		...
		void UpdateAI(uint32 diff)
		{
			...
			{
				if (me->GetHealth() != _spawnHealth) // healing when boss cannot be engaged (lower spire not finxed, cheating) doesn't start the fight, prevent winning this way
					me->SetHealth(_spawnHealth);
				return;
			}
				...
				if (!threatList.empty())
					for (ThreatContainer::StorageType::const_iterator itr = threatList.begin(); itr != threatList.end(); ++itr)
						if (Unit* target = (*itr)->getTarget())
							if (target->IsAlive() && target->GetTypeId() == TYPEID_PLAYER && me->GetExactDist(target) < 200.0f && !target->IsImmunedToDamage(SpellSchoolMask(SPELL_SCHOOL_MASK_ALL)))
								return;
				EnterEvadeMode();
			}
			else
	...
	struct npc_the_lich_king_controllerAI : public ScriptedAI
	{
		...
		void DoAction(int32 action)
		{
			if (action == ACTION_ENTER_COMBAT)
			{
				...
				_events.Reset();
				_events.ScheduleEvent(EVENT_GLUTTONOUS_ABOMINATION_SUMMONER, 22000);
				_events.ScheduleEvent(EVENT_SUPPRESSER_SUMMONER, 30000);
				_events.ScheduleEvent(EVENT_BLISTERING_ZOMBIE_SUMMONER, 35000);
		...
		void UpdateAI(uint32 diff)
		{
			...
			case EVENT_GLUTTONOUS_ABOMINATION_SUMMONER:
				...
				break;
			case EVENT_SUPPRESSER_SUMMONER: // BANDERA
			{
				me->CastSpell(me, SPELL_TIMER_SUPPRESSER, false);

				std::list<Creature*> summoners;
				Trinity::AllCreaturesOfEntryInRange checker(me, NPC_WORLD_TRIGGER, 200.0f);
				Trinity::CreatureListSearcher<Trinity::AllCreaturesOfEntryInRange> searcher(me, summoners, checker);
				...
				for (uint32 i = 0; i < 3; ++i)
					me->CastSpell(summoners.front(), SPELL_SUMMON_SUPPRESSER, true);
				for (uint32 i = 0; i < 3; ++i)
					me->CastSpell(summoners.back(), SPELL_SUMMON_SUPPRESSER, true);
...
public:
	...
	struct npc_blazing_skeletonAI : public ScriptedAI
	{
		...
		{
			_events.Reset();
			_events.ScheduleEvent(EVENT_LEY_WASTE, urand(9000, 12000));
		}
		...
		void UpdateAI(uint32 diff)
		{
			...
			case EVENT_LEY_WASTE:
				...
				_events.ScheduleEvent(EVENT_LEY_WASTE, urand(12000, 18000));
				break;
			default:
				break;
			}
...
public:
	...
	struct npc_blistering_zombieAI : public ScriptedAI
	{
		npc_blistering_zombieAI(Creature* creature) : ScriptedAI(creature)
		{
			timer = 0;
			casted = false;
			me->CastSpell(me, SPELL_CORROSION_AURA);
		}

		uint16 timer;
		bool casted;

		void DamageTaken(Unit*, uint32 &dmg)
		{
			if (dmg >= me->GetHealth())
			{
				dmg = me->GetHealth() - 1;
...
public:
	...
	class spell_dreamwalker_twisted_nightmares_SpellScript : public SpellScript
	{
		...
		void HandleScript(SpellEffIndex effIndex)
		{
			...
			// Trigguea 71940, que es el damage + 10% extra de damage
			if (InstanceScript* instance = GetHitUnit()->GetInstanceScript())
				GetHitUnit()->CastSpell((Unit*)NULL, GetSpellInfo()->Effects[effIndex].TriggerSpell, true, NULL, NULL);
		}
...
public:
	...
	class spell_dreamwalker_decay_periodic_timer_AuraScript : public AuraScript
	{
		...
		{
			int32 timer = aurEff->GetAmplitude();
			if (timer <= 6000)
				return;

			aurEff->SetAmplitude(timer - _decayRate);
		}
...
public:
	...
	class spell_dreamwalker_summon_suppresser_AuraScript : public AuraScript
	{
		...
		void PeriodicTick(AuraEffect const* /*aurEff*/)
		{
			...
			std::list<Creature*> summoners;
			Trinity::AllCreaturesOfEntryInRange checker(caster, NPC_WORLD_TRIGGER, 200.0f);
			Trinity::CreatureListSearcher<Trinity::AllCreaturesOfEntryInRange> searcher(caster, summoners, checker);

			caster->VisitNearbyObject(200.0f, searcher);
			std::list<Creature*> list_copy = summoners;
			summoners.remove_if(Trinity::UnitAuraCheck(true, SPELL_RECENTLY_SPAWNED));
			if (summoners.empty())
			{
...
class spell_valithria_twisted_nightmare_damage : public SpellScriptLoader
{
public:
	spell_valithria_twisted_nightmare_damage() : SpellScriptLoader("spell_valithria_twisted_nightmare_damage") { }

	class spell_valithria_twisted_nightmare_damage_AuraScript : public AuraScript
	{
		PrepareAuraScript(spell_valithria_twisted_nightmare_damage_AuraScript);
		...
		{
			aurEff->SetAmount(GetStackAmount()*200);
			aurEff->SetBonusAmount(0);
			aurEff->SetCanBeRecalculated(false);
		}

		void Register() override
		{
			OnEffectPeriodic += AuraEffectPeriodicFn(spell_valithria_twisted_nightmare_damage_AuraScript::OnPeriodic, EFFECT_0, SPELL_AURA_PERIODIC_DAMAGE);
			OnEffectUpdatePeriodic += AuraEffectUpdatePeriodicFn(spell_valithria_twisted_nightmare_damage_AuraScript::OnUpdate, EFFECT_0, SPELL_AURA_PERIODIC_DAMAGE);
		}
	};

	AuraScript* GetAuraScript() const override
	{
		return new spell_valithria_twisted_nightmare_damage_AuraScript();
	}
};
...
void AddSC_boss_valithria_dreamwalker()
{
	...
	new spell_valithria_twisted_nightmare_damage();
