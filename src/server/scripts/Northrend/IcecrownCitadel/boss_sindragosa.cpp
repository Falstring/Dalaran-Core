...
enum ClassSpells
{
	...
	SPELL_HAMMER_OFTHE_RIGHTEOUS  = 53595,
	...
	// Priest
	SPELL_PENANCE                 = 47540,
	SPELL_CIRCLE_OF_HELING        = 34861,

	// Druid
	SPELL_FERAL_BERSERK           = 50334,
	SPELL_TREE_OF_LIFE            = 65139
};
...
public:
	...
	bool operator()(Unit const* target) const
	{
		...
		if (target->IsImmunedToDamage(SpellSchoolMask(SPELL_SCHOOL_MASK_ALL)) || target->HasAura(SPELL_ICE_TOMB_UNTARGETABLE) || target->HasAura(SPELL_ICE_TOMB_DAMAGE) || target->HasAura(SPELL_TANK_MARKER_AURA) || target->HasAuraType(SPELL_AURA_SPIRIT_OF_REDEMPTION))
			return false;
...
public:
	...
	struct boss_sindragosaAI : public BossAI
	{
		...
		void Reset()
		{
			...
			if (!me->HasUnitState(UNIT_STATE_POSSESSED))
			...
			if (!me->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE)) // this flag is removed after she lands and can be engaged
			{
				...
				for (Map::PlayerList::const_iterator itr = pl.begin(); itr != pl.end(); ++itr)
					if (Player* p = itr->GetSource())
						if (p->IsAlive() && !p->IsGameMaster() && p->GetExactDist(&SindragosaLandPos) < 200.0f && !p->IsImmunedToDamage(SpellSchoolMask(SPELL_SCHOOL_MASK_ALL)))
							me->Kill(p);
			}
			//me->DisableRotate(false);
			if (!me->HasUnitState(UNIT_STATE_POSSESSED))
		...
		void MovementInform(uint32 type, uint32 point)
		{
			...
			case POINT_AIR_PHASE:
				//Quiza aqui un reschedule a 3 segundos mas para el castcustompsoell
				...
				//events.ScheduleEvent(EVENT_CAST_FROST_BEACONS, 3000); // In order to delay the beacon target cast a bit.
		...
		void UpdateAI(uint32 diff)
		{
			...
			case EVENT_TAIL_SMASH:
				...
				events.ScheduleEvent(EVENT_TAIL_SMASH, 10000, EVENT_GROUP_LAND_PHASE); // urand(22000, 27000)
				break;
			...
			case EVENT_ICY_GRIP:
				...
				if (uint32 evTime = events.GetNextEventTime(EVENT_FROST_BREATH))
					if (events.GetTimer() > evTime || evTime - events.GetTimer() < 7000)
						events.RescheduleEvent(EVENT_FROST_BREATH, 9000);
				break;
...
public:
	...
	bool operator()(WorldObject* object) const
	{
		if (Player* p = object->ToPlayer())
		{
			...
			if (p->getClass() == CLASS_PALADIN && (p->HasSpell(SPELL_HAMMER_OFTHE_RIGHTEOUS) || p->HasSpell(SPELL_DIVINE_STORM)) || p->getClass() == CLASS_SHAMAN && p->HasSpell(SPELL_FERAL_SPIRIT) || p->getClass() == CLASS_DRUID && p->HasSpell(SPELL_FERAL_BERSERK))
				return true;

			if (_removeHealers == (p->getClass() == CLASS_DRUID && p->HasSpell(SPELL_TREE_OF_LIFE) || p->getClass() == CLASS_PALADIN && p->HasSpell(SPELL_BEACON_OF_LIGHT) || p->getClass() == CLASS_PRIEST && (p->HasSpell(SPELL_PENANCE) || p->HasSpell(SPELL_CIRCLE_OF_HELING)) || p->getClass() == CLASS_SHAMAN && p->HasSpell(SPELL_RIPTIDE)))
				return true;
...
public:
	...
	class spell_sindragosa_unchained_magic_AuraScript : public AuraScript
	{
		...
		bool CheckProc(ProcEventInfo& eventInfo)
		{
			...
			if (eventInfo.GetDamageInfo()->GetSpellInfo()->AttributesEx & SPELL_ATTR1_CHANNELED_1)
				return false;
			if (eventInfo.GetDamageInfo()->GetSpellInfo()->AttributesEx3 & SPELL_ATTR3_TRIGGERED_CAN_TRIGGER_PROC_2)
				return false;
			...
			switch (eventInfo.GetDamageInfo()->GetSpellInfo()->SpellIconID)
			{
...
public:
	...
	{
		return unit->IsImmunedToDamage(SpellSchoolMask(SPELL_SCHOOL_MASK_ALL));
	}

	bool operator()(WorldObject* object) const
	{
		return object->ToUnit() && object->ToUnit()->IsImmunedToDamage(SpellSchoolMask(SPELL_SCHOOL_MASK_ALL));
	}
};
...
public:
	...
	bool operator()(WorldObject* unit) const
	{
		...
		return !dTree.isInLineOfSight(unit->GetPositionX(), unit->GetPositionY(), unit->GetPositionZ(), ox, oy, oz, unit->GetPhaseMask());
	}
	...
	struct npc_rimefangAI : public ScriptedAI
	{
		...
		void UpdateAI(uint32 diff)
		{
			...
			case EVENT_ICY_BLAST:
			{
				...
				me->GetMotionMaster()->MoveTakeoff(0, { me->GetPositionX(), me->GetPositionY(), destZ });
			...
			case EVENT_ICY_BLAST_CAST:
				...
				else
				{
					...
					me->SendMeleeAttackStop(me->GetVictim());
					me->GetMotionMaster()->MoveLand(POINT_LAND_GROUND, { me->GetPositionX(), me->GetPositionY(), destZ });
				}
				break;
			default:
				break;
