...
enum Spells
{
	...
	SPELL_INOCULATED_TWO = 72101,
	SPELL_INOCULATED_THREE = 72102,
	SPELL_INOCULATED_FOUR = 72103,
...
public:
	...
	struct boss_festergutAI : public BossAI
	{
		...
		void EnterCombat(Unit* who)
		{
			...
			events.ScheduleEvent(EVENT_GASTRIC_BLOAT, urand(10000, 12000));
			if (IsHeroic())
			...
			if (who)
				me->CastSpell(who, SPELL_GASTRIC_BLOAT, false);
	...
	class spell_festergut_blighted_spores_AuraScript : public AuraScript
	{
		...
		void OnRemove(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
		{
			...
			if (GetTarget()->HasAura(SPELL_INOCULATED))
				if (GetTarget()->GetAura(SPELL_INOCULATED)->GetDuration() > 112 * IN_MILLISECONDS)
					return;
			if (GetTarget()->HasAura(SPELL_INOCULATED_TWO))
				if (GetTarget()->GetAura(SPELL_INOCULATED_TWO)->GetDuration() > 112 * IN_MILLISECONDS)
					return;
			if (GetTarget()->HasAura(SPELL_INOCULATED_THREE))
				if (GetTarget()->GetAura(SPELL_INOCULATED_THREE)->GetDuration() > 112 * IN_MILLISECONDS)
					return;
			if (GetTarget()->HasAura(SPELL_INOCULATED_FOUR))
				if (GetTarget()->GetAura(SPELL_INOCULATED_FOUR)->GetDuration() > 112 * IN_MILLISECONDS)
					return;
			GetTarget()->CastSpell(GetTarget(), SPELL_INOCULATED, true);
			if (GetTarget()->HasAura(SPELL_INOCULATED))
...
public:
	...
	struct npc_stinky_iccAI : public ScriptedAI
	{
		...
		void EnterCombat(Unit* /*target*/)
		{
			...
			events.ScheduleEvent(EVENT_DECIMATE, urand(30000, 35000));
