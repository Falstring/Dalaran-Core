...
public:
	...
	struct boss_professor_putricideAI : public BossAI
	{
		...
		void EnterCombat(Unit* who)
		{
			...
			events.ScheduleEvent(EVENT_UNSTABLE_EXPERIMENT, urand(24000, 26000), EVENT_GROUP_ABILITIES);
			if (IsHeroic())
		...
		void UpdateAI(uint32 diff)
		{
			...
			case EVENT_UNBOUND_PLAGUE:
				if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0, NonTankTargetSelector(me)))
				{
					me->CastStop();
					me->StopMoving();
					//Quiza es que no devuelve targets el selector.
					me->CastSpell(target, SPELL_UNBOUND_PLAGUE, true);
...
public:
	...
	class spell_putricide_ooze_channel_SpellScript : public SpellScript
	{
		...
		void SelectTarget(std::list<WorldObject*>& targets)
		{
			...
			// SPELL_GASEUS_BLOAT and SPELL_ADHESIVE_OOZE
			targets.remove_if(Trinity::UnitAuraCheck(true, 70672));
			targets.remove_if(Trinity::UnitAuraCheck(true, 72455));
			targets.remove_if(Trinity::UnitAuraCheck(true, 72832));
			targets.remove_if(Trinity::UnitAuraCheck(true, 72833));

			targets.remove_if(Trinity::UnitAuraCheck(true, 70447));
			targets.remove_if(Trinity::UnitAuraCheck(true, 72836));
			targets.remove_if(Trinity::UnitAuraCheck(true, 72837));
			targets.remove_if(Trinity::UnitAuraCheck(true, 72838));
...
public:
	...
	class spell_putricide_mutated_plague_AuraScript : public AuraScript
	{
		...
		void HandleTriggerSpell(AuraEffect const* aurEff)
		{
			...
			GetTarget()->CastCustomSpell(triggerSpell, SPELLVALUE_BASE_POINT0, damage*1.6, GetTarget(), true, NULL, aurEff, GetCasterGUID());
		}
