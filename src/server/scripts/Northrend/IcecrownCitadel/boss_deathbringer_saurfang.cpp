...
public:
    ...
    struct boss_deathbringer_saurfangAI : public BossAI
    {
        boss_deathbringer_saurfangAI(Creature* creature) : BossAI(creature, DATA_DEATHBRINGER_SAURFANG)
        {
            ...
            //TeamInInstance = instance->GetData(78);
        }
        ...
        void EnterCombat(Unit* who)
        {
            ...
            events.ScheduleEvent(EVENT_CHECK_MARKS, 30 * IN_MILLISECONDS);
            _fallenChampionCastCount = 0;
            instance->DoRemoveAurasDueToSpellOnPlayers(SPELL_MARK_OF_THE_FALLEN_CHAMPION);
        ...
        void DamageTaken(Unit*, uint32& damage)
        {
            ...
            if (!_dead && (damage >= me->GetHealth()))
            {
                ...
                DoCast(me, SPELL_ACHIEVEMENT, true);
                //me->Kill(me);
                //DoRemoveAuraOnAllPlayerSaurfang(instance, SPELL_MARK_OF_THE_FALLEN_CHAMPION);
                DoCast(me, SPELL_PERMANENT_FEIGN_DEATH);
                if (Creature* creature = me->FindNearestCreature(TeamInInstance == ALLIANCE ? NPC_SE_MURADIN_BRONZEBEARD : NPC_SE_HIGH_OVERLORD_SAURFANG, 100.0f, true))
                ...
                if (Creature* dummy = me->SummonCreature(201333, me->GetPosition())) // Summon dummy
                {
                    dummy->AI()->DoCast(dummy, SPELL_PERMANENT_FEIGN_DEATH);
                    dummy->setFaction(35);
                }
                me->Kill(me);
                /*if (instance->GetData(DATA_SAURFANG_DONE) == 0)
                    instance->SetData(DATA_SAURFANG_DONE, 1);*/
        ...
        void UpdateAI(uint32 diff)
        {
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_BOILING_BLOOD:
                    ...
                    break;
                case EVENT_CHECK_MARKS:
                    if (Map* map = me->GetMap())
                    {
                        uint32 actualMarks = 0;
                        if (map && map->IsDungeon())
                        {
                            ...
                            if (actualMarks < _fallenChampionCastCount)
                                DoAction(ACTION_MARK_OF_THE_FALLEN_CHAMPION);                            
                        }
                    }
                    events.RescheduleEvent(EVENT_CHECK_MARKS, 5 * IN_MILLISECONDS);
                    break;
                case EVENT_BERSERK:
        ...
        void DoAction(int32 action)
        {
            ...
            case ACTION_MARK_OF_THE_FALLEN_CHAMPION:
                if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 1, 0.0f, true, -SPELL_MARK_OF_THE_FALLEN_CHAMPION))
                {
                    ...
                    me->CastSpell(target, SPELL_MARK_OF_THE_FALLEN_CHAMPION, true);
                    me->SetPower(POWER_ENERGY, 0);
...
public:
    ...
    struct npc_high_overlord_saurfangAI : public ScriptedAI
    {
        ...
        void DoAction(int32 action)
        {
            ...
            {
                if (Creature* eventTrigger = me->FindNearestCreature(NPC_MARTYR_STALKER_IGB_SAURFANG, 100.0f))
                    eventTrigger->AI()->DoAction(ACTION_START_HORDE_EVENT);

                me->RemoveAurasDueToSpell(SPELL_GRIP_OF_AGONY);
                Talk(SAY_OUTRO_HORDE_1);
                _events.ScheduleEvent(EVENT_OUTRO_HORDE_2, 2 * IN_MILLISECONDS);   // say                
                me->SetInhabitType(INHABIT_GROUND);
                me->SetDisableGravity(false);
                me->SetCanFly(false);
            ...
            case ACTION_EVADE:
            {
                ...
                for (std::list<Creature*>::iterator itr = _guardList.begin(); itr != _guardList.end(); ++itr)
                {
                    ...
                    (*itr)->StopMoving();
                    (*itr)->SetInhabitType(INHABIT_GROUND);
        ...
        void SpellHit(Unit* /*caster*/, SpellInfo const* spell)
        {
            ...
            {
                me->SetInhabitType(INHABIT_GROUND | INHABIT_AIR);
                me->SetDisableGravity(true);
                me->GetMotionMaster()->MoveTakeoff(POINT_CHOKE, chokePos[0]);
            }
        }
        ...
        void MovementInform(uint32 type, uint32 id)
        {
            if (type == POINT_MOTION_TYPE)
            {
                ...
                case POINT_CORPSE:
                    ...
                    _events.ScheduleEvent(EVENT_OUTRO_HORDE_3, 2 * IN_MILLISECONDS);    // move
                    _events.ScheduleEvent(EVENT_OUTRO_HORDE_5, 4 * IN_MILLISECONDS);    // move
                    break;
                case POINT_FINAL:
        ...
        void UpdateAI(uint32 diff)
        {
            ...
            case EVENT_OUTRO_HORDE_2:   // say
                ...
                _events.ScheduleEvent(EVENT_OUTRO_HORDE_4, 2 * IN_MILLISECONDS);   // move
                break;
    ...
    bool OnGossipHello(Player* player, Creature* creature)
    {
        ...
        if (instance && instance->GetBossState(DATA_DEATHBRINGER_SAURFANG) != DONE && instance->GetBossState(DATA_DEATHBRINGER_SAURFANG) != IN_PROGRESS && creature->GetPositionZ() >= 520.711f)
        {
...
public:
    ...
    struct npc_muradin_bronzebeard_iccAI : public ScriptedAI
    {
        ...
        void DoAction(int32 action)
        {
            ...
            {
                if (Creature* eventTrigger = me->FindNearestCreature(NPC_MARTYR_STALKER_IGB_SAURFANG, 100.0f))
                    eventTrigger->AI()->DoAction(ACTION_START_ALLIANCE_EVENT);
                //Not working properly for unknown reason
                //if (Creature* eventTrigger = ObjectAccessor::GetCreature(*me, me->GetInstanceScript()->GetData64(NPC_MARTYR_STALKER_IGB_SAURFANG)))
                //    eventTrigger->AI()->DoAction(ACTION_START_ALLIANCE_EVENT);
                me->RemoveAurasDueToSpell(SPELL_GRIP_OF_AGONY);
    ...
    bool OnGossipHello(Player* player, Creature* creature)
    {
        ...
        if (instance && instance->GetBossState(DATA_DEATHBRINGER_SAURFANG) != DONE && instance->GetBossState(DATA_DEATHBRINGER_SAURFANG) != IN_PROGRESS && creature->GetPositionZ() >= 520.711f)
        {
            player->ADD_GOSSIP_ITEM(0, "We're ready, Muradin.", 631, -ACTION_START_EVENT + 1);
            player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
        }
    ...
    struct npc_saurfang_eventAI : public ScriptedAI
    {
        ...
        void SpellHit(Unit* /*caster*/, SpellInfo const* spell)
        {
            if (spell->Id == SPELL_GRIP_OF_AGONY)
            {
                me->SetInhabitType(INHABIT_GROUND | INHABIT_AIR);
                me->SetDisableGravity(true);
                me->GetMotionMaster()->MoveTakeoff(POINT_CHOKE, chokePos[_index]);
            }
        }
...
public:
    ...
    class spell_deathbringer_blood_link_AuraScript : public AuraScript
    {
        ...
        bool CheckProc(ProcEventInfo& eventInfo)
        {
            ...
            return eventInfo.GetActor() && eventInfo.GetActionTarget() && (eventInfo.GetDamageInfo()->GetDamage() || eventInfo.GetHitMask() & PROC_EX_ABSORB);
        }
        ...
        void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
        {
            ...
            // Bandera para comprobar el blood power, cantidad por spell.
...
class spell_deathbringer_mark_fallen_champion_damage : public SpellScriptLoader
{
public:
    spell_deathbringer_mark_fallen_champion_damage() : SpellScriptLoader("spell_deathbringer_mark_fallen_champion_damage") { }

    class spell_deathbringer_mark_fallen_champion_damage_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_deathbringer_mark_fallen_champion_damage_SpellScript);
        ...
        void RecalculateDamage()
        {
            ...
            SetHitDamage(int32(spellInfo->Effects[0].BasePoints + urand(200, 900)));
        }

        void Register() override
        {
            OnHit += SpellHitFn(spell_deathbringer_mark_fallen_champion_damage_SpellScript::RecalculateDamage);
        }
    };

    SpellScript* GetSpellScript() const override
    {
        return new spell_deathbringer_mark_fallen_champion_damage_SpellScript();
    }
};
...
void AddSC_boss_deathbringer_saurfang()
{
    ...
    new spell_deathbringer_mark_fallen_champion_damage();
}
