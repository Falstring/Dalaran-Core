...
public:
	...
	struct boss_prince_taldaramAI : public ScriptedAI
	{
		...
		void JustSummoned(Creature* summon)
		{
			summons.Summon(summon);
			Unit* target = nullptr;
			if (summon->GetEntry() == NPC_BALL_OF_INFERNO_FLAME || summon->GetEntry() == NPC_BALL_OF_FLAME)
			{
				summon->CastSpell(summon, SPELL_BALL_OF_FLAMES, true);
				target = CheckPlayersInRange(RAID_MODE<uint32>(4, 9, 4, 9), 15.0f, 100.0f);  // try to select a target wich is farer than 15y
			}

			if (!target)
				target = SelectTarget(SELECT_TARGET_RANDOM, 0, NonTankTargetSelector(me)); // too bad for you raiders, its going to boom
...
public:
	...
	struct npc_blood_queen_lana_thelAI : public ScriptedAI
	{
		...
		void MoveInLineOfSight(Unit* who)
		{
			...
			if (who->GetTypeId() != TYPEID_PLAYER || me->GetExactDist2d(who) > 45.0f)
				return;
...
public:
	...
	struct npc_dark_nucleusAI : public ScriptedAI
	{
		...
		void EnterEvadeMode()
		{
			if (Unit* target = me->SelectNearestPlayer(50.f))
				AttackStart(target);
			else
				ScriptedAI::EnterEvadeMode();
		}
...
public:
	...
	struct npc_ball_of_flameAI : public ScriptedAI
	{
		...
		void Reset()
		{
			...
			if (me->GetEntry() == NPC_BALL_OF_FLAME)
			{
		...
		void UpdateAI(uint32 diff)
		{
			if (!_chaseDistanceTimer)
				return;

			if (_chaseDistanceTimer <= diff)
			{
				_chaseDistanceTimer = 500;
				if (Player* target = ObjectAccessor::GetPlayer(*me, _chaseGUID))
				{
					...
					{
						DoCast(me, SPELL_FLAMES, true);
						me->RemoveAurasDueToSpell(SPELL_BALL_OF_FLAMES_PERIODIC);
						me->RemoveAurasDueToSpell(SPELL_BALL_OF_FLAMES_PROC);
						_despawnTimer = 500;
