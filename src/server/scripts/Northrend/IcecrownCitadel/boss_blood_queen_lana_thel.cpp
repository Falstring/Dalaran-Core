...
enum Events
{
	...
	EVENT_AIR_FLY_DOWN,
	EVENT_CHECK_EMPOWERED_BLOOD,
	EVENT_STOP_CHECKING_EMPOWERED_BLOOD,
};
...
public:
	...
	struct boss_blood_queen_lana_thelAI : public BossAI
	{
		...
		std::set<ObjectGuid> _bloodboltedPlayers;
		std::set<ObjectGuid> _vampires;
		...
		void EnterCombat(Unit* who)
		{
			...
			events.ScheduleEvent(EVENT_CHECK_EMPOWERED_BLOOD, 1000);
			events.ScheduleEvent(EVENT_STOP_CHECKING_EMPOWERED_BLOOD, 10000);
			...
			if (_creditBloodQuickening)
			{
				...
				for (Map::PlayerList::const_iterator itr = pl.begin(); itr != pl.end(); ++itr)
					if (Player* p = itr->GetSource())
						p->KilledMonsterCredit(Is25ManRaid() ? NPC_BLOOD_QUICKENING_CREDIT_25 : NPC_INFILTRATOR_MINCHAR_BQ, ObjectGuid::Empty);
		...
		void UpdateAI(uint32 diff)
		{
			...
			case EVENT_AIR_FLY_DOWN:
				...
				break;
			case EVENT_CHECK_EMPOWERED_BLOOD:
				if (instance)
					instance->DoRemoveAurasDueToSpellOnPlayers(70227);
				events.RescheduleEvent(EVENT_CHECK_EMPOWERED_BLOOD, 1 * IN_MILLISECONDS);
				break;
			case EVENT_STOP_CHECKING_EMPOWERED_BLOOD:
				events.CancelEvent(EVENT_CHECK_EMPOWERED_BLOOD);
				break;
			}
	...
	class spell_blood_queen_presence_of_the_darkfallen_SpellScript : public SpellScript
	{
		...
		void HandleScript(SpellEffIndex effIndex)
		{
			...
			if (InstanceScript* instance = GetHitUnit()->GetInstanceScript())
				GetHitUnit()->CastSpell((Unit*)NULL, GetSpellInfo()->Effects[effIndex].TriggerSpell, true, NULL, NULL, instance->GetGuidData(DATA_BLOOD_QUEEN_LANA_THEL));
		}
