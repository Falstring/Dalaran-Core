...
public:
    npc_alchemist_finklestein() : CreatureScript("npc_alchemist_finklestein") { }
        ...
        struct npc_alchemist_finklesteinAI : public ScriptedAI
        {
            ...
            {
                while (!_playerMap.empty())
                {
                    _playerMap.erase(_playerMap.begin());
                }
            }
            ...
            void Reset() override
            {
                ...
                clearQueue(_playerQueue);
            }

            void SpellHit(Unit* caster, const SpellInfo* spell) override
            {
                if (spell->Id != SPELL_SET_LAST_PLAYER)
                    return;

                if (caster->HasAura(SPELL_ALCHEMIST_APPRENTICE_INVISBUFF))
                    _playerQueue.push(caster->ToPlayer()->GetGUID());
                else
                {
                    _playerMap.erase(caster->ToPlayer()->GetGUID());
                    return;
                }

                switch (_playerMap[caster->ToPlayer()->GetGUID()])
                {
                    case 2:
                    case 3:
            ...
            void UpdateAI(uint32 diff) override
            {
                ...
                while (uint32 eventId = _events.ExecuteEvent())
                {
                    ObjectGuid _lastActionPlayerGUID;
                    switch (eventId)
                    {
                        ...
                        case EVENT_TURN_BACK:
                            ...
                            break;
                        case EVENT_EASY_123:
                            _lastActionPlayerGUID = getLastActionPlayerGuidWithoutAura();
                            if (Player* player = ObjectAccessor::GetPlayer(*me, _lastActionPlayerGUID))
                            {
                                Talk(SAY_EASY_123, player);
                                DoCast(player, SPELL_RANDOM_INGREDIENT_EASY_AURA);
                                uint8 oldIngredientEntry = _playerMap[_lastActionPlayerGUID];
                                _playerMap[_lastActionPlayerGUID] = ++oldIngredientEntry;
                            }
                            break;
                        case EVENT_MEDIUM_4:
                            _lastActionPlayerGUID = getLastActionPlayerGuidWithoutAura();
            ...
            void sGossipSelect(Player* player, uint32 /*menuId*/, uint32 /*gossipListId*/) override
            {
                ...
                _events.ScheduleEvent(EVENT_EASY_123, 100);
                _playerMap[player->GetGUID()] = 1;
                _playerQueue.push(player->GetGUID());
            }

            void sQuestAccept(Player* player, Quest const* /*quest*/) override
            {
                if (player->HasAura(SPELL_ALCHEMIST_APPRENTICE_INVISBUFF))
                    player->RemoveAura(SPELL_ALCHEMIST_APPRENTICE_INVISBUFF);
            }

        private:
            EventMap _events;
            std::map<ObjectGuid, uint8> _playerMap;
            std::queue<ObjectGuid> _playerQueue;

            void clearQueue( std::queue<ObjectGuid> &q )
            {
               std::queue<ObjectGuid> empty;
               std::swap( q, empty );
            }

            ObjectGuid getLastActionPlayerGuidWithoutAura()
            {
                ObjectGuid _lastActionPlayerGUID = ObjectGuid::Empty;
                if (!_playerQueue.empty())
                {
                    _lastActionPlayerGUID = _playerQueue.front();
                    _playerQueue.pop();
                }

                bool playerAlreadyHasAura = true;
                while (playerAlreadyHasAura && !_playerQueue.empty())
                {
                    if (Player* player = ObjectAccessor::GetPlayer(*me, _lastActionPlayerGUID))
                    {
                        if (hasIngredientAura(player))
                        {
                            _lastActionPlayerGUID = _playerQueue.front();
                            _playerQueue.pop();
                        }
                        ...
                        {
                            playerAlreadyHasAura = false;
                        }
                    }
                }
                return _lastActionPlayerGUID;
            }

            bool hasIngredientAura(Player* player)
            {
                    ...
                    if (player->HasAura(FetchIngredients[i][0]))
                    {
                        ...
                        if (player->HasAura(FetchIngredients[i][1]))
                        {
                            ...
                            if (i < 15)
                            {
                                if (Creature* finklestein = GetClosestCreatureWithEntry(player, NPC_FINKLESTEIN, 25.0f))
                                {
                                    player->CastSpell(finklestein, SPELL_SET_LAST_PLAYER);
                                }
                                return;
                            }
                            else
                ...
                if (GetTargetApplication()->GetRemoveMode() == AURA_REMOVE_BY_EXPIRE)
                    if (target->HasAura(SPELL_ALCHEMIST_APPRENTICE_INVISBUFF))
                        if (Creature* finklestein = GetClosestCreatureWithEntry(target, NPC_FINKLESTEIN, 100.0f))
                        {
                            ...
                            target->CastSpell(finklestein, SPELL_SET_LAST_PLAYER);
                        }
            }
...
enum EnragedMammoth
{
    SPELL_MEDALLION_OF_MAMTHOTH              = 52596,
    SPELL_RIDING_ENRAGED_MAMMOTH             = 52600,
    AREA_ALTAR_OF_MAMTHOTH                   = 4329
};

class npc_enraged_mammoth : public CreatureScript
{
public:
    npc_enraged_mammoth() : CreatureScript("npc_enraged_mammoth") { }

    struct npc_enraged_mammoth_vehicleAI : public VehicleAI
    {
        npc_enraged_mammoth_vehicleAI(Creature* creature) : VehicleAI(creature)
        {
            uiCheckAuraTimer = 1000;
            hitBySpell = false;
            beignControlled = false;
            counter = 0;			
        }

        uint32 uiCheckAuraTimer;
        bool hitBySpell;
        bool beignControlled;
        ...
        void AttackStart(Unit* unit) override {
            ...
            CreatureAI::AttackStart(unit);
        }

        void SpellHit(Unit* caster, const SpellInfo* spell) override
        {
            if (spell->Id == SPELL_MEDALLION_OF_MAMTHOTH)
            {
            ...
            if (spell->Id == SPELL_RIDING_ENRAGED_MAMMOTH)
                beignControlled = true;
        }
        ...
        void UpdateAI(uint32 uiDiff) override
        {
            ...
            if (uiCheckAuraTimer <= uiDiff)
            {
                if (hitBySpell && !beignControlled) {
                    if (counter < 15)
                        counter++;
                    else
                        Reset();
                ...
                if (!me->HasAura(SPELL_RIDING_ENRAGED_MAMMOTH) && beignControlled)
                {
                    beignControlled = false;
                    me->DespawnOrUnsummon();
                    return;
                ...
                if (me->GetAreaId() != AREA_ALTAR_OF_MAMTHOTH)
                {
                    me->RemoveAura(SPELL_RIDING_ENRAGED_MAMMOTH);
                }

                uiCheckAuraTimer = 1000;
            } else uiCheckAuraTimer -= uiDiff;
    ...
    CreatureAI* GetAI(Creature* creature) const override
    {
        return new npc_enraged_mammoth_vehicleAI(creature);
    }
};
...
class spell_spirit_burn_aura : public SpellScriptLoader
{
    public: spell_spirit_burn_aura() : SpellScriptLoader("spell_spirit_burn_aura") { }

        class spell_spirit_burn_aura_AuraScript : public AuraScript
        {
            PrepareAuraScript(spell_spirit_burn_aura_AuraScript);
            ...
            void OnApply(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
            {
                ...
                {
                    float modValue = -45.0f;
                    target->HandleStatModifier(UnitMods(UNIT_MOD_STAT_START + i), TOTAL_VALUE, modValue, true);
                    if (target->GetTypeId() == TYPEID_PLAYER || target->IsPet())
                        target->ApplyStatBuffMod(Stats(i), modValue, true);
                }
            ...
            void OnRemove(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
            {
                ...
                target->InitStatBuffMods();
            }

            void Register() override
            {
                OnEffectApply += AuraEffectApplyFn(spell_spirit_burn_aura_AuraScript::OnApply, EFFECT_1, SPELL_AURA_MOD_PERCENT_STAT, AURA_EFFECT_HANDLE_REAL);
                OnEffectRemove += AuraEffectRemoveFn(spell_spirit_burn_aura_AuraScript::OnRemove, EFFECT_1, SPELL_AURA_MOD_PERCENT_STAT, AURA_EFFECT_HANDLE_REAL);
            }
        };

        AuraScript* GetAuraScript() const override
        {
            return new spell_spirit_burn_aura_AuraScript();
        }
};
    ...
    struct npc_nerubian_cocoonAI : public ScriptedAI
    {
        ...
        void JustDied(Unit* killer) override
        {
            if (urand(0, 100) <= 50)
            {
                ...
                me->SummonCreature(NPC_DRAKKARI_CAPTIVE, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), 0.0f, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 240000);
        }
...
void AddSC_zuldrak()
{
    ...
    new npc_enraged_mammoth();
    new spell_spirit_burn_aura();
