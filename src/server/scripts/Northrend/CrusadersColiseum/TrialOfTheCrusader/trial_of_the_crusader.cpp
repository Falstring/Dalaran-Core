        ...
        bool OnGossipHello(Player* player, Creature* creature) override
        {
            ...
            for (; i < NUM_MESSAGES; ++i)
            {
                    ...
                    || (_GossipMessage[i].state && instance->GetBossState(_GossipMessage[i].encounter) == DONE))
                {
					...
					{
					case 0:
						_message = "Bring forth the first challenge!"; // to activate Northrend beasts.
						break;
					case 1:
						_message = "We're ready for the next challenge."; // Activate Lord Jaraxxus
						break;
					case 2: // Activate champions
						_message = "Give the signal! We're ready to go!";
						break;
					case 3: // Activate valkyrs
						_message = "Val'kyr? We're ready for them.";
						break;
					case 4: // Activate Lich King
						_message = "That is strange...";
						break;
					default:
						break;
					}
...
class npc_tirion_toc : public CreatureScript
{
    public:
        ...
        struct npc_tirion_tocAI : public ScriptedAI
        {
            ...
            {
                _instance = me->GetInstanceScript();
				_factionControlerSummoned = false;
            }
                    ...
                    switch (_instance->GetData(TYPE_EVENT))
                    {
                        ...
                        case 150:
                            ...
                            if (_instance->GetBossState(BOSS_BEASTS) != DONE)
                            {
                                ...
                                {
                                    temp->GetMotionMaster()->MovePoint(0, ToCCommonLoc[5].GetPositionX(), ToCCommonLoc[5].GetPositionY()-25, ToCCommonLoc[5].GetPositionZ());
                        ...
                        case 3091:
							if (!_factionControlerSummoned)
							{
								...
								_factionControlerSummoned = true;
							}
							else
							{
                        ...
                        case 4010:
                            Talk(SAY_STAGE_3_02);
                            if (Creature* temp = me->SummonCreature(NPC_LIGHTBANE, ToCSpawnLoc[1].GetPositionX(), ToCSpawnLoc[1].GetPositionY(), ToCSpawnLoc[1].GetPositionZ()+6, 5, TEMPSUMMON_CORPSE_TIMED_DESPAWN, DESPAWN_TIME))
                            {
                            ...
                            if (Creature* temp = me->SummonCreature(NPC_DARKBANE, ToCSpawnLoc[2].GetPositionX(), ToCSpawnLoc[2].GetPositionY(), ToCSpawnLoc[2].GetPositionZ()+6, 5, TEMPSUMMON_CORPSE_TIMED_DESPAWN, DESPAWN_TIME))
                            {
								temp->SetDisableGravity(true);
                                temp->SetVisible(false);
                                temp->SetReactState(REACT_PASSIVE);
                        ...
                        case 4015:
                            ...
                            {
                                temp->GetMotionMaster()->MovePoint(1, ToCCommonLoc[8].GetPositionX(), ToCCommonLoc[8].GetPositionY()-1, ToCCommonLoc[8].GetPositionZ()+6);
                                temp->SetVisible(true);
                            }
                            if (Creature* temp = ObjectAccessor::GetCreature((*me), _instance->GetGuidData(NPC_DARKBANE)))
                            {
                                temp->GetMotionMaster()->MovePoint(1, ToCCommonLoc[9].GetPositionX(), ToCCommonLoc[9].GetPositionY()-1, ToCCommonLoc[9].GetPositionZ()+6);
                                temp->SetVisible(true);
                            }
                            _updateTimer = 10*IN_MILLISECONDS;
                        ...
                        case 4016:
                            ...
                            break;
                        case 4040:
                            _updateTimer = 12*IN_MILLISECONDS; // Here, the Lich king
                            _instance->SetData(TYPE_EVENT, 5000);
                            break;
                        case 5000:
                            Talk(SAY_STAGE_4_01);
                            _updateTimer = 16*IN_MILLISECONDS;
            ...
            private:
                InstanceScript* _instance;
				bool _factionControlerSummoned;
        };
                ...
                if (_updateTimer <= uiDiff)
                {
                    switch (_instance->GetData(TYPE_EVENT))
                    {
                        ...
                        case 3080:
                            ...
                            break;
                        case 4020:
                            Talk(SAY_STAGE_3_03a); // Varyan, right after the gossip selection to anub
                            _updateTimer = 5*IN_MILLISECONDS;
                            _instance->SetData(TYPE_EVENT, 4040);
                            break;
                        default:
