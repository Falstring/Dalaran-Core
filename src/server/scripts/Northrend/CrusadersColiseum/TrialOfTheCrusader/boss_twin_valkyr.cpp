...
enum Events
{
	...
	EVENT_SUMMON_BULLET_CONTROLLER	=6,

	EVENT_ORB_CHANGE_POINT	= 7,
	EVENT_ORB_DESPAWN		= 8,
...
struct boss_twin_baseAI : public BossAI
{
    ...
    void EnterCombat(Unit* /*who*/) override
    {
		...
		if (me->GetEntry() == 34497) // Fjola Lightbane.
			events.ScheduleEvent(EVENT_SUMMON_BULLET_CONTROLLER, 2 * IN_MILLISECONDS);
    ...
    void DoAction(int32 action) override
    {
        switch (action)
        {
            ...
            case ACTION_PACT:
				...
				events.ScheduleEvent(EVENT_SURGE_CHECK, 1000);				
                break;
            default:
                break;
        }
    ...
    void ExecuteEvent(uint32 eventId) override
    {
        ...
        switch (eventId)
        {
			case EVENT_SUMMON_BULLET_CONTROLLER:
			...
			break;
			case EVENT_MOVE:
				if (!me->HasUnitState(UNIT_STATE_CASTING)) // Do Also a correct position
				{
					float x = me->GetVictim()->GetPositionX();
					float y = me->GetVictim()->GetPositionY();
					float z = me->GetVictim()->GetPositionZ();
					if (me->IsWithinDist3d(x, y, z, 2.0f))
					{
						me->GetMotionMaster()->Initialize();
					}
					else
						me->GetMotionMaster()->MovePoint(0, x-1.0f, y-1.0f, 401.04f);
				}
            ...
            case EVENT_BERSERK:
                ...
                Talk(SAY_BERSERK);
                break;
			case EVENT_SURGE_CHECK:				
				
				if (!me->FindCurrentSpellBySpellId(TwinPactSpellId))
				{
					me->RemoveAura(SurgeSpellId);
					DoCast(me, SurgeSpellId);
					events.RescheduleEvent(EVENT_SURGE_CHECK, 15000);
					break;
				}
				else			
					events.RescheduleEvent(EVENT_SURGE_CHECK, 1000);
...
class boss_fjola : public CreatureScript
{
    public:
        ...
        struct boss_fjolaAI : public boss_twin_baseAI
        {
            ...
            void Reset() override
			{				
				me->ApplySpellImmune(0, IMMUNITY_STATE, SPELL_AURA_MOD_HEALING, true); // Apply immumity to KNOCKBACKS
				me->ApplySpellImmune(0, IMMUNITY_STATE, SPELL_AURA_MOD_HEALING_PCT, true); // Apply immumity to KNOCKBACKS
				me->SetCanFly(true);
			...
			/*
			    ...
			    {586.0f, 169.0f, 395.14f, 0},               //  8 Move 1 Right
	{541.0f, 167.0f, 395.14f, 0 },               //  9 Move 1 Left
			...
			*/
            ...
            void EnterCombat(Unit* who) override
            {
                ...
                events.ScheduleEvent(EVENT_SPECIAL_ABILITY, 45 * IN_MILLISECONDS);
				events.ScheduleEvent(EVENT_SUMMON_BULLET_CONTROLLER, 15 * IN_MILLISECONDS);
...
struct npc_unleashed_ballAI : public ScriptedAI
{
    ...
    void Initialize()
    {
		...
		pointId = 0;
		//me->DespawnOrUnsummon(urand(15*IN_MILLISECONDS, 22*IN_MILLISECONDS));
    }
	...
	void MoveToNextPoint()
	{
		...
		me->GetMotionMaster()->MovePoint(pointId, x, y, me->GetPositionZ());
		pointId++;
	}
    ...
    void Reset() override
    {
        ...
        MoveToNextPoint();
        Initialize();
		ChangePointTimer = urand(9 * IN_MILLISECONDS, 12 * IN_MILLISECONDS);
		me->DespawnOrUnsummon(urand(20 * IN_MILLISECONDS, 30*IN_MILLISECONDS));
    }

    protected:
        uint32 RangeCheckTimer;
		uint32 ChangePointTimer;
		uint32 pointId;
};
    ...
    public:
        ...
        struct npc_unleashed_darkAI : public npc_unleashed_ballAI
        {
            ...
            void UpdateAI(uint32 diff) override
            {
				...
				if (ChangePointTimer <= diff)
				{
					//std::cout << "TRYING TO CHANGE POINT";
					MoveToNextPoint();
					ChangePointTimer = 8 * IN_MILLISECONDS;
				}
				else
					ChangePointTimer -= diff;
            }
        };
        ...
        struct npc_unleashed_lightAI : public npc_unleashed_ballAI
        {
            ...
            void UpdateAI(uint32 diff) override
            {
				...
				if (ChangePointTimer <= diff)
				{
					//std::cout << "TRYING TO CHANGE POINT";
					MoveToNextPoint();
					ChangePointTimer = 10 * IN_MILLISECONDS;
				}
				else
					ChangePointTimer -= diff;
            }
...
class TouchOfLightTargetSelector
{
public:
	TouchOfLightTargetSelector() { }
...
class spell_twin_valkyr_touchof_light : public SpellScriptLoader
{
public:
	spell_twin_valkyr_touchof_light() : SpellScriptLoader("spell_twin_valkyr_touchof_light") { }

	class spell_twin_valkyr_touchof_light_AuraScript : public AuraScript
	{
		PrepareAuraScript(spell_twin_valkyr_touchof_light_AuraScript);


		/*void FilterTargets(std::list<WorldObject*>& targets)
		{
			
			targets.remove_if(TouchOfLightTargetSelector());
		}*/
		...
		void OnPeriodic(AuraEffect const* aurEff)
		{
			if (Unit* caster = GetCaster())
			{
				...
				if (map && map->IsDungeon())
				{
					...
					for (Map::PlayerList::const_iterator itr = Players.begin(); itr != Players.end(); ++itr) // Hacer un if GetTarget diferente del player (no se debe de hacer daño)
						if (Player* player = itr->GetSource())							
							if (target->IsWithinDistInMap(player, 10.0f, true))
							{
								//caster->DealDamage(player, 1000000, 0, DOT, SPELL_SCHOOL_MASK_FIRE, NULL, false);
								if (player->HasAura(67177)) // or 25 one	
								{
									caster->DealDamage(player, 3100, NULL, DIRECT_DAMAGE, SPELL_SCHOOL_MASK_NORMAL, NULL, false);
								}									
								else if (player->HasAura(67178))
								{
									caster->DealDamage(player, 6200, NULL, DIRECT_DAMAGE, SPELL_SCHOOL_MASK_NORMAL, NULL, false);
								}									
							}
				// Y hacer un getcastercastcustomspell del propio surge of light o surge of darkness?				
		...
		void Register() override
		{
			OnEffectPeriodic += AuraEffectPeriodicFn(spell_twin_valkyr_touchof_light_AuraScript::OnPeriodic, EFFECT_0, SPELL_AURA_PERIODIC_DAMAGE);
			/*OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_twin_valkyr_touchof_light_SpellScript::FilterTargets, EFFECT_0, TARGET_UNIT_SRC_AREA_ENEMY);
			OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_twin_valkyr_touchof_light_SpellScript::FilterTargets, EFFECT_2, TARGET_UNIT_SRC_AREA_ENEMY);*/
		}
	};

	AuraScript* GetAuraScript() const override
	{
		return new spell_twin_valkyr_touchof_light_AuraScript();
	}
};
...
class spell_twin_valkyr_touchof_darkness : public SpellScriptLoader
{
public:
	spell_twin_valkyr_touchof_darkness() : SpellScriptLoader("spell_twin_valkyr_touchof_darkness") { }

	class spell_twin_valkyr_touchof_darkness_AuraScript : public AuraScript
	{
		PrepareAuraScript(spell_twin_valkyr_touchof_darkness_AuraScript);
				...
				if (map && map->IsDungeon())
				{
					...
					for (Map::PlayerList::const_iterator itr = Players.begin(); itr != Players.end(); ++itr) // Hacer un if GetTarget diferente del player (no se debe de hacer daño)
						if (Player* player = itr->GetSource())
							if (target->IsWithinDistInMap(player, 10.0f, true))
							{																
								if (player->HasAura(67223)) // or 25 one	
								{
									//std::cout << "\n 10 HC * Touch of Darkness - Source: " << std::string(target->GetName()) << " - Caster: " << std::string(caster->GetName()) << " - Player recibiendo damage: " << std::string(player->GetName()) << "\n";
									caster->DealDamage(player, 3100, NULL, DIRECT_DAMAGE, SPELL_SCHOOL_MASK_NORMAL, NULL, false);
								}									
								else if (player->HasAura(67224))
								{
									//std::cout << "\n 25 HC * Touch of Darkness - Source: " << std::string(target->GetName()) << " - Caster: " << std::string(caster->GetName()) << " - Player recibiendo damage: " << std::string(player->GetName()) << "\n";
									caster->DealDamage(player, 6200, NULL, DIRECT_DAMAGE, SPELL_SCHOOL_MASK_NORMAL, NULL, false);
								}																	
							}
				}
		...
		void Register() override
		{
			OnEffectPeriodic += AuraEffectPeriodicFn(spell_twin_valkyr_touchof_darkness_AuraScript::OnPeriodic, EFFECT_0, SPELL_AURA_PERIODIC_DAMAGE);
			/*OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_twin_valkyr_touchof_light_SpellScript::FilterTargets, EFFECT_0, TARGET_UNIT_SRC_AREA_ENEMY);
			OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_twin_valkyr_touchof_light_SpellScript::FilterTargets, EFFECT_2, TARGET_UNIT_SRC_AREA_ENEMY);*/
		}
	};
	...
	AuraScript* GetAuraScript() const override
	{
		return new spell_twin_valkyr_touchof_darkness_AuraScript();
	}
};
...
void AddSC_boss_twin_valkyr()
{
	...
	new spell_twin_valkyr_touchof_light();
	new spell_twin_valkyr_touchof_darkness();
}
