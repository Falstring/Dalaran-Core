    ...
    public:
        ...
        struct boss_toc_champion_controllerAI : public ScriptedAI
        {
            ...
            void SummonChampions(Team playerTeam)
            {
				...
				if (_wiped)
				{
					if (!_championsVector.empty())
						vChampionEntries = _championsVector;					
				}
				else
				{					
					vChampionEntries = SelectChampions(playerTeam);
					_championsVector = vChampionEntries;
					_wiped = true; // If this is called again, it means it is wipe until the controller initilizes itself (crash).
				}                				
            ...
            private:
				...
				bool _wiped;
				std::vector<uint32> _championsVector;				
        };
...
struct boss_faction_championsAI : public BossAI
{
    ...
    {
        float dist_mod =  15.0f / (12.0f + distance);		
        //float armor_mod = (_aiType == AI_MELEE || _aiType == AI_PET) ? armor / 16635.0f : 0.0f;
        float eh = (health + 1) * 2;// (1.0f + armor_mod)
		//std::cout << "\n Health "<< uint32(health)<< "Dist MOD: " << float(dist_mod) << " EH: " << float(eh) << "\n";
    ...
    void UpdateThreat()
    {
        ...
        for (std::list<HostileReference*>::const_iterator itr = tList.begin(); itr != tList.end(); ++itr)
        {
            ...
            if (unit && me->getThreatManager().getThreat(unit))
            {
                ...
                {										
					...
					//std::cout << "\n INTENTANDO RESETEAR EL AGGRO PARA EL JUGADOR " << std::string(unit->GetName()) << " CON MODIFIER: " << float(threat) << "\n";
					me->getThreatManager().doAddThreat(unit, -(me->getThreatManager().getThreat(unit)));
					me->getThreatManager().doAddThreat(unit, 1000000.0f * threat);
					//me->getThreatManager().doAddThreat(unit, 1000000.0f * threat);
                }
            }
        }*/
		...
		for (Map::PlayerList::const_iterator itr = lPlayers.begin(); itr != lPlayers.end(); ++itr)
		{
			...
			{	
				...
				float threat = CalculateThreat(me->GetDistance2d(player), (float)player->GetArmor(), player->GetHealth());				
				me->AddThreat(player, 1000000.0f * threat);
			}
		}
    }
