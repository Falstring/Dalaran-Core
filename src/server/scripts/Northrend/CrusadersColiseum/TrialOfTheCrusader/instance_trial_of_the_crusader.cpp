    ...
    public:
        ...
        struct instance_trial_of_the_crusader_InstanceMapScript : public InstanceScript
        {
            instance_trial_of_the_crusader_InstanceMapScript(Map* map) : InstanceScript(map)
            {
				...
				JormungarsEntranceForced = false;
				IcehowlEntranceForced = false;
            }
            ...
            void OnPlayerEnter(Player* player) override
            {
                ...
                if (GetBossState(BOSS_LICH_KING) == DONE && TrialCounter && GetBossState(BOSS_ANUBARAK) != DONE && GetBossState(BOSS_ANUBARAK) != SPECIAL)
            ...
            void SetData(uint32 type, uint32 data) override
            {
                switch (type)
                {
					...
					case 1021:
						...
						break;
					case 1024:
						EventStage = 200; // Start jormungars.
						JormungarsEntranceForced = true;
						// controlar mediante un: JORMUNGAR CALLED
					break;
					case 1025:
						if (!IcehowlEntranceForced)
						{
							EventStage = 300; // Start Icehowl
							IcehowlEntranceForced = true;
						}
						/*						if (!IcehowlEntranceForced)
						{
							std::cout << "\n ICEHOWL VARIABLE ESTA A TRUE \n";
						}
						else
							std::cout << "\n ICEHOWL VARIABLE ESTA A FALSE \n";*/					
					break;
                    case TYPE_COUNTER:
                        TrialCounter = data;
                        data = DONE;
                    ...
                    case TYPE_NORTHREND_BEASTS:
                        ...
                        switch (data)
                        {
                            case GORMOK_DONE:
								...
								if (!JormungarsEntranceForced) // hacer un: if !jormungarcalled, then EventStage=200
									EventStage = 200;
            ...
            protected:
				...
				bool JormungarsEntranceForced;
				bool IcehowlEntranceForced;
        };
