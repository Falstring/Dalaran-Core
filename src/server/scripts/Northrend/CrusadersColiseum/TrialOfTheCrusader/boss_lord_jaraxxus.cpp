...
enum Events
{
	...
	EVENT_TOUCH_OF_JARAXXUS			= 11,
	EVENT_CHECK_EXPIRATED_NETHERPOWER =12,
};
...
class boss_jaraxxus : public CreatureScript
{
    public:
        ...
        struct boss_jaraxxusAI : public BossAI
        {
            ...
            {
                _Reset();
				_checkNetherPower = true;
                events.ScheduleEvent(EVENT_FEL_FIREBALL, 5*IN_MILLISECONDS);
                events.ScheduleEvent(EVENT_FEL_LIGHTNING, 10*IN_MILLISECONDS);
				events.ScheduleEvent(EVENT_INCINERATE_FLESH, 15*IN_MILLISECONDS);
				...
				if (instance->instance->GetDifficulty() == RAID_DIFFICULTY_25MAN_HEROIC)									
					events.ScheduleEvent(EVENT_TOUCH_OF_JARAXXUS, 15 * IN_MILLISECONDS);				
            }
			...
			void CheckSpellNetherPower()
			{
				if (me->HasAura(SPELL_NETHER_POWER))
					me->RemoveAura(SPELL_NETHER_POWER);
				if (me->HasAura(67106))
					me->RemoveAura(67106);
				if (me->HasAura(67107))
					me->RemoveAura(67107);
		...
		void SpellHit(Unit* caster, const SpellInfo* spell)
		{
				...
				case 30449:
					{
						...
						if( Aura* a = me->GetAura(id) )
						{
							...
							{
								// Start checking if Jaraxxus has ANY NETHER POWER BUFF
								_checkNetherPower = true;								
								a->Remove();								
							}								
            ...
            void UpdateAI(uint32 diff) override
            {
				...
				if (_checkNetherPower)
					CheckSpellNetherPower();
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_FEL_FIREBALL:
                            ...
                            events.ScheduleEvent(EVENT_FEL_FIREBALL, 9*IN_MILLISECONDS);							
                            return;
                        case EVENT_FEL_LIGHTNING:
                            ...
                            events.RescheduleEvent(EVENT_FEL_LIGHTNING, 12*IN_MILLISECONDS);							
                            return;
                        case EVENT_INCINERATE_FLESH:
							...
							events.RescheduleEvent(EVENT_INCINERATE_FLESH, 24*IN_MILLISECONDS);
							events.ScheduleEvent(EVENT_LEGION_FLAME, 13 * IN_MILLISECONDS);
                            return;
                        case EVENT_NETHER_POWER:							
							_checkNetherPower = false;							
							me->CastStop();
							me->StopMoving();
							me->GetSpellHistory()->ResetAllCooldowns();
							...
							events.ScheduleEvent(EVENT_CHECK_EXPIRATED_NETHERPOWER, 26 * IN_MILLISECONDS);
							events.DelayEvents(5000);
                            events.ScheduleEvent(EVENT_NETHER_POWER, 40*IN_MILLISECONDS);
                            return;
						case EVENT_CHECK_EXPIRATED_NETHERPOWER:
                        ...
                        case EVENT_LEGION_FLAME:
							...
							if (Unit* target = CheckPlayersInRange(1, 14.0f, 90.0f))
							{
							...
							{
								if (Unit* target = target = SelectTarget(SELECT_TARGET_RANDOM, 0, 150.0f, true))
								{
									DoCast(target, SPELL_LEGION_FLAME);
									Talk(EMOTE_LEGION_FLAME, target);
								}
    ...
    public:
        ...
        struct npc_infernal_volcanoAI : public ScriptedAI
        {
            ...
            void Reset() override
            {
				...
				me->ApplySpellImmune(0, IMMUNITY_EFFECT, SPELL_EFFECT_KNOCK_BACK, true); // Apply immumity to KNOCKBACKS
...
class npc_fel_infernal : public CreatureScript
{
    public:
        ...
        struct npc_fel_infernalAI : public ScriptedAI
        {
            ...
            void Initialize()
            {
                _felStreakTimer = 10 * IN_MILLISECONDS;				
            }
            ...
            void UpdateAI(uint32 diff) override
            {
                ...
                {
					// for 6 seconds, ADD immunity to GRIP mechanics
					
					if (Creature* jaraxxus = ObjectAccessor::GetCreature(*me, _instance->GetGuidData(NPC_JARAXXUS)))
					{
						if (Unit* target = jaraxxus->AI()->SelectTarget(SELECT_TARGET_RANDOM, 0, 0.0f, true))
						{
							DoCast(target, 66493);
							DoCast(target, SPELL_FEL_STREAK);							
						}							
							/*std::string text = "INFERNAL TARGET IS: ";
							text.append(target->GetName());

							sWorld->SendWorldText(21000, text.c_str());*/							
																	
					}                        
                    _felStreakTimer = 20*IN_MILLISECONDS;
                }
                else
...
class npc_mistress_of_pain : public CreatureScript
{
    public:
        ...
        struct npc_mistress_of_painAI : public ScriptedAI
        {
            ...
            void Reset() override
            {
                _events.ScheduleEvent(EVENT_SHIVAN_SLASH, 18*IN_MILLISECONDS);
                _events.ScheduleEvent(EVENT_SPINNING_STRIKE, 10*IN_MILLISECONDS);
                if (IsHeroic())
