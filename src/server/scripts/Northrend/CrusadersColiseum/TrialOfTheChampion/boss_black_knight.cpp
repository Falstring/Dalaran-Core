...
public:
	...
	struct boss_black_knightAI : public ScriptedAI
	{
			...
			if (Phase == INTRO)
			{
				if (uiIntroTimer <= uiDiff)
				{
					...
					case 2:
					{
						...
						if (pAnnouncer = ObjectAccessor::GetCreature(*me, instance->GetGuidData(DATA_ANNOUNCER)))
						{
							pAnnouncer->SetOrientation(1.53f);
							pAnnouncer->SetFacingTo(1.53f);
						}
						++uiIntroPhase;
						uiIntroTimer = 2000;
						break;
					...
					case 3:
						// Probably cast the thing over the announcer

						/*SET ORIENTATION HACIA EL BLACK KNIGHT DEL PANNOUNCER*/
						...
						break;
					case 4:
						// update entry to the risen ghoul and attack the player at once with the black knight.
						// 67715 raise dead jarene
						// 67705 raise arelas
						if (pAnnouncer)
							pAnnouncer->AddAura(66798, pAnnouncer);

						//pAnnouncer->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAGS_NO_TARGETEABLE
					...
					case 5:
						...
						if (pAnnouncer)
						{
							pAnnouncer->AI()->DoCast(pAnnouncer, 66797);
							DoCast(pAnnouncer, 66797); // Death's push.
						}
					...
					case 6:
						...
						if (pAnnouncer)
						{
							pAnnouncer->RemoveAura(66798);
							pAnnouncer->AddAura(66804, pAnnouncer);
						}

						//pAnnouncer->AI()->DoCast(pAnnouncer, 66804); // Feign death del announcer.
					...
					case 7:
						...
						y hacer un feign death con él + no targeteable ni nada (el feign de los grand champions)    66804 parece que es este

						en cuanto el black knight entre en combate, el announcer tiene que hacer un unaura del feign + death raise 67705 arelas
		...
		void EnterCombat(Unit* who)
		{
			...
			if (pAnnouncer)
			{
				pAnnouncer->RemoveAura(66804);
				if (instance->GetData(DATA_TEAM_IN_INSTANCE) == HORDE)
					pAnnouncer->AI()->DoCast(ZOMBIE_JAEREN);
				else
					pAnnouncer->AI()->DoCast(ZOMBIE_ARELAS);
			...
			/*if (Creature* zombie = me->FindNearestCreature(35564, 200.0f))
			{
			zombie->SetInCombatWith(who);
			zombie->Attack(who,true);
			}
			else if (Creature* zombie = me->FindNearestCreature(35545, 200.0f))
			{
			zombie->SetInCombatWith(who);
			zombie->Attack(who, true);
			}*/
...
public:
	...
	struct npc_risen_announcerAI : public ScriptedAI
	{
		npc_risen_announcerAI(Creature* creature) : ScriptedAI(creature)
		{
			...
			{
				//std::cout << "\n PLAYER DETECTADO: " << player->GetName();
				me->GetMotionMaster()->MoveJump(player->GetPosition(), 15.0f, 15.0f);
...
public:
	...
	struct npc_black_knight_skeletal_gryphonAI : public npc_escortAI
	{
		...
		void WaypointReached(uint32 uiPointId)
		{
			...
			{
				me->SetSpeed(MOVE_FLIGHT, 2.0f);
				if (Creature* pHighlord = ObjectAccessor::GetCreature(*me, instance->GetGuidData(DATA_HIGHLORD)))
