...
public:
	...
	struct boss_hadronoxAI : public BossAI
	{
		...
		bool CanDetectFeignDeathOf(const Unit* target) const
		{
		...
		bool AnyPlayerValid() const
		{
			...
			for (Map::PlayerList::const_iterator itr = playerList.begin(); itr != playerList.end(); ++itr)
				if (me->GetDistance(itr->GetSource()) < 130.0f && itr->GetSource()->IsAlive() && !itr->GetSource()->IsGameMaster() && me->CanCreatureAttack(itr->GetSource()) && CanDetectFeignDeathOf(itr->GetSource()))
					return true;
	...
	struct npc_anub_ar_crusherAI : public ScriptedAI
	{
		...
		{		
			if (InstanceScript* instance = me->GetInstanceScript())
				if (instance->GetBossState(DATA_HADRONOX_EVENT) == NOT_STARTED)
				{
					if (me->ToTempSummon())
						if (Unit* summoner = me->ToTempSummon()->GetSummoner())
							if (summoner->GetEntry() != me->GetEntry())
