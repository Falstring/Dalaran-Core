...
/*#####
# spell_northsea_force_reaction
#####*/

class spell_northsea_force_reaction : public SpellScriptLoader
{
    enum Northsea
    {
        NORTHSEA_FACTION = 1046
    };
    public: spell_northsea_force_reaction() : SpellScriptLoader("spell_northsea_force_reaction") { }

        class spell_northsea_force_reaction_SpellScript : public SpellScript
        {
            PrepareSpellScript(spell_northsea_force_reaction_SpellScript);

            void OnApply()
            {
                if (Player* player = GetHitPlayer())
                {
                    ReputationRank factionRank = ReputationRank(REP_FRIENDLY);
                    player->GetReputationMgr().ApplyForceReaction(NORTHSEA_FACTION, factionRank, true);
            ...
            void Register() override
            {
                AfterHit += SpellHitFn(spell_northsea_force_reaction_SpellScript::OnApply);
            }
        };

        SpellScript* GetSpellScript() const override
        {
            return new spell_northsea_force_reaction_SpellScript();
        }
};
...
void AddSC_howling_fjord()
{
    ...
    new spell_northsea_force_reaction();
 }
