            ...
            if (bCanEat || bIsEating)
            {
                if (EatTimer <= diff)
                {
                    if (bCanEat && !bIsEating)
                    {
                        ...
                        {
                            if (GameObject* go = unit->FindNearestGameObject(GO_CARCASS, 30))
                            {
                    ...
                    else if (bIsEating)
                    {
                        ...
                        if (Player* player = ObjectAccessor::GetPlayer(*me, uiPlayerGUID))
                        {
                            ...
                            if (GameObject* go = player->FindNearestGameObject(GO_CARCASS, 30))
                                go->Delete();
                        }
    ...
    class spell_q10612_10613_the_fel_and_the_furious_SpellScript : public SpellScript
    {
        ...
        void HandleScriptEffect(SpellEffIndex effIndex)
        {
            ...
            for (std::list<GameObject*>::const_iterator itr = gList.begin(); itr != gList.end(); ++itr, ++counter)
            {
                ...
                charmer->KilledMonsterCredit(21959, ObjectGuid::Empty);
            }
        }
