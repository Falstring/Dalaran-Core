...
// Quest Dimensius the All-Devouring (10439)
enum saeed
{
...
public:
    ...
    struct npc_captain_saeedAI : public npc_escortAI
    {
        ...
        void SetGUID(ObjectGuid playerGUID, int32 type) override
        {
            if (type == DATA_START_ENCOUNTER)
            {
                ...
                me->GetCreatureListWithEntryInGrid(cl, NPC_PROTECTORATE_AVENGER, 20.0f);
                ...
                cl.clear();
                me->GetCreatureListWithEntryInGrid(cl, NPC_PROTECTORATE_DEFENDER, 20.0f);
                for (std::list<Creature*>::iterator itr = cl.begin(); itr != cl.end(); ++itr)
                {
                    summons.Summon(*itr);
                    (*itr)->HandleEmoteCommand(EMOTE_ONESHOT_ROAR);
                ...
                cl.clear();
                me->GetCreatureListWithEntryInGrid(cl, NPC_PROTECTORATE_REGENERATOR, 20.0f);
                for (std::list<Creature*>::iterator itr = cl.begin(); itr != cl.end(); ++itr)
                {
                    summons.Summon(*itr);
                    (*itr)->HandleEmoteCommand(EMOTE_ONESHOT_ROAR);
        ...
        {
            float i = 0;
            for (std::list<ObjectGuid>::iterator itr = summons.begin(); itr != summons.end(); ++itr, i += 1.0f)
                if (Creature* cr = ObjectAccessor::GetCreature(*me, *itr))
                {
                    if (who == NULL)
                    {
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 /*uiSender*/, uint32 uiAction) override
    {
        ...
        if (uiAction == GOSSIP_ACTION_INFO_DEF+1)
        {
            ...
            player->KilledMonsterCredit(creature->GetEntry(), ObjectGuid());
        }
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        if (player->GetQuestStatus(QUEST_DIMENSIUS_DEVOURING) == QUEST_STATUS_INCOMPLETE)
        {
            ...
            {
                player->ADD_GOSSIP_ITEM_DB(GOSSIP_0, 0, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+1);
                player->SEND_GOSSIP_MENU(player->GetGossipTextId(creature), creature->GetGUID());
            }
            else
            {
                player->ADD_GOSSIP_ITEM_DB(GOSSIP_1, 0, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+2);
                player->SEND_GOSSIP_MENU(GOSSIP_1_TEXT, creature->GetGUID());
            }
        }
        else
        {
            ...
            void WaypointReached(uint32 waypointId) override
            {
                ...
                switch (waypointId)
                {
                    ...
                    case 7:
                        ...
                        break;
                    case 8:
                        me->CastSpell(me, SPELL_SUMMON_SMOKE);
                        if (Player* player = GetPlayerForEscort())
