...
class npc_bloodmaul_brutebane : public CreatureScript
{
    public:
        ...
        struct npc_bloodmaul_brutebaneAI : public ScriptedAI
        {
            ...
            {
                if (Creature* Ogre = me->FindNearestCreature(NPC_OGRE_BRUTE, 30.0f, true))
                {
                    Ogre->SetReactState(REACT_DEFENSIVE);
                    Ogre->GetMotionMaster()->Clear();
                    Ogre->GetMotionMaster()->MovePoint(1, me->GetPositionX() - 1, me->GetPositionY() + 1, me->GetPositionZ());
                }
                else if (Creature* Ogre = me->FindNearestCreature(NPC_OGRE_SHAMAN, 30.0f, true))
                {
                    Ogre->SetReactState(REACT_DEFENSIVE);
                    Ogre->GetMotionMaster()->Clear();
                    Ogre->GetMotionMaster()->MovePoint(1, me->GetPositionX() - 1, me->GetPositionY() + 1, me->GetPositionZ());
                }
                else if (Creature* Ogre = me->FindNearestCreature(NPC_OGRE_COOK, 30.0f, true))
                {
                    Ogre->SetReactState(REACT_DEFENSIVE);
                    Ogre->GetMotionMaster()->Clear();
                    Ogre->GetMotionMaster()->MovePoint(1, me->GetPositionX() - 1, me->GetPositionY() + 1, me->GetPositionZ());
    ...
    public:
        ...
        struct npc_bloodmaul_bruteAI : public ScriptedAI
        {
            ...
            void EnterCombat(Unit* /*who*/) override
            {
                ...
                {
                    case NPC_OGRE_BRUTE:
                        events.ScheduleEvent(EVENT_CLEAVE, urand(9, 12) * IN_MILLISECONDS);
                        events.ScheduleEvent(EVENT_DEBILITATING_STRIKE, 15 * IN_MILLISECONDS);
                        break;
                    case NPC_OGRE_SHAMAN:
                        events.ScheduleEvent(EVENT_SHIELD, 1 * IN_MILLISECONDS);
                        events.ScheduleEvent(EVENT_BOLT, urand(2, 4) * IN_MILLISECONDS);
                        break;
                    case NPC_OGRE_COOK:
                        events.ScheduleEvent(EVENT_MEAT_SLAP, urand(15, 20) * IN_MILLISECONDS);
                        events.ScheduleEvent(EVENT_TENDERIZE, urand(15, 20) * IN_MILLISECONDS);
                        break;
                    default:
                        break;
                }
            ...
            void MovementInform(uint32 /*type*/, uint32 id) override
            {
                if (id == 1)
                {
                    if (GameObject* Keg = me->FindNearestGameObject(GO_KEG, 20.0f))
                        Keg->Delete();
                    ...
                    Creature* Credit = me->FindNearestCreature(NPC_QUEST_CREDIT, 50.0f, true);
                    if (player && Credit)
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_DEBILITATING_STRIKE:
                            ...
                            break;
                        case EVENT_MEAT_SLAP:
                            DoCastVictim(SPELL_MEAT);
                            events.ScheduleEvent(EVENT_MEAT_SLAP, urand(15, 20) * IN_MILLISECONDS);
                            break;
                        ...
                        case EVENT_BOLT:
                            ...
                            events.ScheduleEvent(EVENT_BOLT, urand(8, 12) * IN_MILLISECONDS);
                            break;
                        default:
                            break;
                    }
...
class npc_deaths_door_wrap_gate : public CreatureScript
{
public:
    npc_deaths_door_wrap_gate() : CreatureScript("npc_deaths_door_wrap_gate") { }

    CreatureAI* GetAI(Creature* creature) const
    {
        return new npc_deaths_door_wrap_gateAI(creature);
    }

    struct npc_deaths_door_wrap_gateAI : public ScriptedAI
    {
        npc_deaths_door_wrap_gateAI(Creature *c) : ScriptedAI(c)
        {
        }

        uint8 count;
        uint32 timer;
        void Reset() override { count = 0; timer = 0;}
            ...
            if (spellInfo->Id == SPELL_ARTILLERY_ON_THE_WRAP_GATE)
            {
                ...
                if (count >= 6)
                {
                    if (me->GetEntry() == NPC_SOUTH_GATE)
                        plr->KilledMonsterCredit(NPC_SOUTH_GATE_CREDIT, ObjectGuid::Empty);
                    else
                        plr->KilledMonsterCredit(NPC_NORTH_GATE_CREDIT, ObjectGuid::Empty);
        ...
        void UpdateAI(uint32 diff) override
        {
            if (timer)
            {
                ...
                {
                    for (uint8 i = 0; i < 3; ++i)
                        if (Creature* cr = me->SummonCreature((roll_chance_i(50) ? 22474 : 22501), me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), 0.0f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 30000))
                            if (Creature* cannon = GetClosestCreatureWithEntry(me, NPC_CANNON, 50.0f))                            
                                cr->AI()->AttackStart(cannon);
...
class spell_deaths_door_fel_cannon : public SpellScriptLoader
{
    public: spell_deaths_door_fel_cannon() : SpellScriptLoader("spell_deaths_door_fel_cannon") { }

        class spell_deaths_door_fel_cannon_AuraScript : public AuraScript
        {
            ...
            PrepareAuraScript(spell_deaths_door_fel_cannon_AuraScript);

            void OnApply(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
            {                
                PreventDefaultAction();
                Unit* target = GetTarget();
                if (Creature* cannon = GetClosestCreatureWithEntry(target, NPC_CANNON, 20.0f))
                {                        
                    cannon->SetCharmedBy(target, CHARM_TYPE_CHARM);
                    cannon->AddUnitState(UNIT_STATE_ROOT);
                }
            }
            ...
            void Register() override
            {
                OnEffectApply += AuraEffectApplyFn(spell_deaths_door_fel_cannon_AuraScript::OnApply, EFFECT_0, SPELL_AURA_MOD_CHARM, AURA_EFFECT_HANDLE_REAL);
                OnEffectRemove += AuraEffectRemoveFn(spell_deaths_door_fel_cannon_AuraScript::OnRemove, EFFECT_0, SPELL_AURA_MOD_CHARM, AURA_EFFECT_HANDLE_REAL);
            }
        };

        AuraScript* GetAuraScript() const override
        {
            return new spell_deaths_door_fel_cannon_AuraScript();
        }

        class spell_deaths_door_fel_cannon_SpellScript : public SpellScript
        {
            PrepareSpellScript(spell_deaths_door_fel_cannon_SpellScript);
            ...
            void Register() override
            {            
                OnEffectHitTarget += SpellEffectFn(spell_deaths_door_fel_cannon_SpellScript::HandleEffect, EFFECT_0, SPELL_EFFECT_APPLY_AURA);
            }
        };

        SpellScript* GetSpellScript() const override
        {
            return new spell_deaths_door_fel_cannon_SpellScript();
        }
};
...
void AddSC_blades_edge_mountains()
{
    ...
    new npc_deaths_door_wrap_gate();
    new spell_deaths_door_fel_cannon();
}
