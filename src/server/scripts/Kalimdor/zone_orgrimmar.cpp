...
#define POS_PORTAL_OG_TO_SW     1921.752441f, -4151.148438f, 40.623848f
...
public:
    ...
    bool OnTrigger(Player* player, AreaTriggerEntry const* /*trigger*/) override
    {
        ...
        if (player->IsAlive() && player->GetQuestStatus(QUEST_FATE_UP_AGAINST) == QUEST_STATUS_COMPLETE)
        {
            if (Creature* thrall = player->FindNearestCreature(NPC_THRALL_ALLIANCE, 50.0f))
            {
                thrall->AI()->DoAction(ACTION_START_SCENE);
                thrall->SummonGameObject(GO_OG_PORTAL_TO_SW, POS_PORTAL_OG_TO_SW, 0, 0, 0, 0, 0, 180 * IN_MILLISECONDS);
                return true;
            }
        }
        return false;
