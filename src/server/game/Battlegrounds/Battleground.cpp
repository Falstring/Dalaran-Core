                    ...
                    if (!player->IsGameMaster())
                    {
                        ...
                        for (Unit::AuraApplicationMap::iterator iter = auraMap.begin(); iter != auraMap.end();)
                        {
                            ...
                            if (!aura->IsPermanent()
                                ...
                                && aurApp->IsPositive()
                                //&& (!aura->GetSpellInfo()->HasAttribute(SPELL_ATTR0_UNAFFECTED_BY_INVULNERABILITY)) // optimization
...
void Battleground::RewardXPToTeam(uint32 winner, uint32 TeamID)
{
    uint32 bonus_extra = 2;
    if (winner == TeamID)
        bonus_extra = 6;

    for (BattlegroundPlayerMap::const_iterator itr = m_Players.begin(); itr != m_Players.end(); ++itr)
        if (Player* player = _GetPlayerForTeam(TeamID, itr, "RewardxpToTeam"))
        {
            uint32 base_exp = 10;
            ...
            player->GiveXP(level*base_exp*bonus_extra, NULL);
        }            
}
...
void Battleground::SetBracket(PvPDifficultyEntry const* bracketEntry)
{
	...
	SetLevelRange(bracketEntry->minLevel, bracketEntry->maxLevel >= 80 ? 80 : bracketEntry->maxLevel);
}
