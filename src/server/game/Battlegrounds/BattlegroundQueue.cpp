...
GroupQueueInfo* BattlegroundQueue::AddGroup(Player* leader, Group* grp, BattlegroundTypeId BgTypeId, PvPDifficultyEntry const*  bracketEntry, uint8 ArenaType, bool isRated, bool isPremade, uint32 ArenaRating, uint32 MatchmakerRating, uint32 arenateamid)
{
    ...
    if (isRated && sWorld->getBoolConfig(CONFIG_ARENA_QUEUE_ANNOUNCER_ENABLE))
    {
		if (BgTypeId == 6)
		{
			...
			for (uint8 j = 0; j <= 1; j++)
			{
				for (itr = m_QueuedGroups[14][j].begin(); itr != m_QueuedGroups[14][j].end(); ++itr)
				{					
					std::map<ObjectGuid, PlayerQueueInfo*>::const_iterator itr_players;
					// Announce for each player of that group the new team queing.
					for (itr_players = (*itr)->Players.begin(); itr_players != (*itr)->Players.end(); itr_players++)
					{
						if (Player* player = ObjectAccessor::FindPlayer(itr_players->first))
						{							
							if (Team)
								ChatHandler(player->GetSession()).PSendSysMessage(LANG_ARENA_QUEUE_ANNOUNCE_WORLD_JOIN, Team->GetName().c_str(), ginfo->ArenaType, ginfo->ArenaType, ginfo->ArenaTeamRating);
						}
					}
				}
			}
    ...
    //add GroupInfo to m_QueuedGroups
    {
        ...
        if (!isRated && !isPremade && sWorld->getBoolConfig(CONFIG_BATTLEGROUND_QUEUE_ANNOUNCER_ENABLE))
        {
            if (Battleground* bg = sBattlegroundMgr->GetBattlegroundTemplate(ginfo->BgTypeId))
            {
				...
				uint32 q_max_level = bracketEntry->maxLevel >= 80 ? 80 : bracketEntry->maxLevel;
                GroupsQueueType::const_iterator itr;
			...
			for (uint8 j = 0; j <= 1; j++)
			{
				...
				{					
					...
					for (itr_players = (*itr)->Players.begin(); itr_players != (*itr)->Players.end(); itr_players++)
					{
						...
						{							
							if (Team)
								ChatHandler(player->GetSession()).PSendSysMessage(LANG_ARENA_QUEUE_ANNOUNCE_WORLD_EXIT, Team->GetName().c_str(), group->ArenaType, group->ArenaType, group->ArenaTeamRating);
						}
					}
