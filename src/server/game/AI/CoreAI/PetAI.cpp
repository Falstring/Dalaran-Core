...
void PetAI::UpdateAI(uint32 diff)
{
    ...
    if (!me->HasUnitState(UNIT_STATE_CASTING))
    {
        ...
        for (uint8 i = 0; i < me->GetPetAutoSpellSize(); ++i)
        {
            ...
            // Ghoul energy never go below 40 when autocasting
            if (me->getPowerType() == POWER_ENERGY && me->GetPower(me->getPowerType()) <= 60
                && me->GetOwner() && me->GetOwner()->getClass() == CLASS_DEATH_KNIGHT)
                continue;
...
Unit* PetAI::SelectNextTarget(bool allowAutoSelect) const
{
    ...
    if (Unit* myAttacker = me->getAttackerForHelper())
        if (!myAttacker->HasBreakableByDamageCrowdControlAura() && !me->isTargetNotAcceptableByMMaps(myAttacker->GetGUID(), GameTime::GetGameTime(), myAttacker))
            return myAttacker;
    ...
    if (Unit* ownerAttacker = me->GetCharmerOrOwner()->getAttackerForHelper())
        if (!ownerAttacker->HasBreakableByDamageCrowdControlAura() && !me->isTargetNotAcceptableByMMaps(ownerAttacker->GetGUID(), GameTime::GetGameTime(), ownerAttacker))
            return ownerAttacker;

    // Check owner victim
    // 3.0.2 - Pets now start attacking their owners victim in defensive mode as soon as the hunter does
    if (Unit* ownerVictim = me->GetCharmerOrOwner()->GetVictim())
        if (!me->isTargetNotAcceptableByMMaps(ownerVictim->GetGUID(), GameTime::GetGameTime(), ownerVictim))
            return ownerVictim;
