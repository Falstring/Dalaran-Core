...
void SmartScript::ProcessAction(SmartScriptHolder& e, Unit* unit, uint32 var0, uint32 var1, bool bvar, const SpellInfo* spell, GameObject* gob)
{
    ...
    switch (e.GetActionType())
    {
                    ...
                    if (me)
                    {
                        ...
                        if (e.action.cast.flags & SMARTCAST_COMBAT_MOVE)
                        {
                            ...
                            if (me->GetDistance(*itr) > spellInfo->GetMaxRange(true) ||
                                ...
                                !me->GetSpellHistory()->IsSchoolLocked(SpellSchoolMask(spellInfo->SchoolMask)) ||
        ...
        case SMART_ACTION_SET_RESPAWN_TIME:
        {
            ObjectList* targets = GetTargets(e, unit);
            if (!targets)
                break;
            ...
            {
                if (IsCreature(*itr))
                    (*itr)->ToCreature()->SetRespawnTime(e.action.respawnTime.timer);
            }

            delete targets;
            break;
        }
        case SMART_ACTION_MOVE_HOME_POS:
        {
            if (!me)
                break;
        ...
        case SMART_ACTION_REMOVE_EVADE:
        {
            ObjectList* targets = GetTargets(e, unit);
            if (!targets)
                break;
            ...
            {
                if (IsCreature(*itr))
                    (*itr)->ToCreature()->ClearUnitState(UNIT_STATE_EVADE);;
            }
