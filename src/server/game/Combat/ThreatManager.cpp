...
void HostileReference::addThreatPercent(int32 percent)
{
    ...
    // also apply percentage to iTempThreatModifier to avoid strange results if temp threat is resetted
    AddPct(iTempThreatModifier, percent);
}
...
HostileReference* ThreatContainer::selectNextVictim(Creature* attacker, HostileReference* currentVictim) const
{
    ...
    if (currentVictim)
        if (attacker->isTargetNotAcceptableByMMaps(currentVictim->getTarget()->GetGUID(), GameTime::GetGameTime(), currentVictim->getTarget()))
    ...
    for (ThreatContainer::StorageType::const_iterator iter = iThreatList.begin(); iter != iThreatList.end() && !found;)
    {
        ...
        if (attacker->CanCreatureAttack(target) && !attacker->isTargetNotAcceptableByMMaps(target->GetGUID(), GameTime::GetGameTime(), target))
        {
