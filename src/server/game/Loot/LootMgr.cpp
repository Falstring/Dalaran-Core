...
enum NorthrendAchievementsAndEmblems
{
	...
	EMBLEM_HEROISM			= 40752,
	//EMBLEM_VALOR			= 40753,
	EMBLEM_CONQUEST			= 45624,
	EMBLEM_TRIUMPH			= 47241,
	//EMBLEM_FROST			= 49426,
};

bool IsEmblem(uint32 itemId)
{
	return (itemId == EMBLEM_HEROISM || itemId == EMBLEM_CONQUEST || itemId == EMBLEM_TRIUMPH);
}


uint32 GetEmblemBasedOnAchievement(Player const* player)
{
	if (player->HasAchieved(ACHIEVEMENT_TOC_10) || player->HasAchieved(ACHIEVEMENT_TOC_25))
		return EMBLEM_TRIUMPH;
	...
	if (player->HasAchieved(ACHIEVEMENT_NAXXRAMAS_10) || player->HasAchieved(ACHIEVEMENT_NAXXRAMAS_25))
		return EMBLEM_CONQUEST;

	return EMBLEM_HEROISM;
}
...
bool LootItem::AllowedForPlayer(Player const* player) const
{
	...
	// Northrend Heroic Dungeons Emblems
	if (IsEmblem(itemid))
		if (player->GetMap() && player->GetMap()->IsHeroic()) // Quitar los mapas: Frozen halls y ToC 5
			if (itemid != GetEmblemBasedOnAchievement(player))
				return false;
