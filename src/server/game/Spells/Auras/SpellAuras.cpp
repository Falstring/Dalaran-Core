...
Aura* Aura::TryRefreshStackOrCreate(SpellInfo const* spellproto, uint8 tryEffMask, WorldObject* owner, Unit* caster, int32* baseAmount /*= NULL*/, Item* castItem /*= NULL*/, ObjectGuid casterGUID /*= ObjectGuid::Empty*/, bool* refresh /*= NULL*/, bool periodicReset /*= false*/)
{
    ASSERT(spellproto);
    ASSERT(owner);
...
m_isRemoved(false), m_isSingleTarget(false), m_isUsingCharges(false), m_isStolen(false), m_dropEvent(nullptr)
{
...
void Aura::RefreshTimers(bool periodicReset)
{
...
bool Aura::CanBeSaved() const
{
    ...
    switch (GetId())
    {
		...
		case 63277: // Shadow Crash, Ulduar
        ...
        case SPELLFAMILY_PALADIN:
            ...
            if (GetSpellInfo()->GetSpellSpecific() == SPELL_SPECIFIC_AURA)
            {
                ...
                if ((GetSpellInfo()->SpellIconID != 555) && (caster->HasAura(53648) || caster->HasAura(53484) || caster->HasAura(53379) || caster->HasAura(31869)))
                {
                    ...
                    {
                        /// @todo: robinsch - this is a hack
                        // Aura should not be removed if target is still affected by another Paladin Aura and Paladin has Sanctified Retribution or Swift Retribution talents
                        // Determinates if aura should be remove
                        bool remove = true;
                        ...
                        for (Unit::AuraMap::const_iterator itr = auras.begin(); itr != auras.end(); ++itr)
                        {
                            ...
                            {
                                if (auraCaster->HasSpell(31869)) // Sanctified Retribution
                                {
                                    remove = false;
                                    break;
                                }
...
void Aura::CallScriptSteal(StealInfo* stealInfo)
{
    for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin(); scritr != m_loadedScripts.end(); ++scritr)
    {
        (*scritr)->_PrepareScriptCall(AURA_SCRIPT_HOOK_STEAL);
        std::list<AuraScript::AuraStealHandler>::iterator hookItrEnd = (*scritr)->OnSteal.end(), hookItr = (*scritr)->OnSteal.begin();
        for (; hookItr != hookItrEnd; ++hookItr)
            hookItr->Call(*scritr, stealInfo);

        (*scritr)->_FinishScriptCall();
    }
}

void Aura::CallScriptAfterSteal(StealInfo* stealInfo)
{
    for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin(); scritr != m_loadedScripts.end(); ++scritr)
    {
        (*scritr)->_PrepareScriptCall(AURA_SCRIPT_HOOK_AFTER_STEAL);
        std::list<AuraScript::AuraStealHandler>::iterator hookItrEnd = (*scritr)->AfterSteal.end(), hookItr = (*scritr)->AfterSteal.begin();
        for (; hookItr != hookItrEnd; ++hookItr)
            hookItr->Call(*scritr, stealInfo);
