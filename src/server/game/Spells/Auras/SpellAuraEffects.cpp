...
void AuraEffect::CalculatePeriodic(Unit* caster, bool create, bool load)
{
    ...
    else // aura just created or reapplied
    {
        ...
        // @robinsch: Don't reset periodic tick for special cases
        if (resetPeriodicTimer)
        {
            switch (m_spellInfo->SpellFamilyName)
            {
                case SPELLFAMILY_DRUID:
                    if (m_spellInfo->SpellFamilyFlags[1] & 0x00000100) // Lacerate
...
void AuraEffect::HandleAuraModShapeshift(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (apply)
    {
        ...
        if (GetSpellInfo()->SpellFamilyName != SPELLFAMILY_ROGUE) // Shadow Dance can stack with other shapeshift
...
void AuraEffect::HandlePeriodicHealAurasTick(Unit* target, Unit* caster) const
{
    ...
    if (GetAuraType() == SPELL_AURA_OBS_MOD_HEALTH && !gain)
        return;
