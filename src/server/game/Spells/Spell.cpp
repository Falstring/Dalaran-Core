...
void Spell::SelectSpellTargets()
{
    ...
    if (m_targets.HasDst())
    {
        if (m_targets.HasTraj())
        {
            ...
            m_delayMoment = CalculateDelayMoment(m_targets.GetDist2d());
        }
        else
        {
            float dist = m_caster->GetDistance(*m_targets.GetDstPos());
            m_delayMoment = CalculateDelayMoment(dist);
        }
    }
}
...
void Spell::SelectImplicitCasterDestTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType)
{
    ...
    switch (targetType.GetTarget())
    {
        ...
        default:
        {
			...
			// Hack: Lightning Tendrils (Visual) from Brundir, for some reason.
			if (m_spellInfo->Id == 61884)
				pos.m_positionZ = 427.28f;
...
void Spell::SelectEffectTypeImplicitTargets(uint8 effIndex)
{
    ...
    switch (m_spellInfo->Effects[effIndex].Effect)
    {
        ...
        case SPELL_EFFECT_SUMMON_PLAYER:
			...
			{
				if (m_spellInfo->Id == 48955 || m_spellInfo->Id == 45927) // Refer A Friend
				{
					...
					if (Group* group = m_caster->ToPlayer()->GetGroup())
						for (GroupReference* itr = group->GetFirstMember(); itr != nullptr; itr = itr->next())
						{
							...
							bool ARecruitedB = (player->GetSession()->GetRecruiterId() == m_caster->ToPlayer()->GetSession()->GetAccountId());
							bool BRecruitedA = (m_caster->ToPlayer()->GetSession()->GetRecruiterId() == player->GetSession()->GetAccountId());
							if (ARecruitedB || BRecruitedA)
							{
								float x, y, z;
								m_caster->GetClosePoint(x, y, z, player->GetObjectSize());
								player->TeleportTo(m_caster->GetMapId(), x, y, z, player->GetOrientation());
								AddUnitTarget(player->ToUnit(), 1 << effIndex, false);
								return;
							}
						}
				}
				else if (m_caster->GetTarget())
				{
...
void Spell::SearchChainTargets(std::list<WorldObject*>& targets, uint32 chainTargets, WorldObject* target, SpellTargetObjectTypes objectType, SpellTargetCheckTypes selectType, ConditionContainer* condList, bool isChainHeal)
{
	...
	// Thorim - Chain lighting bouncing radius correction.
	if (m_spellInfo->Id == 62131 || m_spellInfo->Id == 64390)
		jumpRadius = 4.0f;
...
void Spell::AddUnitTarget(Unit* target, uint32 effectMask, bool checkIfValid /*= true*/, bool implicit /*= true*/, Position const* losPosition /*= nullptr*/)
{
    ...
    if (m_caster != target)
    {
        ...
        if (dist < 5.0f)
            dist = 5.0f;
        targetInfo.timeDelay = CalculateDelayMoment(dist);
...
void Spell::AddGOTarget(GameObject* go, uint32 effectMask)
{
    ...
    if (dist < 5.0f)
        dist = 5.0f;
    target.timeDelay = CalculateDelayMoment(dist);
    ...
    if (aura_effmask)
    {
        ...
        if (m_originalCaster)
        {
            ...
            m_spellAura = Aura::TryRefreshStackOrCreate(aurSpellInfo, effectMask, unit,
                m_originalCaster, (aurSpellInfo == m_spellInfo) ? &m_spellValue->EffectBasePoints[0] : &basePoints[0], m_CastItem, ObjectGuid::Empty, &refresh, !(_triggeredCastFlags & TRIGGERED_NO_PERIODIC_RESET));
            if (m_spellAura)
            {
...
bool Spell::UpdateChanneledTargetList()
{
    ...
    if (channelAuraMask)
    {
        range = m_spellInfo->GetMaxRange(m_spellInfo->IsPositive()) * 1.2f;
        if (Player* modOwner = m_caster->GetSpellModOwner())
...
void Spell::prepare(SpellCastTargets const* targets, AuraEffect const* triggeredByAura)
{
    ...
    if (m_spellInfo->HasAttribute(SPELL_ATTR6_SUSPEND_ATTACK_TIMER))
        m_caster->suspendAttackTimer(true);
...
void Spell::cast(bool skipCheck)
{
    ...
    if (((m_delayMoment || m_spellInfo->Speed > 0.0f) && !m_spellInfo->IsChanneled()) || m_spellInfo->HasAttribute(SPELL_ATTR4_UNK4))
    {
        ...
        if (m_caster->GetSpellModOwner())
            m_caster->GetSpellModOwner()->RemoveSpellMods(this);
...
void Spell::finish(bool ok)
{
    ...
    // Slam
    if (m_spellInfo->HasAttribute(SPELL_ATTR6_SUSPEND_ATTACK_TIMER))
        m_caster->suspendAttackTimer(false);
...
void Spell::TakeRunePower(bool didHit)
{
    ...
    for (uint32 i = 0; i < MAX_RUNES; ++i)
    {
        ...
        {
            player->SetRuneCooldown(i, didHit ? player->GetRuneBaseCooldown(i, false) : uint32(RUNE_MISS_COOLDOWN), true);
    ...
    if (player->HasAura(70656))
    {
        bool allRunesOnCD = true;
        for (uint8 i = 0; i < MAX_RUNES; ++i)
            if (player->GetRuneCooldown(i) == 0)
                allRunesOnCD = false;

        if (allRunesOnCD)
            player->CastSpell(player, 70657, true);
    }
...
SpellCastResult Spell::CheckCast(bool strict)
{
    ...
    if (Unit* target = m_targets.GetUnitTarget())
    {
        ...
        if (target != m_caster)
        {
            ...
            if (m_caster->GetEntry() != WORLD_TRIGGER) // Ignore LOS for gameobjects casts (wrongly cast by a trigger)
            {
                ...
                if (!m_caster->IsTotem() && !m_spellInfo->HasAttribute(SPELL_ATTR2_CAN_TARGET_NOT_IN_LOS) && !DisableMgr::IsDisabledFor(DISABLE_TYPE_SPELL, m_spellInfo->Id, NULL, SPELL_DISABLE_LOS) && !target->IsWithinLOSInMap(losTarget))
    ...
    // Hack - Will fix Bestial Wrath and Mend pet issues.
    ...
    // Vanish - Need improvements, silence, stunned or something.  27617 1856
	...
	if (m_spellInfo->Id == 48955 || m_spellInfo->Id == 45927) // Refer A Friend hack, this needs to be done properly.
	{

		bool requireSelection = m_spellInfo->Id != 48955; // Refer a Friend Summon
		if (m_caster->GetTypeId() != TYPEID_PLAYER)
			return SPELL_FAILED_BAD_TARGETS;
		/*if (requireSelection && !m_caster->GetTarget())
			return SPELL_FAILED_BAD_TARGETS;*/

		Player* target = ObjectAccessor::FindPlayer(m_caster->ToPlayer()->GetTarget());
		if (requireSelection && (!target || m_caster->ToPlayer() == target || !target->IsInSameRaidWith(m_caster->ToPlayer())))
			return SPELL_FAILED_BAD_TARGETS;
		...
		bool ARecruitedB = (target->GetSession()->GetRecruiterId() == m_caster->ToPlayer()->GetSession()->GetAccountId());
		bool BRecruitedA = (m_caster->ToPlayer()->GetSession()->GetRecruiterId() == target->GetSession()->GetAccountId());
        ...
        switch (m_spellInfo->Effects[i].Effect)
        {
            ...
            case SPELL_EFFECT_CHARGE:
            {
				...
				if (GetSpellInfo()->NeedsExplicitUnitTarget())
				{
					...
					Position pos = target->GetFirstCollisionPosition(2.6f/*get closer to target, should prevent crappy behavior*/, target->GetRelativeAngle(m_caster));
                    float range = m_spellInfo->GetMaxRange(true) * 1.5f;
                    //Temporary Hack for Glyph of Charge.
                    if (m_caster->HasAura(58097))
                        range += 5.0f;
					m_preGeneratedPath.SetPathLengthLimit(range);
					float targetObjectSize = target->GetObjectSize() >= 2.0f ? std::min(target->GetObjectSize(), 2.5f) : target->GetObjectSize() + 1.0f;
					bool result = m_preGeneratedPath.CalculatePath(pos.m_positionX, pos.m_positionY, pos.m_positionZ + targetObjectSize, false, true);
					if (m_preGeneratedPath.GetPathType() & PATHFIND_SHORT)
						return SPELL_FAILED_OUT_OF_RANGE;
					else if (!result || m_preGeneratedPath.GetPathType() & (PATHFIND_NOPATH | PATHFIND_INCOMPLETE))
					{
						//Si éste resulta raro, pues directamente poner
...
SpellCastResult Spell::CheckCasterAuras() const
{
    ...
    if (unitflag & UNIT_FLAG_STUNNED)
    {
        ...
        if (usableInStun)
        {
            ...
            ///@robinsch: Hack to fix being able to use glyphed Pain Supression in Polymorph
    ...
    // Force prevented reason even if spell cast grant immunity for SPELL_ATTR0_CU_FORCE_PREVENTED_REASON
    if (m_spellInfo->HasAttribute(SPELL_ATTR0_CU_FORCE_PREVENTED_REASON) && prevented_reason != SPELL_CAST_OK)
        return prevented_reason;
...
bool Spell::CheckEffectTarget(Unit const* target, uint32 eff, Position const* losPosition) const
{
    ...
    if (IsTriggered())
    {
        if (m_triggeredByAuraSpell && (m_triggeredByAuraSpell->HasAttribute(SPELL_ATTR2_CAN_TARGET_NOT_IN_LOS) || DisableMgr::IsDisabledFor(DISABLE_TYPE_SPELL, m_triggeredByAuraSpell->Id, NULL, SPELL_DISABLE_LOS)))
            return true;
...
void Spell::DoAllEffectOnLaunchTarget(TargetInfo& targetInfo, float* multiplier)
{
        ...
        if (targetInfo.effectMask & (1<<i))
        {
            ...
            if (m_damage > 0)
            {
                if (m_spellInfo->Effects[i].IsTargetingArea() || m_spellInfo->Effects[i].IsAreaAuraEffect() || m_spellInfo->Effects[i].IsEffect(SPELL_EFFECT_PERSISTENT_AREA_AURA) ||
					(m_spellInfo->SpellFamilyName == SPELLFAMILY_DRUID && m_spellInfo->SpellFamilyFlags[1] & 0x00800000))
                {
    ...
    // Check m_originalCaster crit chance if exists
    Unit* caster = m_caster;
    if (m_originalCaster)
        caster = m_originalCaster;

    targetInfo.crit = caster->IsSpellCrit(unit, m_spellInfo, m_spellSchoolMask, m_attackType);
}
...
uint64 Spell::CalculateDelayMoment(float distance)
{
    /// @todo: robinsch - move this to database
    switch (m_spellInfo->SpellFamilyName)
    {
        case SPELLFAMILY_PALADIN:
            // Exorcism
            if (m_spellInfo->SpellFamilyFlags[1] & 0x2)
                return m_caster->IsInCombat() ? 0 : 250;
            break;
        default:
            break;
    }

    return ((m_spellInfo->Speed) > 0.0f ? (uint64)std::floor(distance / m_spellInfo->Speed * 1000.0f) : 0);
}
