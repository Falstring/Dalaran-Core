...
{
	const std::string forbiddenChars = "\\/:;¿?\"<>|.,-+*#()[]{}'\"@~`^!¡&%$·¬=¨´ç_";
...
{
	/*if (sSuspectMgr->IsSuspectIP(GetRemoteAddress()) || sSuspectMgr->IsSuspectedForOpcode(_player->GetGUID(), CMSG_SEND_MAIL))
		return;

	sSuspectMgr->OnOpcode(_player, CMSG_SEND_MAIL, getMSTime());*/
	...
	// Character with minimum level requirementon account.

	if (player->getLevel() < 20)
	{
		CharacterInfo const* charInfo = sWorld->GetCharacterInfo(player->GetGUID());

		if (charInfo && charInfo->AccountId)
		{

			PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_ACCOUNT_CHARACTERS);
			stmt->setInt32(0, charInfo->AccountId);

			PreparedQueryResult result = CharacterDatabase.Query(stmt);

			if (!result)
			{
				SendNotification("In order to avoid spam, you must have at least a level 20 character on this account before being able to send mails.");
	...
	// Check the web Okogames

	const uint8 cheksSize = 4;
	...
	std::string tmpsubject = subject;

	std::transform(tmpsubject.begin(), tmpsubject.end(), tmpsubject.begin(), ::tolower);

	tmpsubject = stringWithAccentsChangedToNormalLetters(tmpsubject);
	std::transform(tmpsubject.begin(), tmpsubject.end(), tmpsubject.begin(), ClearForbidden);
	tmpsubject.erase(std::remove_if(tmpsubject.begin(), tmpsubject.end(), isspace), tmpsubject.end());

	for (int i = 0; i < cheksSize; ++i)
	{
		if (tmpsubject.find(checks[i]) != std::string::npos)
		{
	...
	std::transform(tmpbody.begin(), tmpbody.end(), tmpbody.begin(), ::tolower);

	tmpbody = stringWithAccentsChangedToNormalLetters(tmpbody);
	std::transform(tmpbody.begin(), tmpbody.end(), tmpbody.begin(), ClearForbidden);
	tmpbody.erase(std::remove_if(tmpbody.begin(), tmpbody.end(), isspace), tmpbody.end());

	for (int i = 0; i < cheksSize; ++i)
	{
		if (tmpbody.find(checks[i]) != std::string::npos)
		{
			std::string accountName;
			AccountMgr::GetName(player->GetSession()->GetAccountId(), accountName);
			sWorld->BanAccount(BAN_ACCOUNT, accountName, "30d", "Gold selling, probably not the account owner.", "Server");
	...
	// Log mails from certain account
	switch (GetAccountId())
	{
        case 157262:
        case 293098:
		...
		case 205139:
        {
            TC_LOG_TRACE("trade.mail", "Player %s from account %u has sent a mail to player %s from account %u. Money: %u gold. Subject: \" %s \" - Body: \" %s \" ", player->GetName(), GetAccountId(), receiverName.c_str(), receiverAccountId, (money / 100) / 100, subject, body);
        }
        default:
            break;
	}
