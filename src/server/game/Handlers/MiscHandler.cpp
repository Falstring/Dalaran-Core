...
void WorldSession::HandleWhoOpcode(WorldPacket& recvData)
{
	...
	/*if (sSuspectMgr->IsSuspectIP(GetRemoteAddress()) || sSuspectMgr->IsSuspectedForOpcode(_player->GetGUID(), CMSG_WHO))
		return;

	sSuspectMgr->OnOpcode(_player, CMSG_WHO, getMSTime());*/
	...
	for (WhoListPlayerInfo const& target : whoList)
	{
		...
		std::string aname;
		if (AreaTableEntry const* areaEntry = GetAreaEntryByAreaID(playerZoneId))
...
void WorldSession::HandleSetRaidDifficultyOpcode(WorldPacket& recvData)
{
	...
	if (map->GetId() == 631)
    {
		return; // Can't set the difficulty while inside ICC.
        ...
        return; // ojo, este return.*/
    }

	// cannot reset while in an instance
	if (map && map->IsDungeon())
	{
		TC_LOG_DEBUG("network", "WorldSession::HandleSetRaidDifficultyOpcode: player %d tried to reset the instance while inside!", _player->GetName().c_str(), _player->GetGUID().GetCounter());
		return;
	}
