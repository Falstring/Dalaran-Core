...
void VisibleNotifier::SendToSelf()
{
    ...
    for (auto it = vis_guids.begin(); it != vis_guids.end(); ++it)
    {
        ...
        //Hack, inside the Frostmourne Chamber, do not erase Arthas Precipice go from players visibility.
        if (i_player.GetAreaId() == 4910)
        if (WorldObject* obj = ObjectAccessor::GetWorldObject(i_player, *it))        
            if (obj->ToGameObject())            
                if (obj->ToGameObject()->GetEntry() == 202078)
                    continue;
