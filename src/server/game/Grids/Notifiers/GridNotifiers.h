...
namespace Trinity
{
        ...
        public:
            ...
            bool operator()(Unit* u)
            {
                ...
                if (i_funit->_IsValidAttackTarget(u, _spellInfo, i_obj->GetTypeId() == TYPEID_DYNAMICOBJECT ? i_obj : NULL) && i_obj->IsWithinDistInMap(u, i_range) &&
					((i_obj->GetTypeId() == TYPEID_DYNAMICOBJECT && u->GetMapId() != 631) ? i_obj->GetDistanceZ(u) < 3.0f : true))
                    return true;
    ...
    */
    ...
    class AllCreaturesInRangeInCombat
    {
    public:
        AllCreaturesInRangeInCombat(const WorldObject* object, float maxRange) : m_pObject(object), m_fRange(maxRange) { }
        bool operator() (Unit* unit)
        {
            if (unit->IsInCombat() && m_pObject->IsWithinDist(unit, m_fRange, false))
                return true;
