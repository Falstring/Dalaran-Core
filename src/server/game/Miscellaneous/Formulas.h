...
namespace Trinity
{
    namespace Honor
    {
        inline float hk_honor_at_level_f(uint8 level, float multiplier = 1.0f)
        {
            ...
            //sScriptMgr->OnHonorCalculation(honor, level, multiplier); // optimization
            return honor;
        }
    ...
    namespace XP
    {
        inline uint8 GetGrayLevel(uint8 pl_level)
        {
            ...
            //sScriptMgr->OnGrayLevelCalculation(level, pl_level); // optimization
            return level;
        }
        ...
        inline XPColorChar GetColorCode(uint8 pl_level, uint8 mob_level)
        {
            ...
            //sScriptMgr->OnColorCodeCalculation(color, pl_level, mob_level); // optimization.
            return color;
        }
        ...
        inline uint8 GetZeroDifference(uint8 pl_level)
        {
            ...
            //sScriptMgr->OnZeroDifferenceCalculation(diff, pl_level); // optimization
            return diff;
        }
        ...
        inline uint32 BaseGain(uint8 pl_level, uint8 mob_level, ContentLevels content)
        {
            ...
            //sScriptMgr->OnBaseGainCalculation(baseGain, pl_level, mob_level, content); // optimization
            return baseGain;
        }
        ...
        inline uint32 Gain(Player* player, Unit* u, bool isBattleGround = false)
        {
            ...
            //sScriptMgr->OnGainCalculation(gain, player, u); // optimization
            return gain;
        }
        ...
        inline float xp_in_group_rate(uint32 count, bool isRaid)
        {
            ...
            //sScriptMgr->OnGroupRateCalculation(rate, count, isRaid); // optimization
            return rate;
        }
