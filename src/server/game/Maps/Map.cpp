...
float Map::GetHeight(float x, float y, float z, bool checkVMap /*= true*/, float maxSearchDist /*= DEFAULT_HEIGHT_SEARCH*/) const
{
    ...
    if (checkVMap)
    {
        ...
        //if (vmgr->isHeightCalcEnabled()) // optimization
...
Map::EnterState InstanceMap::CannotEnter(Player* player)
{
	...
	if (GetId() == 631)
	{
		if (GetInstanceScript()->GetBossState(12) == IN_PROGRESS) // HACK: Icecrown Citadel, avoid exploit with Valkyrs.
			return CANNOT_ENTER_ZONE_IN_COMBAT;

		if (GetInstanceScript()->GetBossState(8) == IN_PROGRESS) // Blood-Queen, to avoid the essence exploit.
			return CANNOT_ENTER_ZONE_IN_COMBAT;

	}

	// HACK: Ulduar, avoid Insane exploit from Yogg-Saron.
	if (GetId() == 603)
		if (GetInstanceScript()->GetBossState(15) == IN_PROGRESS)
			return CANNOT_ENTER_ZONE_IN_COMBAT;
