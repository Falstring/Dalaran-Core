...
void LFGMgr::LeaveAllLfgQueues(ObjectGuid guid, bool allowGroup, ObjectGuid groupGuid)
{
    ...
    if (groupGuid && groupGuid.IsGroup())
    {
        pguid = guid;
        gguid = groupGuid;
    }
...
enum NorthrendAchievementsAndEmblems
{
	ACHIEVEMENT_NAXXRAMAS_10 = 576,
	ACHIEVEMENT_NAXXRAMAS_25 = 577,
	ACHIEVEMENT_ULDUAR_10 = 2894, // Secrets of Ulduar
	ACHIEVEMENT_ULDUAR_25 = 2895, // Secrets of Ulduar
	ACHIEVEMENT_TOC_10 = 3917,
	ACHIEVEMENT_TOC_25 = 3916,

	QUEST_HEROISM  = 24788,
	QUEST_CONQUEST = 45000,
	QUEST_TRIUMPH  = 45001,
	QUEST_FROST    = 45002,
};

uint32 GetQuestIdBasedOnAchievement(Player const* player)
{

	if (player->HasAchieved(ACHIEVEMENT_TOC_10) || player->HasAchieved(ACHIEVEMENT_TOC_25)) // Remove this for the releae.
		return QUEST_FROST;
	if (player->HasAchieved(ACHIEVEMENT_ULDUAR_10) || player->HasAchieved(ACHIEVEMENT_ULDUAR_25))
		return QUEST_TRIUMPH;
	if (player->HasAchieved(ACHIEVEMENT_NAXXRAMAS_10) || player->HasAchieved(ACHIEVEMENT_NAXXRAMAS_25))
		return QUEST_CONQUEST;

	return QUEST_HEROISM;
}
...
void LFGMgr::FinishDungeon(ObjectGuid gguid, const uint32 dungeonId)
{
    ...
    for (GuidSet::const_iterator it = players.begin(); it != players.end(); ++it)
    {
		...
		//Si es la quest 24788 Daily Heroic Random (1st), entonces analizar los achievements y completar quests custom agregadas en DB, especificar los emblemas ahi

		if (quest->GetQuestId() == QUEST_HEROISM)
		{
			uint32 newQuestId = GetQuestIdBasedOnAchievement(player);
			Quest const* properQuest = sObjectMgr->GetQuestTemplate(newQuestId);
			if (!properQuest)
				continue;

			if (player->CanRewardQuest(properQuest, false))
				player->RewardQuest(properQuest, 0, NULL, false);
		}
		else if (player->CanRewardQuest(quest, false)) // if we can take the quest, means that we haven't done this kind of "run", IE: First Heroic Random of Day.
...
LfgLockMap const LFGMgr::GetLockedDungeons(ObjectGuid guid)
{
    ...
    for (LfgDungeonSet::const_iterator it = dungeons.begin(); it != dungeons.end(); ++it)
    {
		...
		else if (dungeon->map == 632) // Only Forge of Souls
		{
			if (!player->HasAchieved(3917) && !player->HasAchieved(3916)) // ToC 10 / 25
				lockData = LFG_LOCKSTATUS_MISSING_ACHIEVEMENT;
		}
		else if (dungeon->map == 650) // Trial of the Champion
		{
			if (!player->HasAchieved(2891) && !player->HasAchieved(2890)) // The Keepers of Ulduar 10 / 25
				lockData = LFG_LOCKSTATUS_MISSING_ACHIEVEMENT;
		}
