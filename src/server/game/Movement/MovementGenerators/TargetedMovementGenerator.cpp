...
void TargetedMovementGeneratorMedium<T, D>::_setTargetLocation(T* owner, bool updateDestination)
{
    ...
    if (updateDestination || !i_path)
    {
        if (!i_offset)
        {
            ...
            if (i_target->IsWithinMeleeRange(owner, MELEE_RANGE) && i_target->IsWithinLOSInMap(owner) && owner->movespline->Finalized() && GetMovementGeneratorType() == CHASE_MOTION_TYPE)
                return;
