...
bool BattlefieldWG::SetupBattlefield()
{
    ...
    //Manually spawn banners before Workshop update.
    SpawnBanners();
...
void BattlefieldWG::OnBattleEnd(bool endByTimer)
{
	...
	uint32 damagedDef = endByTimer ? SPELL_DAMAGED_TOWER : SPELL_DAMAGED_BUILDING;
	uint32 brokenDef = endByTimer ? SPELL_DESTROYED_TOWER : SPELL_INTACT_BUILDING;
	uint32 damagedAtt = endByTimer ? SPELL_DAMAGED_BUILDING : SPELL_DAMAGED_TOWER;
	uint32 brokenAtt = endByTimer ? SPELL_INTACT_BUILDING : SPELL_DESTROYED_TOWER;
	uint32 damagedTowerDef = endByTimer ? GetData(BATTLEFIELD_WG_DATA_DAMAGED_TOWER_ATT) : GetData(BATTLEFIELD_WG_DATA_DAMAGED_TOWER_DEF);
	uint32 brokenTowerDef = endByTimer ? GetData(BATTLEFIELD_WG_DATA_BROKEN_TOWER_ATT) : GetData(BATTLEFIELD_WG_DATA_BROKEN_TOWER_DEF);
	uint32 damagedTowerAtt = endByTimer ? GetData(BATTLEFIELD_WG_DATA_DAMAGED_TOWER_DEF) : GetData(BATTLEFIELD_WG_DATA_DAMAGED_TOWER_ATT);
	uint32 brokenTowerAtt = endByTimer ? GetData(BATTLEFIELD_WG_DATA_BROKEN_TOWER_DEF) : GetData(BATTLEFIELD_WG_DATA_BROKEN_TOWER_ATT);

	// This way players shall get their reward even if they are death.

	// Victory

	for (GuidSet::const_iterator itr = m_players[GetDefenderTeam()].begin(); itr != m_players[GetDefenderTeam()].end(); ++itr)
    {
        ...
        {			
			...
			CastRewardSpell(player, damagedDef, damagedTowerDef);
			CastRewardSpell(player, brokenDef, brokenTowerDef);
	...
	for (GuidSet::const_iterator itr = m_players[GetAttackerTeam()].begin(); itr != m_players[GetAttackerTeam()].end(); ++itr)
	{
		...
		{			
			...
			CastRewardSpell(player, damagedAtt, damagedTowerAtt);
			CastRewardSpell(player, brokenAtt, brokenTowerAtt);
			RemoveAurasFromPlayer(player);
		}		
	}
    ...
    // Update Archavon's Log
    WintergraspWorldStates logEntry;
    if (!endByTimer) // Captured (Now defending team is victorious)
...
void BattlefieldWG::CreateCapturePoint(GameObject* go, WintergraspWorkshop* workshop, TeamId teamControl)
{
    WintergraspCapturePoint* capturePoint = new WintergraspCapturePoint(this, teamControl);
...
uint32 BattlefieldWG::GetData(uint32 data) const
{
    switch (data)
	{
		...
		case AREA_THE_CHILLED_QUAGMIRE:
			if (BfGraveyard const* graveyard = GetGraveyardById(BATTLEFIELD_WG_GY_WORKSHOP_NW))
...
{
    WorldPacket data(SMSG_INIT_WORLD_STATES, (4 + 4 + 4 + 2 + (BuildingsInZone.size() * 8) + (Workshops.size() * 8) + 4 * 2 * 4));

    data << uint32(m_MapId);
    data << uint32(m_ZoneId);
    data << uint32(0);                                              // AreaId
    data << uint16(10 + BuildingsInZone.size() + Workshops.size() + 4 * 2 * 4); // Number of fields
...
void BattlefieldWG::BrokenWallOrTower(TeamId teamId)
{
...
/*    if (team == GetDefenderTeam())
	...
	if (teamId == GetDefenderTeam())
	{
		for (GuidSet::const_iterator itr = m_PlayersInWar[GetDefenderTeam()].begin(); itr != m_PlayersInWar[GetDefenderTeam()].end(); ++itr)
		{
			if (Player* player = ObjectAccessor::FindPlayer(*itr))
				player->KilledMonsterCredit(CREDIT_TOWERS, ObjectGuid::Empty);
		}
...
void BattlefieldWG::UpdateTenacity()
{
    ...
    {
        for (GuidSet::const_iterator itr = m_PlayersInWar[m_tenacityTeam].begin(); itr != m_PlayersInWar[m_tenacityTeam].end(); ++itr)
            if (Player* player = ObjectAccessor::FindPlayer(*itr))            
                player->RemoveAurasDueToSpell(SPELL_TENACITY);            

        for (GuidSet::const_iterator itr = m_vehicles[m_tenacityTeam].begin(); itr != m_vehicles[m_tenacityTeam].end(); ++itr)
            if (Creature* creature = GetCreature(*itr))
...
ObjectGuid BattlefieldWG::GetBannerData(uint32 data)
{
    switch (data)
    {
    case GO_WINTERGRASP_FACTORY_BANNER_NE:
        return m_workshopBannerNEGUID;
    case GO_WINTERGRASP_FACTORY_BANNER_NW:
        return m_workshopBannerNWGUID;
    case GO_WINTERGRASP_FACTORY_BANNER_SE:
        return m_workshopBannerSEGUID;
    case GO_WINTERGRASP_FACTORY_BANNER_SW:
        return m_workshopBannerSWGUID;
    default:
        break;
    }
...
void BattlefieldWG::SpawnBanners()
{
    if (GameObject* bannerNE = GetGameObject(m_workshopBannerNEGUID))
        bannerNE->RemoveFromWorld();

    if (GameObject* bannerNW = GetGameObject(m_workshopBannerNWGUID))
        bannerNW->RemoveFromWorld();

    if (GameObject* bannerSE = GetGameObject(m_workshopBannerSEGUID))
        bannerSE->RemoveFromWorld();

    if (GameObject* bannerSW = GetGameObject(m_workshopBannerSWGUID))
        bannerSW->RemoveFromWorld();

    SpawnGameObject(GO_WINTERGRASP_FACTORY_BANNER_NE, 4949.34f, 2432.59f, 320.177, 1.38621f);
    SpawnGameObject(GO_WINTERGRASP_FACTORY_BANNER_NW, 4948.52f, 3342.34f, 376.875f, 4.40057f);
    SpawnGameObject(GO_WINTERGRASP_FACTORY_BANNER_SE, 4398.08f, 2356.5f, 376.19f, 0.525406f);
    SpawnGameObject(GO_WINTERGRASP_FACTORY_BANNER_SW, 4390.78f, 3304.09f, 372.429f, 6.09702f);
}

void BattlefieldWG::CastRewardSpell(Player* player, uint32 spellEntry, uint32 amount)
{
...
void WintergraspWorkshop::GiveControlTo(TeamId teamId, bool init /*= false*/)
{
    ...
    if (!init)
    {
        ...
        _wg->CapturePointTaken(_wg->GetAreaByGraveyardId(_staticInfo->WorkshopId));
    }        
}

void WintergraspWorkshop::LinkWorkshopData(uint32 bannerData, TeamId team)
{

    WintergraspCapturePoint* capturePoint = new WintergraspCapturePoint(_wg, team);
    
    if (GameObject* banner = _wg->GetGameObject(_wg->GetBannerData(bannerData)))
        _wg->CreateCapturePoint(banner, this, team);               
    
    _wg->GetGraveyardById(_staticInfo->WorkshopId)->GiveControlTo(team);
...
{
    switch (_staticInfo->WorkshopId)
    {
    case BATTLEFIELD_WG_WORKSHOP_NE:        
        LinkWorkshopData(GO_WINTERGRASP_FACTORY_BANNER_NE, _wg->GetDefenderTeam());
        break;
    case BATTLEFIELD_WG_WORKSHOP_NW:
        LinkWorkshopData(GO_WINTERGRASP_FACTORY_BANNER_NW, _wg->GetDefenderTeam());
        break;
    case BATTLEFIELD_WG_WORKSHOP_SE:
        LinkWorkshopData(GO_WINTERGRASP_FACTORY_BANNER_SE, _wg->GetAttackerTeam());
        break;
    case BATTLEFIELD_WG_WORKSHOP_SW:
        LinkWorkshopData(GO_WINTERGRASP_FACTORY_BANNER_SW, _wg->GetAttackerTeam());
        break;
