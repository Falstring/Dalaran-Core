...
void World::LoadConfigSettings(bool reload)
{
	...
	m_bool_configs[CONIFG_AVERAGE_UPDATE_DIFF_ENABLED] = sConfigMgr->GetBoolDefault("AverageUpdateDiff.Enabled", false);
	m_bool_configs[CONFIG_DYNAMIC_VISIBILITY_DISTANCE_ENABLED] = sConfigMgr->GetBoolDefault("DynamicVisibilityDistance.Enabled", false);
    ...
    m_int_configs[CONFIG_WINTERGRASP_PLR_MIN_LVL] = sConfigMgr->GetIntDefault("Wintergrasp.PlayerMinLvl", 75);
...
void World::Update(uint32 diff)
{
	...
	/// Average Update Diff
	if (m_bool_configs[CONIFG_AVERAGE_UPDATE_DIFF_ENABLED])
	{
		// delete old m_averageUpdateTimeTotal value

		m_averageUpdateTimeTotal += diff;
		m_averageUpdateTimeIndex++;

		if (m_averageUpdateTimeIndex >= AVERAGE_UPDATE_DIFF_COUNT)
		{
			m_averageUpdateTime = m_averageUpdateTimeTotal / AVERAGE_UPDATE_DIFF_COUNT;
			m_lastAverageUpdateTime = m_averageUpdateTime;
			m_averageUpdateTimeIndex = 0;
			m_averageUpdateTimeTotal = 0;
		}

	}

	/// Dynamic Visibility Distance System
	if (m_bool_configs[CONFIG_DYNAMIC_VISIBILITY_DISTANCE_ENABLED] && m_averageUpdateTime)
	{
		if (m_averageUpdateTime < 200)
		{
			sMapMgr->SetVisibilityDistanceInfo(100.0f);
			m_visibility_notify_periodOnContinents = 1000;
			sMapMgr->InitializeVisibilityDistanceInfo();
			m_averageUpdateTime = 0;
		}
		else if (m_averageUpdateTime >= 200 && m_averageUpdateTime < 450)
		{
			m_visibility_notify_periodOnContinents = 3000;
			m_MaxVisibleDistanceOnContinents = 65.0f;
			sMapMgr->SetVisibilityDistanceInfo(65.0f);
			sMapMgr->InitializeVisibilityDistanceInfo();
			m_averageUpdateTime = 0;
		}
		else if (m_averageUpdateTime >= 450)
		{
			/*ShutdownServ(0, 0, SHUTDOWN_EXIT_CODE);
			m_averageUpdateTime = 0;*/
		}
	}
