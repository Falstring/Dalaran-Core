...
class Player : public Unit, public GridObject<Player>
{
    ...
    public:
        ...
        Creature* GetCritter() const; // non-combat pet
        ...
        std::string GetOptionTextWithEntry(uint32 entry, uint32 id = 0);
        std::string GetOptionBoxTextWithEntry(uint32 entry, uint32 id = 0);
        uint32 GetOptionBoxMoneyWithEntry(uint32 entry, uint32 id = 0);
        void AddGossipItem(uint32 menu_id, uint32 id = 0, uint8 gossip_sender = 1/*GOSSIP_SENDER_MAIN*/, uint32 gossip_action = 1001/*GOSSIP_ACTION_INFO_DEF+1*/);
        ...
        uint32 GetTalentPointsInTab(uint8 Tab);
        bool IsHealer();
        bool IsTank();
        ...
        uint32 GetRuneGracePeriod(uint8 index) const { return m_runes->runes[index].GracePeriod; }
        ...
        int8 GetBloodTapRuneIndex() { return m_runes->bloodTapRuneIndex; }
        void SetBloodTapRuneIndex(int8 index) { m_runes->bloodTapRuneIndex = index; }
        void SetBaseRune(uint8 index, RuneType baseRune) { m_runes->runes[index].BaseRune = baseRune; }
        void SetCurrentRune(uint8 index, RuneType currentRune) { m_runes->runes[index].CurrentRune = currentRune; }
        void SetRuneCooldown(uint8 index, uint32 cooldown, bool casted = false) { m_runes->runes[index].Cooldown = cooldown; m_runes->SetRuneState(index, (cooldown == 0) ? true : false); }
        void SetRuneGracePeriod(uint8 index, uint32 period) { m_runes->runes[index].GracePeriod = period; }
        ...
        uint32 GetMusicId() const { return m_MusicId; }
