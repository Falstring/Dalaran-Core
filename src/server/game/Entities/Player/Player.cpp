...
#define MUSIC_UPDATE_INTERVAL (5*IN_MILLISECONDS)
...
void Player::Update(uint32 p_time)
{
    ...
    if (m_MusicId)
    {
        if (p_time >= m_MusicTimer)
        {
            PlayDirectMusic(m_MusicId, this);
            m_MusicTimer = MUSIC_UPDATE_INTERVAL;
        }
        else
            m_MusicTimer -= p_time;
    }
        ...
        for (uint8 i = 0; i < MAX_RUNES; ++i)
        {
            if (uint32 cd = GetRuneCooldown(i))
            {
                ...
                if (IsInCombat() && cd <= m_regenTimer)
                    SetRuneGracePeriod(i, std::max<uint32>(m_regenTimer - cd, 1));
            }
            else if (uint32 graceTimer = GetRuneGracePeriod(i))
            {
                if (graceTimer < RUNE_GRACE_PERIOD)
                    SetRuneGracePeriod(i, std::min<uint32>(graceTimer + m_regenTimer, RUNE_GRACE_PERIOD));
            }
        }
    }
...
void Player::RegenerateHealth()
{
    ...
    else if (!IsInCombat() || HasAuraType(SPELL_AURA_MOD_REGEN_DURING_COMBAT))
    {
        ...
        if (!IsInCombat())
        {
            ...
            addValue += GetTotalAuraModifier(SPELL_AURA_MOD_REGEN) * 0.25f;
        }
...
bool Player::AddSpell(uint32 spellId, bool active, bool learning, bool dependent, bool disabled, bool loading /*= false*/, uint32 fromSkill /*= 0*/)
{
    ...
    if (!disabled_case) // skip new spell adding if spell already known (disabled spells case)
    {
        ...
        /// @todo: robinsch - remove this. It seems like the above call doesn't work out for Primal Fury
        else if (spellInfo->HasAttribute(SPELL_ATTR0_CU_UNLEARN_PREVIOUS_SPELL))
        {
...
bool Player::ResetTalents(bool no_cost)
{
    ...
    _SaveTalents(trans);
    _SaveSpells(trans);
    CharacterDatabase.CommitTransaction(trans);*/ // optimization - Cuidado con esto. Bandera
...
// Amount of spent talent points in one Tab (0,1 or 2) the active spec
uint32 Player::GetTalentPointsInTab(uint8 tab) {

    if (tab > 2)
        return 0;

    uint32 playerClass = getClass();
    uint8 activespec = uint8(m_activeSpec); // active Specc
    uint32 const* talentTabIds = GetTalentTabPages(playerClass); // TabIds of this class
    uint32 talentTabId = talentTabIds[tab]; // CorrectTab

    uint32 amount = 0;

    for (uint32 talentId = 0; talentId < sTalentStore.GetNumRows(); ++talentId) // iterate through all talents
    {
        TalentEntry const* talentInfo = sTalentStore.LookupEntry(talentId); // find talent
        if (!talentInfo || (talentInfo->TalentTab != talentTabId)) // talent not found or not in correct tab
            continue;

        // check how many points the player has spent for this talent (0-5)
        int8 spent_points = 0;
        for (int8 rank = MAX_TALENT_RANK - 1; rank >= 0; --rank) // rank X = X+1 points spent
        {
            if (talentInfo->RankID[rank] && HasTalent(talentInfo->RankID[rank], activespec))
            {
                spent_points = rank + 1;
                break;
            }
...
// Returns true if the current spec of the player is a heal spec (more points in holy/disc/resto than in each other tab)
bool Player::IsHealer()
{
    // only paladins, priests, shamans and druids can be healers
    uint32 playerClass = getClass();
    if (playerClass != CLASS_PALADIN && playerClass != CLASS_PRIEST && playerClass != CLASS_SHAMAN && playerClass != CLASS_DRUID)
        return false;

    uint32 distrib[3];
    distrib[0] = GetTalentPointsInTab(0);
    distrib[1] = GetTalentPointsInTab(1);
    distrib[2] = GetTalentPointsInTab(2);

    switch (playerClass)
    {
        case CLASS_PALADIN: // holy
            return (distrib[0] > distrib[1] && distrib[0] > distrib[2]);
        case CLASS_PRIEST: // disc or holy
            return ((distrib[0] > distrib[1] && distrib[0] > distrib[2]) || (distrib[1] > distrib[0] && distrib[1] > distrib[2]));
        case CLASS_SHAMAN: // resto
            return (distrib[2] > distrib[0] && distrib[2] > distrib[1]);
        case CLASS_DRUID: // resto
            return (distrib[2] > distrib[0] && distrib[2] > distrib[1]);
        default:
            return false;
    }
...
// Returns true if the current spec of the player is a tank spec
bool Player::IsTank()
{
    // only paladins, warrior, dks and druids can be tanks
    uint32 playerClass = getClass();
    if (playerClass != CLASS_PALADIN && playerClass != CLASS_WARRIOR && playerClass != CLASS_DEATH_KNIGHT && playerClass != CLASS_DRUID)
        return false;
    ...
    switch (playerClass)
    {
        case CLASS_PALADIN: // Prot
            return (distrib[1] > distrib[0] && distrib[1] > distrib[2]);
        case CLASS_WARRIOR: // Prot
            return (distrib[2] > distrib[0] && distrib[2] > distrib[1]);
        case CLASS_DRUID: // Dire Bear Form
            return (HasAura(57877) && HasAura(16931) && distrib[1] > distrib[0] && distrib[1] > distrib[2]);
        case CLASS_DEATH_KNIGHT: // DK - Toughness + Anticipation
            return (HasAura(55129) || HasAura(55130) || HasAura(55131) || HasAura(55132) || HasAura(55133) ||
                HasAura(49042) || HasAura(49786) || HasAura(49787) || HasAura(49788) || HasAura(49789));
        default:
            return false;
    }
}
...
void Player::MoveItemToInventory(ItemPosCountVec const& dest, Item* pItem, bool update, bool in_characterInventoryDB)
{
    ...
    //SendNewItem(pItem, pItem->GetCount(), true, false, update);
...
void Player::RemoveArenaEnchantments(EnchantmentSlot slot)
{
    ...
    // check arena allowed enchantments
...
/**
* @fn std::string Player::GetOptionTextWithEntry(int32 entry)
*
* @brief Get texts from Database for GossipOptions.
*
* @param entry menu_id in gossip_menu_option and locales_gossip_menu_option
* @param2 id id in gossip_menu_option and locales_gossip_menu_option
*
* @return The string from DB in gossip_menu_option with menu_id = entry
*/
std::string Player::GetOptionTextWithEntry(uint32 entry, uint32 id)
{
    std::string strOptionText;

    GossipMenuItemsMapBounds menuItemBounds = sObjectMgr->GetGossipMenuItemsMapBounds(entry);
    if (menuItemBounds.first != menuItemBounds.second)
        for (GossipMenuItemsContainer::const_iterator itr = menuItemBounds.first; itr != menuItemBounds.second; ++itr)
            if ((*itr).second.MenuId == entry && (*itr).second.OptionIndex == id)
                strOptionText = (*itr).second.OptionText;

    int32 locale = GetSession()->GetSessionDbLocaleIndex();
    if (locale >= 0)
    {
        uint32 idxEntry = MAKE_PAIR32(entry, id);
        if (GossipMenuItemsLocale const* localeString = sObjectMgr->GetGossipMenuItemsLocale(idxEntry))
            ObjectMgr::GetLocaleString(localeString->OptionText, locale, strOptionText);
    }

    if (strOptionText == "")
    {
        strOptionText = "<error>";
        TC_LOG_ERROR("sql.sql", "(locales_)gossip_menu_option with menu_id %d and id %d don't result a string, but will called with GetOptionTextWithEntry.", entry, id);
    }
...
/**
* @fn std::string Player::GetOptionBoxTextWithEntry(int32 entry)
*
* @brief Get box texts from Database for GossipOptions.
*
* @param entry menu_id in gossip_menu_option and locales_gossip_menu_option
* @param2 id id in gossip_menu_option and locales_gossip_menu_option
*
* @return The box string from DB in gossip_menu_option with menu_id = entry
*/
std::string Player::GetOptionBoxTextWithEntry(uint32 entry, uint32 id)
{
    std::string strOptionBoxText;

    GossipMenuItemsMapBounds menuItemBounds = sObjectMgr->GetGossipMenuItemsMapBounds(entry);
    if (menuItemBounds.first != menuItemBounds.second)
        for (GossipMenuItemsContainer::const_iterator itr = menuItemBounds.first; itr != menuItemBounds.second; ++itr)
            if ((*itr).second.MenuId == entry && (*itr).second.OptionIndex == id)
                strOptionBoxText = (*itr).second.BoxText;
    ...
    if (locale >= 0)
    {
        uint32 idxEntry = MAKE_PAIR32(entry, id);
        if (GossipMenuItemsLocale const* localeString = sObjectMgr->GetGossipMenuItemsLocale(idxEntry))
            ObjectMgr::GetLocaleString(localeString->BoxText, locale, strOptionBoxText);
    }

    if (strOptionBoxText == "")
    {
        strOptionBoxText = "<error>";
        TC_LOG_ERROR("sql.sql", "(locales_)gossip_menu_option with menu_id %d and id %d don't result a string, but will called with GetOptionBoxTextWithEntry.", entry, id);
    }

    return strOptionBoxText;
}

/**
* @fn std::string Player::GetOptionBoxMoneyWithEntry(int32 entry)
*
* @brief Get box money from Database for GossipOptions.
*
* @param entry menu_id in gossip_menu_option and locales_gossip_menu_option
* @param2 id id in gossip_menu_option and locales_gossip_menu_option
*
* @return The box money from DB in gossip_menu_option with menu_id = entry
*/
uint32 Player::GetOptionBoxMoneyWithEntry(uint32 entry, uint32 id)
{
    GossipMenuItemsMapBounds menuItemBounds = sObjectMgr->GetGossipMenuItemsMapBounds(entry);
    if (menuItemBounds.first != menuItemBounds.second)
        for (GossipMenuItemsContainer::const_iterator itr = menuItemBounds.first; itr != menuItemBounds.second; ++itr)
            if ((*itr).second.MenuId == entry && (*itr).second.OptionIndex == id)
                return (*itr).second.BoxMoney;
    return 0;
}

void Player::AddGossipItem(uint32 menu_id, uint32 id/* = 0*/, uint8 gossip_sender/* = 1 GOSSIP_SENDER_MAIN*/, uint32 gossip_action/* = 1001 GOSSIP_ACTION_INFO_DEF+1*/)
{
    uint8 option_icon = 0;

    GossipMenuItemsMapBounds menuItemBounds = sObjectMgr->GetGossipMenuItemsMapBounds(menu_id);
    if (menuItemBounds.first != menuItemBounds.second)
        for (GossipMenuItemsContainer::const_iterator itr = menuItemBounds.first; itr != menuItemBounds.second; ++itr)
            if ((*itr).second.MenuId == menu_id && (*itr).second.OptionIndex == id)
                option_icon = (*itr).second.OptionIcon;

    std::string text;
    text = GetOptionTextWithEntry(menu_id, id);

    PlayerTalkClass->GetGossipMenu().AddMenuItem(-1, option_icon, text, gossip_sender, gossip_action, "", 0);
}
...
#define RelocateToHomebind(){ mapId = m_homebindMapId; instanceId = 0; Relocate(m_homebindX, m_homebindY, m_homebindZ); }
    ...
    // m_achievementMgr->CheckAllAchievementCriteria();// optimization
...
bool Player::Satisfy(AccessRequirement const* ar, uint32 target_map, bool report)
{
    if (!IsGameMaster() && ar)
    {
		...
		//Not leader, check it player, individually.
		...
		if (mapEntry->MapID == 631)
		{
			if (GetGroup())
			{
				ObjectGuid leaderGuid = GetGroup()->GetLeaderGUID();
				if (leaderGuid)
				{
					if (Player* leader = ObjectAccessor::FindPlayer(leaderGuid))
					{
						...
						case RAID_DIFFICULTY_10MAN_HEROIC:
							if (!leader->HasAchieved(4532))
							{
								ChatHandler(GetSession()).PSendSysMessage("%s", "The Raid leader must have completed Fall of the Lich King (10 players) first.");
								return false;
							}
							break;
						case RAID_DIFFICULTY_25MAN_HEROIC:
							if (!leader->HasAchieved(4608))
							{
								ChatHandler(GetSession()).PSendSysMessage("%s", "The Raid leader must have completed Fall of the Lich King (25 players) first.");
								return false;
							}
							break;
						}
		...
		// Here, access requirement - bandera
		...
		if (mapEntry->MapID == 650)
		{
			if (!player->HasAchieved(2891) && !player->HasAchieved(2890))
			{
				ChatHandler(GetSession()).PSendSysMessage("%s", "You must complete either The Keepers of Ulduar (10 players) or The Keepers of Ulduar (25 players) first.");
				SendTransferAborted(target_map, TRANSFER_ABORT_DIFFICULTY, target_difficulty);
				return false;
			}
			return true;
		}
		else if (mapEntry->MapID == 632) // Forge of Souls
		{
			if (!player->HasAchieved(3917) && !player->HasAchieved(3916))
			{
				ChatHandler(GetSession()).PSendSysMessage("%s", "You must complete either Trial of the Crusader (10 players) or Trial of the Crusader (25 players) first.");
				SendTransferAborted(target_map, TRANSFER_ABORT_DIFFICULTY, target_difficulty);
				return false;
			}
			return true;
		...
		if (LevelMin || LevelMax || missingItem || missingQuest || missingAchievement)
		{
			if (report)
			{
				...
				else if (missingQuest && !ar->questFailedText.empty())
					ChatHandler(GetSession()).PSendSysMessage("%s", ar->questFailedText.c_str());
...
void Player::StopCastingCharm()
{
    ...
    if (GetCharmGUID())
    {
        ...
        if (charm->GetCharmerGUID())
        {
            ...
            // ABORT();
            charm->ToCreature()->DespawnOrUnsummon(1);
...
bool Player::ActivateTaxiPathTo(std::vector<uint32> const& nodes, Creature* npc /*= NULL*/, uint32 spellid /*= 0*/)
{
    ...
    if (npc)
    {
        firstcost = (uint32)ceil(firstcost*GetReputationPriceDiscount(npc));
...
void Player::SendInitialPacketsAfterAddToMap()
{
	...
	if (GetMap()->GetId() == 631)
		SendRaidDifficulty(GetGroup() != NULL, GetGroup() ? GetGroup()->GetDifficulty(true) : GetDifficulty(true));
	else
		if (GetMap()->IsRaid())
        {
...
void Player::CheckTitansGrip()
{
    // SPELL_WARRIOR_TITANS_GRIP_DAMAGE_REDUCTION = 49152
    if (!CanTitanGrip())
        RemoveAurasDueToSpell(49152);
    else
    {
        Item* mainHand = GetWeaponForAttack(BASE_ATTACK);
        Item* offHand = GetWeaponForAttack(OFF_ATTACK);
        if (mainHand && offHand && (mainHand->GetTemplate()->InventoryType == INVTYPE_2HWEAPON || offHand->GetTemplate()->InventoryType == INVTYPE_2HWEAPON))
        {
            if (!HasAura(49152))
...
uint32 Player::GetRuneBaseCooldown(uint8 index, bool skipGrace)
{
    ...
    if (!skipGrace)
        cooldown -= GetRuneGracePeriod(index) < 250 ? 0 : GetRuneGracePeriod(index) - 250;
...
void Player::ResyncRunes(uint8 count)
{
    ...
    for (uint32 i = 0; i < count; ++i)
    {
        ...
        data << uint8(255 - ((GetRuneCooldown(i) * 255) / RUNE_BASE_COOLDOWN));     // passed cooldown time (0-255)
    }
    GetSession()->SendPacket(&data);
}
...
void Player::ActivateSpec(uint8 spec)
{
    ...
    // Needs to check this after total spec change, some pets are/might-be-in-the-future dependent on talent auras
    if (Pet* pet = GetPet())
    {
        // This case applies only to DK ghoul?
        // We changed to a new spec without having master of ghouls talent, wipe the perma pet outta existence
        if (pet->GetEntry() == 26125 && !HasAura(52143))
            RemovePet(pet, PET_SAVE_AS_DELETED);
        else
    ...
    sScriptMgr->OnTalentSpecializationChange(this);
}
...
void Player::SetMusicId(uint32 musicId)
{
    m_MusicId = musicId;
    m_MusicTimer = MUSIC_UPDATE_INTERVAL;
}
...
Pet* Player::SummonPet(uint32 entry, float x, float y, float z, float ang, PetType petType, uint32 duration)
{
    ...
    if (petType == SUMMON_PET && pet->LoadPetFromDB(this, entry))
    {
        ...
        //! Aspect of the beast staying active even if owner do not have the aura
        if (pet->IsHunterPet() && !HasAura(61669) && pet->HasAura(61669))
            pet->RemoveAura(61669);
...
Creature* Player::GetCritter() const
{
    if (ObjectGuid critterGuid = GetCritterGUID())
    {
        Creature* critter = ObjectAccessor::GetCreature(*this, critterGuid);

        if (!critter)
            return nullptr;

        if (IsInWorld() && critter)
            return critter;
    }
