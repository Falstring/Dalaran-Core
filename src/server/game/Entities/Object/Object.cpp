...
TempSummon* Map::SummonCreature(uint32 entry, Position const& pos, SummonPropertiesEntry const* properties /*= NULL*/, uint32 duration /*= 0*/, Unit* summoner /*= NULL*/, uint32 spellId /*= 0*/, uint32 vehId /*= 0*/)
{
    ...
    if (summoner)
    {
        ...
        // Mass spawn report system
        switch (summoner->GetEntry())
        {
        case 26583: // Horrified Drakkari Shaman
        case 29173: // Darion Mograine
        case 32401: // King Varian
        case 32518: // Thrall
        case 23472: //(Azjol-Nerub) - World Trigger (Large AOI, Not Immune PC/NPC)
            break;
        default:
            switch(mask)
            {
            case UNIT_MASK_TOTEM: // Temp summon to skip
			case UNIT_MASK_MINION:
			case UNIT_MASK_GUARDIAN:
                break;
            default:
                summoner->IncrTempTotalSummonCount();
				if (GameTime::GetGameTime() < summoner->GetLastSummonTime() + uint32(10))
                    summoner->IncrTempSummonCount();

                // All summons in 10 seconds
                if (summoner->GetTempSummonCount(0) > 4)
                {
                    TC_LOG_WARN("entities.unit", "\n MASSIVE %s - (%s) - %s ID: %u (.go xyz %lf %lf %lf %u) has spawned %u temp summons in 10 seconds with ID: %u", TimeToTimestampStrNewFormat(time(NULL)), GetMapName(), summoner->GetName().c_str(), summoner->GetEntry(), summoner->GetPositionX(), summoner->GetPositionY(), summoner->GetPositionZ(), summoner->GetMapId(), summoner->GetTempSummonCount(0), entry);
                    summoner->ResetTempSummonCount();
                }

                // Total summon count
                if (summoner->GetTempSummonCount(1) >= 50)
                {
                    TC_LOG_WARN("entities.unit", "\n TOTAL %s - (%s) - %s ID: %u (.go xyz %lf %lf %lf %u) has spawned %u temp summons with ID: %u", TimeToTimestampStrNewFormat(time(NULL)), GetMapName(), summoner->GetName().c_str(), summoner->GetEntry(), summoner->GetPositionX(), summoner->GetPositionY(), summoner->GetPositionZ(), summoner->GetMapId(), summoner->GetTempSummonCount(1), entry);
                    summoner->ResetTempTotalSummonCount();
                }

				summoner->SetLastSummonTime(GameTime::GetGameTime());
                break;
            }
        }
    }
