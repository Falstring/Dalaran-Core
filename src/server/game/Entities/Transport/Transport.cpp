...
void Transport::AddPassenger(WorldObject* passenger)
{
    ...
    if (_passengers.insert(passenger).second)
    {
        ...
        ///@todo: Move to database (transport_music?)
        if (passenger->GetTypeId() == TYPEID_PLAYER)
        {
            const uint32 NORTHREND_TRANSPORT_DAY_MUSIC = 14952;
            const uint32 NORTHREND_TRANSPORT_NIGHT_MUSIC = 14953;
            const uint8 EVENT_NIGHTS = 25;
            switch (GetEntry())
            {
                case 181688: // Menethil Harbor, Wetlands and Valgarde, Howling Fjord ("Northspear")
                case 181689: // Undercity, Tirisfal Glades and Vengeance Landing, Howling Fjord ("Zeppelin, Horde (Cloudkisser)")
                case 186238: // Orgrimmar, Durotar and Warsong Hold, Borean Tundra ("Zeppelin, Horde (The Mighty Wind)")
                case 190536: // Stormwind Harbor and Valiance Keep, Borean Tundra ("The Kraken")
                    passenger->ToPlayer()->SetMusicId(IsEventActive(EVENT_NIGHTS) ?
                        NORTHREND_TRANSPORT_NIGHT_MUSIC :
                        NORTHREND_TRANSPORT_DAY_MUSIC);
                    break;
                default:
                    break;
            }
    ...
    if (erased || _staticPassengers.erase(passenger)) // static passenger can remove itself in case of grid unload
    {
        ...
        if (passenger->GetTypeId() == TYPEID_PLAYER)
        {
            switch (GetEntry())
            {
                ...
                case 186238: // Orgrimmar, Durotar and Warsong Hold, Borean Tundra ("Zeppelin, Horde (The Mighty Wind)")
                case 190536: // Stormwind Harbor and Valiance Keep, Borean Tundra ("The Kraken")
                    passenger->ToPlayer()->SetMusicId(0);
                    break;
                default:
                    break;
            }
