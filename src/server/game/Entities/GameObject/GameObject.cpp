        ...
        case GO_NOT_READY:
        {
            switch (GetGoType())
            {
                case GAMEOBJECT_TYPE_TRAP:
                {
                    ...
                    {
                        time_t trapDelayTime = GameTime::GetGameTimeMS() + goInfo->trap.startDelay * IN_MILLISECONDS;
                        m_cooldownTime = owner->IsInCombat() ? trapDelayTime : (trapDelayTime / 2);
                    }
            ...
            switch (GetGoType())
            {
                ...
                case GAMEOBJECT_TYPE_TRAP:
                {
                    ...
                    else if (Unit* target = ObjectAccessor::GetUnit(*this, m_lootStateUnitGUID))
                    {
                        ...
                        {
                            //@robinsch: Super bad workaround to make Explosive Trap deal initial damage versus creatures
                            switch (goInfo->trap.spellId)
                            {
                                ...
                                case 49065: // Explosive Trap Effect
                                    if (Unit* owner = GetOwner())
                                        owner->CastSpell(target, goInfo->trap.spellId, true);
                                    break;
                                default:
...
float GameObject::GetInteractionDistance()
{
    switch (GetGoType())
    {
        ...
        case GAMEOBJECT_TYPE_QUESTGIVER:
            return INTERACTION_DISTANCE + GetFloatValue(OBJECT_FIELD_SCALE_X) + (GetGOInfo()->flags & GO_FLAG_HUGE_SIZE ? 40.0f : 0.0f);
        default:
            return INTERACTION_DISTANCE_GO;
    }
}
