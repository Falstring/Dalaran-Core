...
enum TriggerCastFlags
{
    ...
    TRIGGERED_NO_PERIODIC_RESET                     = 0x00100000,   //! Will ignore periodic aura tick reset on reapply
...
{
public:
    explicit StealInfo(Unit* stealer) :
        _stealerUnit(stealer) { }

    Unit* GetStealer() const { return _stealerUnit; }
private:
    Unit* _stealerUnit;
};
...
class Unit : public WorldObject
{
    public:
        ...
        bool isAttackTimerSuspended() const { return m_attackTimerSuspended; }
        void suspendAttackTimer(bool on) { m_attackTimerSuspended = on; }
        ...
        Aura* _TryStackingOrRefreshingExistingAura(SpellInfo const* newAura, uint8 effMask, Unit* caster, int32* baseAmount = NULL, Item* castItem = NULL, ObjectGuid casterGUID = ObjectGuid::Empty, bool periodicReset = false);
        ...
        std::map<ObjectGuid, MMapTargetData> m_targetsNotAcceptable;
        bool isTargetNotAcceptableByMMaps(ObjectGuid guid, uint32 currTime, const Position* t = NULL) const { std::map<ObjectGuid, MMapTargetData>::const_iterator itr = m_targetsNotAcceptable.find(guid); if (itr != m_targetsNotAcceptable.end() && (itr->second._endTime >= currTime || t && !itr->second.PosChanged(*this, *t))) return true; return false; }
