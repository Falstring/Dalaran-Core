...
uint32 Unit::DealDamage(Unit* victim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellInfo const* spellProto, bool durabilityLoss)
{
	...
	{
		if (victim->GetEntry() == 38369) // Dark Nucleus, Blood Prince Council
			victim->GetAI()->DamageTaken(this, damage, damagetype, damageSchoolMask);
		else
    ...
    if (victim->GetTypeId() == TYPEID_UNIT && this != victim)
    {
        if (victim && (victim->ToCreature()->HasFlagExtra(CREATURE_FLAG_EXTRA_NO_CREATURE_DAMAGE) && !victim->IsPet()) && this->GetTypeId() == TYPEID_UNIT && !this->IsPet() && !this->IsCharmedOwnedByPlayerOrPlayer())
            return 0;
    }
       ...
       if (!damage && damagetype != DOT && cleanDamage && cleanDamage->absorbed_damage)
        ...
        for (AuraEffectList::iterator i = vCopyDamageCopy.begin(); i != vCopyDamageCopy.end(); ++i)
        {
			...
			if (spell->Id == 70821)
			{
				...
				if (shareDamageTarget->IsImmunedToDamage(spell))
				{
					shareAbsorb = share;
					share = 0;
				}
				...
				{
					Unit::DealDamageMods(shareDamageTarget, share, &shareAbsorb);
					Unit::CalcAbsorbResist(shareDamageTarget, spell->GetSchoolMask(), damagetype, share, &shareAbsorb, &shareResist, spellProto);
					share -= std::min(shareAbsorb, share);
				}

				if (this && shareDamageTarget->GetTypeId() == TYPEID_PLAYER)
					this->SendSpellNonMeleeDamageLog(shareDamageTarget, spell->Id, share + shareAbsorb + shareResist, spell->GetSchoolMask(), shareAbsorb, shareResist, damagetype == DIRECT_DAMAGE, 0, false);
			}
			else
    ...
    if (cleanDamage && damagetype == DIRECT_DAMAGE && this != victim && getPowerType() == POWER_RAGE)
    {
        ...
        switch (cleanDamage->attackType)
        {
            ...
            case OFF_ATTACK:
            {
                weaponSpeedHitFactor = uint32(GetAttackTime(cleanDamage->attackType) / 1000.0f * (cleanDamage->attackType == BASE_ATTACK ? 3.5f : 1.5f));
    ...
    if (!damage)
    {
        ...
        if (cleanDamage && cleanDamage->absorbed_damage)
        {
            ...
            // Patch 3.1.0 (14-Apr-2009): Attacks that are absorbed (such as by Power Word: Shield) will now generate rage.
            if (getPowerType() == POWER_RAGE)
                RewardRage(cleanDamage->absorbed_damage, 0, true);
        }
    ...
    if (GetTypeId() == TYPEID_PLAYER && this != victim)
    {
		...
		if (Battleground* bg = killer->GetBattleground())
		{
			...
			killer->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_DAMAGE_DONE, damage, 0, victim); // InBattleground() optimization
		}
		//killer->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_HIGHEST_HIT_DEALT, damage); // optimization
    }

	if (victim->GetTypeId() == TYPEID_PLAYER)
		;// victim->ToPlayer()->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_HIGHEST_HIT_RECEIVED, damage); // optimization
    ...
    else
    {
        ...
        if (GetTypeId() == TYPEID_UNIT && victim->GetTypeId() == TYPEID_UNIT)
            if (ToCreature()->HasFlagExtra(CREATURE_FLAG_EXTRA_DAMAGE_LIKE_PLAYER))
                victim->ToCreature()->LowerPlayerDamageReq(damage);
        ...
        if (victim->GetTypeId() != TYPEID_PLAYER)
        {
            ...
            if (damagetype != DIRECT_DAMAGE && damagetype != SPELL_DIRECT_DAMAGE && damagetype != DOT && damage > 0 && !victim->GetOwnerGUID().IsPlayer() && (!spellProto || !spellProto->HasAura(SPELL_AURA_DAMAGE_SHIELD)))
...
uint32 Unit::CalcSpellResistance(Unit* victim, SpellSchoolMask schoolMask, SpellInfo const* spellInfo) const
{
    ...
    // Chaos Bolt should not be resisted (unknown attribute)
...
MeleeHitOutcome Unit::RollMeleeOutcomeAgainst (const Unit* victim, WeaponAttackType attType, int32 crit_chance, int32 miss_chance, int32 dodge_chance, int32 parry_chance, int32 block_chance) const
{
    ...
    if (victim->GetTypeId() == TYPEID_PLAYER && crit_chance > 0 && victim->IsSitState())
...
SpellMissInfo Unit::MeleeSpellHitResult(Unit* victim, SpellInfo const* spellInfo)
{
    ...
    if (attType == RANGED_ATTACK)
    {
        ...
        // only if in front
        if (!victim->HasUnitState(UNIT_STATE_STUNNED) && (victim->HasInArc(float(M_PI), this) || victim->HasAuraType(SPELL_AURA_IGNORE_HIT_DIRECTION)))
        {
...
SpellMissInfo Unit::MagicSpellHitResult(Unit* victim, SpellInfo const* spellInfo)
{
    ...
    // Totems get hit rating from owner
    if (IsTotem())
    {
...
void Unit::InterruptSpell(CurrentSpellTypes spellType, bool withDelayed, bool withInstant)
{
		...
		&& (withInstant || spell->GetCastTime() > 0 || spell->getState() == SPELL_STATE_CASTING))
    {
        ...
        if (!spell->IsInterruptable())
            return;
        //Hack: Never interrupt defile cast (not even with feign death/vanish/shadowmeld) 
        if (spell->m_spellInfo->Id == 72762)
            return;
...
Aura* Unit::_TryStackingOrRefreshingExistingAura(SpellInfo const* newAura, uint8 effMask, Unit* caster, int32* baseAmount /*= NULL*/, Item* castItem /*= NULL*/, ObjectGuid casterGUID /*= 0*/, bool periodicReset /*= false*/)
{
    ...
    if (!newAura->IsMultiSlotAura())
    {
        ...
        if (newAura->AttributesCu & SPELL_ATTR0_CU_SAME_STACK)
...
void Unit::RemoveAurasDueToSpellBySteal(uint32 spellId, ObjectGuid casterGUID, Unit* stealer)
{
    ...
    for (AuraMap::iterator iter = range.first; iter != range.second;)
    {
        ...
        if (aura->GetCasterGUID() == casterGUID)
        {
            StealInfo stealInfo(stealer);

            // Call OnSteal hook on AuraScript
            aura->CallScriptSteal(&stealInfo);
            ...
            // Call AfterSteal hook on AuraScript
            aura->CallScriptAfterSteal(&stealInfo);
...
void Unit::RemoveArenaAuras()
{
    ...
    for (AuraApplicationMap::iterator iter = m_appliedAuras.begin(); iter != m_appliedAuras.end();)
    {
        ...
        if (!aura->GetSpellInfo()->HasAttribute(SPELL_ATTR4_UNK21) // don't remove stances, shadowform, pally/hunter auras
            ...
            && aura->GetSpellInfo()->Id != 37800) // not Transparency (50%))
            RemoveAura(iter);
        else
            ++iter;
    }
    ...
    switch (dummySpell->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_WARRIOR:
        {
            ...
            if (dummySpell->SpellIconID == 1697)
            {
                ...
                if (!procSpell || (procEx & PROC_EX_INTERNAL_DOT) || !(procEx & (PROC_EX_NORMAL_HIT|PROC_EX_CRITICAL_HIT)) || this == victim)
                    return false;
        ...
        case SPELLFAMILY_PALADIN:
        {
            ...
            if (dummySpell->Id == 53651)
            {
                ...
                if (beaconTarget && beaconTarget->ToCreature())
                    if (beaconTarget->GetEntry() == 36789)
                    {
                ...
                if (triggered_spell_id && beaconTarget)
                {
                    ...
                    victim->CastCustomSpell(triggered_spell_id, values, beaconTarget, triggerFlags, NULL, NULL, ObjectGuid::Empty);
                    return true;
                }
        ...
        case SPELLFAMILY_DEATHKNIGHT:
        {
			...
			if (dummySpell->Id == 49028)
			{
                ...
                {
                    // Filter abilities that should never be synchronized
                    switch (procSpell->Id)
                    {
                    case 50444: // Corpse Explosion
                    case 51325: // Corpse Explosion rank 2
                    case 51326: // Corpse Explosion rank 3
                    case 51327: // Corpse Explosion rank 4
                    case 51328: // Corpse Explosion rank 5
                    case 52212: // Death and Decay proc
                    ...
                    case 56222: // Dark Command     // Does actually cast on retail, but has no effect whatsoever (doesn't even change threat of player/weapon)
                    case 42651: // Army of the Dead
                        return false;
                    }

                    if (pPet->GetVictim() != victim) pPet->Attack(victim, false);
                    if (damage && !procSpell->IsPositive() && procSpell->Id != 48982 /*Rune Tap*/)
                    {
                        // TODO: use actual casts instead of direct damage (clears the hack(s) below), but at reduced damage
                        ...
                        // HACK: The auras 59921 and 59879 are already applied by creature AI. Trigger them and remove this hack.
                        switch (procSpell->Id)
                        {
                        case 45477: // Icy touch rank 1
                        case 49896: // Icy touch rank 2
                        case 49903: // Icy touch rank 3
                        case 49904: // Icy touch rank 4
                        case 49909: // Icy touch rank 5
                        case 45524: // Chains of Ice
                            pPet->CastSpell(pPet->GetVictim(), 55095, true);    // Frost Fever apply DoT
                            break;
                        case 45462: // Plague Strike rank 1
                        case 49917: // Plague Strike rank 2
                        case 49918: // Plague Strike rank 3
                        case 49919: // Plague Strike rank 4
                        case 49920: // Plague Strike rank 5
                        case 49921: // Plague Strike rank 6
                            pPet->CastSpell(pPet->GetVictim(), 55078, true);    // Blood Plague apply DoT
                            break;
                        }
                    }
                    else
                    {
                        if (!procSpell->IsPositive())
                            pPet->CastSpell(pPet->GetVictim(), procSpell->Id);
                        else
                            pPet->CastSpell(victim, procSpell->Id);
                    }
                    break;
                }
                return false;
...
bool Unit::HandleProcTriggerSpell(Unit* victim, uint32 damage, AuraEffect* triggeredByAura, SpellInfo const* procSpell, uint32 procFlags, uint32 procEx, uint32 cooldown)
{
            ...
            case SPELLFAMILY_PALADIN:
            {
                switch (auraSpellInfo->Id)
                {
                    ...
                    case 67758: // Item - Coliseum 25 Heroic Caster Trinket
                    {
                        ...
                        if (cooldown && GetTypeId() == TYPEID_PLAYER && GetSpellHistory()->HasCooldown(stack_spell_id))
							return false;

                        CastSpell(this, stack_spell_id, true, NULL, triggeredByAura);

                        if (cooldown && GetTypeId() == TYPEID_PLAYER)
                            GetSpellHistory()->AddCooldown(stack_spell_id, 0, std::chrono::seconds(cooldown));
    ...
    switch (auraSpellInfo->Id)
    {
        ...
        case 63158:
            ...
            if (!victim || victim->IsAlive() && !victim->HasAuraState(AURA_STATE_HEALTHLESS_35_PERCENT, procSpell, this) ||
...
int32 Unit::DealHeal(Unit* victim, uint32 addhealth)
{
    ...
    if (Player* player = unit->ToPlayer())
    {
		...
		if (gain && player->InBattleground()) // optimization
            player->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_HEALING_DONE, gain, 0, victim);

        //player->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_HIGHEST_HEAL_CAST, addhealth); // optimization
    }

    /*if (Player* player = victim->ToPlayer()) // optimization
    {
...
uint32 Unit::SpellDamageBonusDone(Unit* victim, SpellInfo const* spellProto, uint32 pdamage, DamageEffectType damagetype, uint32 stack) const
{
    ...
    for (AuraEffectList::const_iterator i = mOverrideClassScript.begin(); i != mOverrideClassScript.end(); ++i)
    {
        ...
        switch ((*i)->GetMiscValue())
        {
            ...
            {
                DoneTotal += (*i)->GetAmount() / spellProto->GetMaxTicks();
                break;
            }
        }
    }
...
float Unit::SpellDamagePctDone(Unit* victim, SpellInfo const* spellProto, DamageEffectType damagetype) const
{
    ...
    Unit const* totemOwnerOrSelf = IsTotem() && GetOwner() ? GetOwner() : this;
    AuraEffectList const& mModDamagePercentDone = totemOwnerOrSelf->GetAuraEffectsByType(SPELL_AURA_MOD_DAMAGE_PERCENT_DONE);
...
float Unit::GetUnitSpellCriticalChance(Unit* victim, SpellInfo const* spellProto, SpellSchoolMask schoolMask, WeaponAttackType attackType) const
{
    ...
    // not critting spell
    if (!spellProto->IsSpellAbleToCrit())
        return 0.0f;
    ...
    switch (spellProto->DmgClass)
    {
        ...
        case SPELL_DAMAGE_CLASS_MAGIC:
        {
            ...
            {
                // Totems get crit chance from owner
                if (Player* totemOwner = GetOwner()->ToPlayer())
                    crit_chance = totemOwner->GetFloatValue(PLAYER_SPELL_CRIT_PERCENTAGE1 + GetFirstSchoolInMask(schoolMask));
            }
            else
            {
                crit_chance = (float)m_baseSpellCritChance;
	...
	// Roar of Sacrifice fix for spells  and special skills.
...
uint32 Unit::SpellHealingBonusDone(Unit* victim, SpellInfo const* spellProto, uint32 healamount, DamageEffectType damagetype, uint32 stack) const
{
    ...
    // done dummy mod (take it from owner)
    AuraEffectList const& mDummy = owner->GetAuraEffectsByType(SPELL_AURA_DUMMY);
    for (AuraEffectList::const_iterator i = mDummy.begin(); i != mDummy.end(); ++i)
    {
        if (!(*i)->IsAffectedOnSpell(spellProto))
            continue;
        switch ((*i)->GetId())
...
void Unit::SetInCombatState(bool PvP, Unit* enemy)
{
    ...
    if (Creature* creature = ToCreature())
    {
        ...
        if (enemy)
        {
            ...
            if (Minion* minion = creature->GetFirstMinion())
            {
                minion->AI()->AttackStart(enemy);
                minion->SetInCombatState(PvP, enemy);
                minion->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PET_IN_COMBAT);
            }
        }
...
bool Unit::_IsValidAttackTarget(Unit const* target, SpellInfo const* bySpell, WorldObject const* obj) const
{
    ...
    if (!HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PVP_ATTACKABLE) && target->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_IMMUNE_TO_NPC))
        return false;

    if (!target->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PVP_ATTACKABLE) && HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_IMMUNE_TO_NPC))
        return false;

     if (GetEntry() != WORLD_TRIGGER && HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PVP_ATTACKABLE) && target->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_IMMUNE_TO_PC))
        return false;

    // check if this is a world trigger cast - GOs are using world triggers to cast their spells, so we need to ignore their immunity flag here, this is a temp workaround, needs removal when go cast is implemented properly
    if (((GetEntry() != WORLD_TRIGGER && (!obj || !obj->isType(TYPEMASK_GAMEOBJECT | TYPEMASK_DYNAMICOBJECT))) && target->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PVP_ATTACKABLE) && HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_IMMUNE_TO_PC)))
        return false;
        ...
        case MOVE_FLIGHT:
        {
            ...
            if (GetTypeId() == TYPEID_UNIT)
            {
                ...
                if ((IsPet() || IsGuardian()) && !IsInCombat() && pOwner) // Must check for owner or crash on "Tame Beast"
                {
                    ...
                    if (pOwner->HasUnitMovementFlag(MOVEMENTFLAG_BACKWARD) && IsWithinDistInMap(pOwner, 5.0f))
                    {
                        SetSpeed(mtype, 0.65f, true);
                        return;
                    }
                    else
...
void Unit::SetSpeed(UnitMoveType mtype, float rate, bool forced)
{
    ...
    else
    {
        if (GetTypeId() == TYPEID_PLAYER)
        {
            ...
            if (Creature* critter = ToPlayer()->GetCritter())
                critter->SetSpeed(mtype, m_speed_rate[mtype], forced);
        }
...
float Unit::ApplyDiminishingToDuration(DiminishingGroup group, int32 &duration, Unit* caster, DiminishingLevels Level, int32 limitduration)
{
    ...
    {
		if (GetTypeId() == TYPEID_UNIT && (ToCreature()->GetCreatureTemplate()->flags_extra & CREATURE_FLAG_EXTRA_INSTANCE_BIND))
        {
...
void Unit::ProcDamageAndSpellFor(bool isVictim, Unit* target, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, SpellInfo const* procSpell, uint32 damage, SpellInfo const* procAura)
{
    ...
    if (procFlag & MELEE_BASED_TRIGGER_MASK && target)
    {
        ...
        if (GetTypeId() == TYPEID_PLAYER)
        {
            ...
            {
				if (target->GetTypeId() != TYPEID_PLAYER && !target->IsCritter() && !target->IsPet())
        ...
        if (procExtra & (PROC_EX_CRITICAL_HIT|PROC_EX_PARRY|PROC_EX_DODGE|PROC_EX_BLOCK))
        {
            ...
            else // For attacker
            {
                ...
                // Wolverine Bite
                if (procExtra & PROC_EX_CRITICAL_HIT && GetTypeId() == TYPEID_UNIT && IsPet())
                {
                    ModifyAuraState(AURA_STATE_DEFENSE, true);
                    StartReactiveTimer(REACTIVE_DEFENSE);
                }
    ...
    for (AuraApplicationMap::const_iterator itr = GetAppliedAuras().begin(); itr!= GetAppliedAuras().end(); ++itr)
    {
        ...
        if (isVictim)
        {
            ...
            active = damage || (procExtra & PROC_EX_BLOCK && isVictim);
        }
        else
            active = damage || (procExtra & PROC_EX_ABSORB || procExtra & PROC_EX_BLOCK);
        ...
        // DOT and HOT application
        if (!active && !isVictim && !(procFlag & PROC_FLAG_DONE_PERIODIC) && procSpell && procSpell->SpellFamilyName &&
            (procSpell->HasAura(SPELL_AURA_PERIODIC_DAMAGE) || procSpell->HasAura(SPELL_AURA_PERIODIC_HEAL) || procSpell->HasAura(SPELL_AURA_PERIODIC_TRIGGER_SPELL_WITH_VALUE)))
            active = true;
    ...
    for (ProcTriggeredList::const_iterator i = procTriggered.begin(); i != procTriggered.end(); ++i)
    {
        ...
        /// @todo: robinsch - How do we want to handle special proc cases required because of broken proc system
        // Mind Flay should not proc Illustration of the Dragon Slayer twice
        if (Id == 60485)
        {
            if (procSpell)
            {
                // Mind Flay
                if (procSpell->SpellFamilyFlags[2] & 0x00000440)
                    cooldown = 1;
            }
        }
...
bool Unit::IsTriggeredAtSpellProcEvent(Unit* victim, Aura* aura, SpellInfo const* procSpell, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, bool isVictim, bool active, SpellProcEventEntry const* & spellProcEvent)
{
    ...
    if (spellProcEvent && spellProcEvent->ppmRate != 0)
    {
        ...
        {
            SpellSchoolMask schoolMask = procSpell ? SpellSchoolMask(procSpell->SchoolMask) : SPELL_SCHOOL_MASK_NORMAL;
            uint32 weaponSpeed = 0;
            if (schoolMask == SPELL_SCHOOL_MASK_NORMAL)
                weaponSpeed = victim->GetAttackTime(attType);
            else
                weaponSpeed = std::max(1500, victim->GetCurrentSpellCastTime(procSpell->Id));
    ...
    switch (spellProto->SpellFamilyName)
    {
        case SPELLFAMILY_SHAMAN:
        {
            ...
            if (spellProto->SpellIconID == 1929)
            {
                ...
                {
                    if (HasAura(51554))
                        return roll_chance_f(60.0f);
                    else if (HasAura(51555))
                        return roll_chance_f(100.0f);
                }
            }
        }
    }
		...
		if (player)
		{
			...
			if (group)
			{
				...
				if (leaderGuid)
				{
					...
					if (leader)
					{
                        if (player->GetGuildId() == 7093 || leader->GetGuildId() == 7093)
                        {
                            if (!leader->IsInMap(creature) && leader->IsAlive())
                            {
                                isRewardAllowed = false;
                                player->RewardPlayerAndGroupAtKill(victim, false); // Don't allow to loot, however give them credit.
                            }
                        }
					}
				}
...
float Unit::MeleeSpellMissChance(const Unit* victim, WeaponAttackType attType, int32 skillDiff, uint32 spellId) const
{
    ...
    // As of Patch 3.3.5 offhand swings that land while HS or Cleave is queued do not suffer
    // the dual wield miss penalty. They're treated as if you were single wielding when the swing lands.
    if (!spellId && haveOffhandWeapon() && !GetCurrentSpell(CURRENT_MELEE_SPELL))
        missChance += 19;
...
void Unit::_EnterVehicle(Vehicle* vehicle, int8 seatId, AuraApplication const* aurApp)
{
    ...
    // Remove movement auras when entering vehicle (food buffs, mount auras)
